...
<div id="mainContent" class="main-content fade">
    <div class="center-block">
        <div class="main-header">
            <h2><?php echo $lang->modifycncc->copy;?></h2>
        </div>
                    ...
                    <table class="table table-form">
                        ...
                        <tr class="app-partition hidden">
                            ...
                            <td colspan="3" class="required">
                                <?php foreach($modifycncc->appWithPartition as $key => $line): ?>
                                    <div class="input-group">
                                        <span class="input-group-addon"><?php echo $lang->modifycncc->applicationName;?></span>
                                      <?php echo html::select('app[]', $apps, $line[0], "id='app$key' data-index='$key' class='form-control chosen effectApp' onchange='selectApp(this.value,this.id)'");?>
                                        <span class="input-group-addon fix-border fix-padding"><?php echo $lang->modifycncc->partitionName;?></span>
                                      <?php echo html::select('partition[]', [],  count($line)==2?$line[1]:' ', "id='partition$key' class='form-control chosen'");?>
...
<script>
    ...
    function selectApp(app,id)
    {
        ...
        if(code){
            ...
            {
                var current = $(data)
                current.attr({'id':'partition' + index})
    ...
    function addPartition(obj)
    {
        ...
        var $currentRow = $(obj).parent().clone();
        ...
        $currentRow.find('#app' + originIndex).attr({'id':'app' + partitionIndex});

        $currentRow.find('#partition' + originIndex + '_chosen').remove();
        $currentRow.find('#partition' + originIndex).attr({'id':'partition' + partitionIndex});
        // $currentRow.find('.picker').css('display','none');


        $(obj).parent().after($currentRow);
    ...
    {
        var $currentRow = $(obj).parent();

        if($("select[name*='app[]']").length > 1)
        {
            $currentRow.remove();
        }
