...
class actionModel extends model
{
     ...
     * @param $historyLogChanges
     * @access public
     * @return int
     */
    public function create($objectType, $objectID, $actionType, $comment = '', $extra = '', $actor = '', $autoDelete = true, $isSetHistory = false, $historyLogChanges = array())
    {
        $this->session->set('mailObjectType',$objectType);
        $this->session->set('mailObjectId',$objectID);
        $this->session->set('mailActionType',$actionType);
        ...
        if($isSetHistory &&  !empty($historyLogChanges)){
            $this->logHistory($actionID, $historyLogChanges);
        }
        ...
        /*触发消息-待办 or 已办 */
        //消息开关
        if (isset($this->config->global->pushMsgEnable) && $this->config->global->pushMsgEnable != 'disable'){
        //触发消息
          $this->loadModel("messagecenter")->pushMessageTo($objectType,$objectID,$actionType);
        }
        return $actionID;
    }

    /**
     * Create a siampleCreate.
     * 应用场景 只做历史记录 不发邮件或送消息等场景
     * @param  string $objectType
     * @param  int    $objectID
     * @param  string $actionType
     * @param  string $comment
     ...
     * @return int
     */
    public function siampleCreate($objectType, $objectID, $actionType, $comment = '', $extra = '', $actor = '', $autoDelete = true, $isSetHistory = false, $historyLogChanges = array())
    {
        if(strtolower($actionType) == 'commented' and empty($comment)) return false;
    ...
    /**
     * 同时多个记录 如果source = 1,则表示需求条目、任务、意向的联动   =0表示用于条目、问题与制版二线等联动数句显示 =2正向
     * @param $objectType
     * @param $ids
     * @param $status
     * @param $lang
     ...
     * @param int $source
     */
    public function createActions($objectType, $ids, $status,$lang,$source=0)
    {
        if(empty($ids)) return;
        foreach ($ids as $id){
            $code = trim(implode(',',$id['code']),',');
            if($source == 1){ //则表示需求条目、任务、意向的联动 目的区分正向拆分历史记录
                //$comment = '由 '.$code.' : '.zget($lang,$id['oldStatus'],'').' 更新为 '.zget($lang,strstr($status,'script',true),'');
                $comment = sprintf($this->lang->action->actionNotesDesc,$code.' : '.zget($lang,$id['oldStatus'],''), zget($lang,strstr($status,'script',true),''));
            }else if($source == 2){//实时更新状态
                $comment = sprintf($this->lang->action->actionNotesDesc,$code.' : '.zget($lang,$id['oldStatus'],''), zget($lang,strstr($status,'actual',true),''));
            }else if($source == 3){//倒挂增加历史记录
                $comment = sprintf($this->lang->action->actionNotesDesc,$code.' : '.zget($lang,$id['oldStatus'],''), zget($lang,$status,''));
            }else{
                $comment = sprintf($this->lang->action->actionNotesDesc,$code.' : '.zget($lang,$id['oldStatus'],''), zget($lang,$id['status'],''));
            }
            $this->create($objectType, $id['id'], $status, $comment, '', 'guestjk');
        }
    }
    ...
    public function getRelatedFields($objectType, $objectID)
    {
        ...
        if(strpos(',story,productplan,release,task,build,bug,case,testtask,doc,issue,risk,', ",{$objectType},") !== false)
        {
            ...
            if($objectType == 'release' && isset($record->build)) $record->project = $this->dao->select('project')->from(TABLE_BUILD)->where('id')->eq($record->build)->fetch('project');
            if($objectType == 'task')
            {
                if($record->story != 0)
                {
...
//         exit();
     ...
     * @return array
     */
    public function getActionInfoList($objectType, $objectID, $action = '')
    {
        $commiters = $this->loadModel('user')->getCommiters();
        $actions   = $this->dao->select('*')->from(TABLE_ACTION)
            ->beginIF($objectType == 'project')
    ...
    public function printAction($action, $desc = '')
    {
        ...
        if ($action->extra == 'mobile'){
            $desc = str_replace('<strong>$extra</strong>','',$desc);
            $desc = str_replace('</strong>','</strong> 在移动端 ',$desc);
        }
    ...
    public function returnAction($action, $desc = '')
    {
        $line = '';
        if(!isset($action->objectType) or !isset($action->action)) return false;
        ...
        if(is_array($desc))
        {
            ...
            if(isset($desc['extra'][$extra]))
            {
                $line .= str_replace('$extra', $desc['extra'][$extra], $desc['main']);
            }
            else
            {
                $line .= str_replace('$extra', $action->extra, $desc['main']);
            }
        }
        else
        {
     ...
     * @return void
     */
    public function printChangesCh($objectType, $histories, $canChangeTag = true)
    {
        if(empty($histories)) return;
        ...
        $plans = $this->loadModel('productplan')->getPairs('1', '0', '', true);
        ...
        foreach($histories as $history)
        {
            ...
            {
                global $lang;
                if($history->field == 'Implementation' || $history->field == 'dept'){
                    $history->old = ltrim($depts["$history->old"],'/');$history->new = ltrim($depts["$history->new"],'/');
                }elseif($history->field == 'assignFor' || $history->field == 'submitter' || $history->field == 'dealuser' || $history->field == 'processingBy'){
                    $history->old = $users["$history->old"];$history->new = $users["$history->new"];
                }elseif($history->field == 'type'){
                    $history->old = zget($lang->demandcollection->typeList,$history->old);
                    $history->new = zget($lang->demandcollection->typeList,$history->new);
                }elseif($history->field == 'Expected' || $history->field == 'Actual'){
                    $history->old = $plans["$history->old"];$history->new = $plans["$history->new"];
                }elseif($history->field == 'state'){
                    $history->old = zget($lang->demandcollection->statusList,$history->old);
                    $history->new = zget($lang->demandcollection->statusList,$history->new);
                }else if($history->field == 'copyFor'){
                    $old = '';$new = '';
                    $oldArr = explode(',',ltrim($history->old,','));
                    foreach($oldArr as $user){
                        $old .= ','.$users["$user"];
                    }
                    $history->old = ltrim($old,',');
                    $newArr = explode(',',ltrim($history->new,','));
                    foreach($newArr as $user){
                        $new .= ','.$users["$user"];
                    }
                    $history->new = ltrim($new,',');
                }
                printf($this->lang->action->desc->diff1, $history->fieldLabel, $history->old, $history->new);
            }
        }
     ...
     * @return void
     */
    public function getPrintChangesCh($objectType, $histories, $canChangeTag = true)
    {
        if(empty($histories)) return;
        ...
        foreach($histories as $history)
        {
            ...
            if($history->diff != '')
            {
                ...
                $history->diff      = nl2br($history->diff);
                $history->noTagDiff = $canChangeTag ? preg_replace('/&lt;\/?([a-z][a-z0-9]*)[^\/]*\/?&gt;/Ui', '', $history->diff) : '';
                $line .= sprintf($this->lang->action->desc->diff2, $history->fieldLabel, $history->noTagDiff, $history->diff);
            }
            else
            {
                global $lang;
                ...
                $line .= sprintf($this->lang->action->desc->diff1, $history->fieldLabel, $history->old, $history->new);
            }
        }
        return $line;
    }
    ...
    public function undelete($actionID)
    {
        ...
        elseif($action->objectType == 'outsideplan')
        {
            $this->dao->update(TABLE_OUTSIDEPLANTASKS)->set('deleted')->eq('0')->where('outsideProjectPlanID')->eq($action->objectID)->exec();
            $this->dao->update(TABLE_OUTSIDEPLANSUBPROJECTS)->set('deleted')->eq('0')->where('outsideProjectPlanID')->eq($action->objectID)->exec();
        }
     ...
     * 消息id，支持多个，数组
     */
    public function pushMessages($msgIds){
        $this->app->loadLang('outwarddelivery');
        $this->app->loadLang('modify');
        $this->app->loadLang('sectransfer');
        $this->app->loadLang('mobileapi');
        $this->app->loadConfig('mobileapi');
        $this->app->loadConfig('problem');

        $url = $this->config->global->pushMessageUrl;
        $h5server = $this->config->global->h5url;
        $pcserver = $this->config->global->pcurl;
        $headers[] = 'X-AppId: ' . $this->config->global->pushMsgAppid;
        $headers[] = 'X-AppSecret: ' . $this->config->global->pushMsgAppSecret;
        if ($this->config->global->pushMsgEnable == 'disable'){
            return true;
        }
        $users = $this->loadModel('user')->getPairs('noletter|noclosed');
        $server   = $this->loadModel('im')->getServer('zentao');
        if (!isset($_SERVER['SERVER_NAME']) || $_SERVER['SERVER_NAME'] != 'dpmp.cfit.cn'){
            $server = "http://" . $_SERVER['HTTP_HOST'];
        }
        $list = $this->dao->select("*")->from(TABLE_NEED_DEAL_MESSAGE)
            ->where('id')->in($msgIds)
        ...
        foreach ($list as $k => $v) {
            $data = [];
            $data['appurl']            = $h5server.'/cfitpmsh5/#/' . $v->objectType . '/view?id='.$v->objectId.'&source=0';
            $data['createdatetime']    = $v->formCreatedDate;
            $data['creatorId']         = $v->formCreatedBy;
            $data['creatorName']       = $users[$v->formCreatedBy];
            $data['flowid']            = $v->objectType.'-'.$v->objectId;
            $data['msgType']           = '1';
//            $data['nodename']          = $v->status;
            $data['nodename']          = $this->lang->{$v->objectType}->{$this->config->mobileapi->objectStatusList[$v->objectType]}[$v->formStatus];
//            $data['pcurl']             = $server . helper::createLink($v->objectType, 'view', "id=$v->objectId", 'html');
            $data['pcurl']             = $pcserver . '/' . $v->objectType . '-view-' . $v->objectId . '.html';
            $data['receivedatetime']   = $v->createdDate;
//            $data['receiver']          = $v->reviewer;
            $data['receiverId']        = $v->reviewer;
            $data['receiverName']      = $users[$v->reviewer];
//            $data['requestContent']    = htmlspecialchars_decode($v->desc);
            $data['requestContent']    = html_entity_decode(strip_tags(str_replace("<br />","\n",strval(htmlspecialchars_decode($v->desc,ENT_QUOTES)))));;
            $data['syscode']           = 'CFITPMS';
//            $data['requestname']       = sprintf($this->lang->api->msgTitle, $this->lang->api->objectTypeList[$v->objectType], $this->lang->api->msgTitleEnd[$v->status]);
            $data['requestname']       = $this->lang->api->objectTypeList[$v->objectType];
            $data['status']            = '0';
            // 发送已办、删除时状态要和之前发送待办的状态一致
            if (in_array($v->status,[2,3])){
                $res = $this->dao->select('`before`')->from(TABLE_CONSUMED)
                    ->where('objectType')->eq($v->objectType)
                    ->andWhere('objectID')->eq($v->objectId)
                    ->andWhere('`after`')->eq($v->formStatus)
                    ->andWhere('deleted')->eq('0')
                    ->orderBy('id_desc')
                    ->fetch();
                $data['nodename']          = $this->lang->{$v->objectType}->{$this->config->mobileapi->objectStatusList[$v->objectType]}[$res->before];
            }
            if ($v->status == 2){
                $data['status']        = '2';
                $data['appurl']        = $h5server.'/cfitpmsh5/#/' . $v->objectType . '/view?id='.$v->objectId.'&source=1';
            }
            if ($v->status == 3){
                $data['status']        = '10';
                $data['appurl']        = $h5server.'/cfitpmsh5/#/' . $v->objectType . '/view?id='.$v->objectId.'&source=1';
     ...
     * @return array
     */
    public function getActionListByObjectIds($objectType, $objectIds){
        $data = [];
        if(!($objectType && $objectIds)){
            return $data;
        }
        $ret = $this->dao->select('*')->from(TABLE_ACTION)
            ->where('objectType')->eq($objectType)
        ...
        $actionIds = array_column($ret, 'id');
        //查询日志详情
        $historyRet =  $this->dao->select('*')
            ->from(TABLE_HISTORY)
            ->where('action')->in($actionIds)
            ->fetchAll();
        $historyData = [];
        if($historyRet){
            foreach ($historyRet as $val){
                $action = $val->action;
                $historyData[$action][] = $val;
            }
        }
        foreach ($ret as $val){
            $objectID = $val->objectID;
            $actionId = $val->id;
            $historyList = zget($historyData, $actionId, []);
