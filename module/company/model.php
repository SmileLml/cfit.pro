...
class companyModel extends model
{
    ...
    public function getUsers($browseType = 'inside', $type, $queryID, $deptID, $sort, $pager)
    {
        ...
        if($type == 'bydept')
        {
           ...
           // return $this->dept->getUsers($browseType, $childDeptIds, $pager, $sort);
            return $this->getAllUsers($browseType, $childDeptIds, $pager, $sort);
        }
        else
        {
            if($queryID)
            ...
            return $this->getByQuery($browseType, $this->session->userQuery, $pager, $sort);
        }
    }
    ...
    {
        if($query && strpos($query, '`account`') !== false)
        {
            $query = str_replace("`account`", "u.account", $query);

        }
        if($query && strpos($query, '`role`') !== false)
        {
            $query = str_replace("`role`", "u.role", $query);

        }
        if($query && strpos($query, '`id`') !== false)
        {
            $query = str_replace("`id`", "u.id", $query);

        }
         return  $this->dao->select('u.*,group_concat(distinct zg.name)usergroup')->from(TABLE_USER)->alias('u')
            ->leftJoin(TABLE_USERGROUP)->alias('zu')
            ->on('u.account = zu.account')
            ->leftJoin(TABLE_GROUP)->alias('zg')
            ->on('zg.id = zu.`group`')
            ->where('deleted')->eq(0)
     ...
     * @return array
     */
    public function getAllUsers($browseType = 'inside', $deptID, $pager = null, $orderBy = 'u.id')
    {
       return  $this->dao->select('u.*,group_concat(distinct zg.name)usergroup')->from(TABLE_USER)->alias('u')
            ->leftJoin(TABLE_USERGROUP)->alias('zu')
            ->on('u.account = zu.account')
