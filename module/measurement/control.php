...
class measurement extends control
{
     ...
     * @return void
     */
    public function browse($browseType = 'all', $param = 0, $orderBy = 'purpose,order_desc', $recTotal = 0, $recPerPage = 10, $pageID = 1)
    {
     ...
     * @return void
     */
    public function delete($measurementID, $confirm = 'no')
    {
        if($confirm == 'no')
        {
            die(js::confirm($this->lang->measurement->confirmDelete, $this->createLink('measurement', 'delete', "measurementID=$measurementID&confirm=yes"), ''));
        }
        else
        {
     ...
     * @return void
     */
    public function searchMeas($types, $measurementID)
    {
        if($types == 'derivemeas')
        {
     ...
     * @return void
     */
    public function template($type = 'complex', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        $this->app->loadClass('pager', $static = true);
    ...
    public function createTemplate()
    {
        if($_POST)
        {
            ...
            $this->send(array('result' => 'success', 'message' => $this->lang->measurement->saveSuccess, 'locate' => $locate));
        }
     ...
     * @return void
     */
    public function viewTemplate($templateID = 0, $projectID = 0)
    {
        ...
        if($_POST)
        {
            ...
            $this->display('measurement', 'report');
            exit;
        }
     ...
     * @return void
     */
    public function setSQL($measurementID)
    {
        $measurement = $this->measurement->getByID($measurementID);
        if($_POST)
        {
            ...
            {
                if(empty($varName)) $this->send(array('result' => 'fail', 'errors' => $this->lang->measurement->tips->noticeVarName));
                ...
                if(!empty($errors)) $this->send(array('result' => 'fail', 'errors' => join("<br>", $errors)));
            ...
            if($queryResult === false) $this->send(array('result' => 'fail', 'message' => $this->measurement->errorInfo));

            if($this->post->action == 'save') $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate' => inlink('browse')));
            $this->send(array('result' => 'success', 'queryResult' => sprintf($this->lang->measurement->saveSqlMeasSuccess, $queryResult)));
        }
    ...
    public function getSqlParams()
    {
        ...
        if(!$postFunction) die("sql error."); //@todo fix error message.
     ...
     * @return void
     */
    public function setTips($type = 'progress', $object = 'SPI')
    {
        ...
        if(strtolower($this->server->request_method) == "post")
        {
            ...
            foreach($mins as $key => $min)
            {
                ...
                {
                    if($min < 0 || $max < 0) $this->send(array('result' => 'fail', 'message' => $this->lang->custom->numberError));
                }
                if(!preg_match("/^(\-|\+?)\d+(\.\d+)?$/", $min))  $this->send(array('result' => 'fail', 'message' => $this->lang->custom->regionMustNumber));
                if(!preg_match("/^(\-|\+?)\d+(\.\d+)?$/", $max))  $this->send(array('result' => 'fail', 'message' => $this->lang->custom->regionMustNumber));
                if(empty($tip)) $this->send(array('result' => 'fail', 'message' => $this->lang->custom->tipNotEmpty));
            ...
            if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
            $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate' => $this->createLink('measurement', 'setTips', "type=$type&object=$object")));
        }
     ...
     * @return void
     */
    public function ajaxSetParams($measurementID = 0)
    {
        $measurement = $this->measurement->getByID($measurementID);
        ...
        {
            if(empty($varName))            $this->send(array('result' => 'fail', 'message' => $this->lang->measurement->tips->noticeVarName));
            if(empty($data->showName[$i])) $this->send(array('result' => 'fail', 'message' => sprintf($this->lang->measurement->tips->noticeShowName, $data->showName[$i])));
            if(empty($data->varType[$i]))  $this->send(array('result' => 'fail', 'message' => sprintf($this->lang->measurement->tips->noticeVarType, $data->varName[$i])));
        ...
        $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'callback' => "hideParamForm()"));
    }
     ...
     * @return void
     */
    public function ajaxGetParamControl($controlType, $optionType = '', $value = '')
    {
     ...
     * @return void
     */
    public function ajaxCheckElementNeedParams($holderValue)
    {
     ...
     * @return void
     */
    public function ajaxBuildParamForm($holderValue)
    {
        $holderValue = base64_Decode($holderValue);
        $sysData     = $this->measurement->parseHolderValue($holderValue);
     ...
     * @return void
     */
    public function ajaxGetModuleActions($module)
    {
        $actions = $this->measurement->getModuleActions($module);
        die(html::select('action', $actions, '', "class='form-control chosen'"));
    }
    ...
    public function batchEdit()
    {
        ...
        elseif($_POST)
        {
            ...
            if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
            die(js::locate($this->createLink('measurement', 'browse'), 'parent'));
        }
