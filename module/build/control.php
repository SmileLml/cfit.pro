...
class build extends control
{
    ...
    public function view($buildID, $type = 'buildInfo', $link = 'false', $param = '', $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 100, $pageID = 1)
    {
        ...
        $this->view->generatedBugs     = $this->bug->getProjectBugs($build->project, $build->product, $build->id, $type, $type == 'generatedBug' ? $orderBy : 'status_desc,id_desc', '', $generatedBugPager);
    ...
    public function delete($buildID, $confirm = 'no')
    {
        if($confirm == 'no') {
          //  die(js::confirm($this->lang->build->confirmDelete, $this->createLink('build', 'delete', "buildID=$buildID&confirm=yes")));
            return print(js::confirm($this->lang->build->confirmDelete, inlink('delete', "buildID=$buildID&confirm=yes")));
        }
        else
        {
            ...
            //同步删除工时
            $this->build->deleteTaskEstimate($build->execution,$build->taskid,$buildID);
            $this->executeHooks($buildID);
    ...
    public function ajaxGetProductBuilds($productID, $varName, $build = '', $branch = 0, $index = 0, $type = 'normal')
    {
        ...
        if($varName == 'openedBuild' )
        {
            ...
            $disableBuilds = '';
            if(empty($builds)) $disableBuilds = 'disabled="disabled"';
            die(html::select($varName . '[]', $builds, $build, 'size=4 class=form-control multiple onchange="loadProductLinkPlans()" ' . $disableBuilds));
        }
     ...
     * @return string
     */
    public function ajaxGetProductBuildsOnlist($productID, $varName, $build = '', $branch = 0, $index = 0, $type = 'normal')
    {
        $branch = $branch ? "0,$branch" : $branch;
        $isJsonView = $this->app->getViewType() == 'json';
        if($varName == 'openedBuild' )
        {
            ...
            $builds = $this->build->getProductBuildPairs($productID, $branch, $params);
            if($isJsonView) die(json_encode($builds));
            else die(html::select($varName . '[]', $builds, $build, 'size=4 class="form-control control-build" multiple  onchange="loadPlansOnList(this,'.$branch.')"'));
        }
        if($varName == 'openedBuilds' )
        {
            $builds = $this->build->getProductBuildPairs($productID, $branch, 'noempty');
            if($isJsonView) die(json_encode($builds));
            else die(html::select($varName . "[$index][]", $builds, $build, 'size=4 class="form-control  control-build" multiple  onchange="loadPlansOnList(this,'.$branch.')"'));
        }
        if($varName == 'resolvedBuild')
        {
            $params = ($type == 'all') ? '' : 'noterminate, nodone';
            $builds = $this->build->getProductBuildPairs($productID, $branch, $params);
            if($isJsonView) die(json_encode($builds));
            else die(html::select($varName, $builds, $build, "class='form-control  control-build' "));
        }
    }
    ...
    {
        if (!empty($_POST)) {
            $buildId = $this->build->rebuild($buildID);
            if(dao::isError()) $this->send(array('result' => 'fail', 'message' => dao::getError()));
            $this->loadModel('action')->create('build', $buildId, 'rebuild',$this->lang->build->oldRelease.$buildID.'<br>'.$this->lang->build->comment.$this->post->desc);
     ...
     */
    public function workloadEdit($buildID = 0, $consumedID = 0)
    {
        if($_POST)
        {
            $changes = $this->build->workloadEdit($buildID, $consumedID);
            ...
            $actionID = $this->loadModel('action')->create('build', $buildID, 'editeStatus', $this->post->comment);
        ...
        $this->view->title    = $this->lang->build->workloadEdit;
        $build  = $this->build->getByID($buildID);
        $this->view->build  = $build;
        $this->view->users    = $this->loadModel('user')->getPairs('noclosed');

        $consumed = $this->build->getConsumedByID($consumedID);
        //相关配合人员详情信息
        $consumed->details = $this->loadModel('consumed')->getConsumedDetailsArray($consumed->details);
        if($consumed->after == 'verifysuccess' and $build->systemverify == '0')
        {
            $consumed->after = 'testsuccess';
        }
        $this->view->consumed = $consumed;

        //检查是否是最后一条工作量信息
        $isLastConsumed =  $this->loadModel('consumed')->checkIsLastConsumed($consumedID, $buildID, 'build');
     ...
     * @param int $consumedID
     */
    public function workloadDelete($buildID = 0, $consumedID = 0)
    {
        if($_POST)
        {
            $changes = $this->build->workloadDelete($buildID, $consumedID);
            ...
            $actionID = $this->loadModel('action')->create('build', $buildID, 'deleteStatus', $this->post->comment);
            $this->action->logHistory($actionID, $changes);
        ...
        $this->view->title   = $this->lang->build->workloadDelete;
        $this->view->build = $this->build->getByID($buildID);
        $this->display();
    }
     ...
     * @param int $consumedID
     */
    public function workloadDetails($buildID = 0, $consumedID = 0)
    {
        $this->view->title    = $this->lang->build->workloadDetails;
        $this->view->users    = $this->loadModel('user')->getPairs('noletter');
    ...
    public function batchDeal($buildID)
    {
        ...
        if($_POST)
        {
            $changes = $this->build->batchDeal($buildID);
            ...
            $buildIDs = explode(',',$buildID);
            foreach ($buildIDs as $id){
                $actionID = $this->loadModel('action')->create('build', $id, 'batchDeal');
                if($changes) $this->action->logHistory($actionID, $changes[$id]);
            }
     ...
     * @return int
     */
    public function ignore($buildID, $confirm = 'no')
    {
        if($confirm == 'no') {
            return print(js::confirm($this->lang->build->confirmIgnore, inlink('ignore', "buildID=$buildID&confirm=yes")));
        } else {
            $build = $this->build->getById($buildID);
            $changes = $this->build->ignore($buildID);
            /* if ajax request, send result. */
            if (!dao::isError()) {
                $actionID = $this->loadModel('action')->create('build', $buildID, 'ignore');
                $this->action->logHistory($actionID, $changes);
            }
            if ($this->server->ajax) {
                if (dao::isError()) {
            ...
            if ($this->app->openApp == 'project') {
                $link = $this->createLink('project', 'build', "projectID=$build->project");
            } else if ($this->app->openApp == 'my') {
                $link = $this->createLink('my', 'work', "mode=audit&type=build");
            } else {
                $link = $this->createLink('execution', 'build', "executionID=$build->execution");
            }
            die(js::locate($link, 'parent'));
        }
    ...
    /**
     * AJAX: get builds of a by app/project/product in html select.
     *
     * @param  string $app
     ...
     * @return string
     */
    public function ajaxGetProductBuildsByJoins($app = '', $projects = '', $productID = '', $varName, $build = '', $branch = 0, $index = 0, $type = 'normal')
    {
        $branch = $branch ? "0,$branch" : $branch;
        $isJsonView = $this->app->getViewType() == 'json';
        if($varName == 'openedBuild' )
        {
            $params = ($type == 'all') ? 'noempty' : 'noempty, noterminate, nodone';
            $builds = $this->build->getPairsByJoins($app, $projects, $productID, $branch, $params);
            if($isJsonView) die(json_encode($builds));
            else die(html::select($varName . '[]', $builds, $build, 'size=4 class=form-control multiple'));
        }
        if($varName == 'openedBuilds' )
        {
            $builds = $this->build->getPairsByJoins($app, $projects, $productID, $branch, 'noempty');
            if($isJsonView) die(json_encode($builds));
            else die(html::select($varName . "[$index][]", $builds, $build, 'size=4 class=form-control multiple'));
        }
        if($varName == 'resolvedBuild')
