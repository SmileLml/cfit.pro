...
class executionModel extends model
{
    ...
    {
        $executions = $this->getPairs($this->session->project, 'all', 'nocode');
        ...
        // 将一级导航左上角下拉切换菜单，设置到二级导航菜单。
        // $this->lang->switcherMenu = $this->getSwitcher($executionID, $this->app->rawModule, $this->app->rawMethod);
        $this->lang->modulePageNav = $this->select($executions, $executionID, 0, $this->app->rawModule, $this->app->rawMethod);

        // 原来此处为执行下的所有菜单设置参数。
        //common::setMenuVars('execution', $executionID);

        foreach($this->lang->waterfall->menu->task['subMenu'] as $label => $menu)
        {
            $this->lang->waterfall->menu->task['subMenu']->$label = common::setMenuVarsEx($menu, $executionID);
        }
    }
    ...
    public function saveState($executionID, $executions)
    {
        ...
        // 判断如果从session和cookie中都没有获取到executionID，则按照先进入未完成一级阶段，一级阶段都完成，则取第一个一级阶段。
        if($executionID == 0 and $this->session->execution == '')
        {
            // 把所有阶段的级别和状态取出来, 区分一级和二级阶段。
            $allExecution = $this->dao->select('id,grade,status')->from(TABLE_EXECUTION)->where('id')->in(array_keys($executions))->orderBy('order_asc')->fetchAll('id');
            $allTopExecution   = array();
            $allChildExecution = array();
            foreach($allExecution as $executionInfo)
            {
                if($executionInfo->grade == 1) $allTopExecution[$executionInfo->id] = $executionInfo;
                if($executionInfo->grade == 2) $allChildExecution[$executionInfo->id] = $executionInfo;
            }

            // 循环拿第一个未完成的一级阶段。
            //foreach($allTopExecution as $topExecution)
            //{
            //    if($topExecution->status != 'done')
            //    {
            //        $this->session->set('execution', $topExecution->id);
            //        break;
            //    }
            //}

            // 循环拿第一个未完成的二级阶段。
            //if($this->session->execution == '')
            //{
            //    foreach($allChildExecution as $childExecution)
            //    {
            //        if($childExecution->status != 'done')
            //        {
            //            $this->session->set('execution', $childExecution->id);
            //            break;
            //        }
            //    }
            //}
            ...
            if($this->session->execution == '')
            {
                $this->session->set('execution', key($allChildExecution));
            }
        }
    ...
    /**
     * get executions by ProjectIdList
     *
     * @param array $projectIDList
     * @acces public
     * @return array
     */
    public function getPairsByProjectIdList($projectIDList)
    {
    ...
    {
        if(!$executionID){
            $this->loadModel('action')->create('execution', $executionID, 'Managed', "维护阶段产品时获取阶段编号为 :{$executionID}，维护失败！".'<br>'.'产品编号：'.implode(',',$products));
            return;
        }
        $this->loadModel('user');
     ...
     * @return void
     */
    public function buildBugSearchForm($productsPairs, $queryID, $actionURL,$productID = 0, $type = 'execution')
    {
        $modules = array();
        $builds  = array();
        $plans  = array();
        ...
        {
            $products = array_keys($productsPairs);
            $products = array_filter($products, function($productID) {
                return $productID > 0;
            });
            $plans    = $this->loadModel('productplan')->getPairsOfProductName($products);
        }
        ...
        $childTypeList = $this->loadModel('bug')->getChildTypeParentNameList();
    ...
    public function getKanbanTasks($executionID, $orderBy = 'status_asc, id_desc', $pager = null)
    {
        ...
        foreach($tasks as $key => $task)
        {
            $isParent = $this->dao->select('id')->from(TABLE_TASK)->where('parent')->eq($task->id)->fetch('id');
            if($isParent) unset($tasks[$key]);
        }
    ...
    public function echoChildren($task, $tasks, $field, $users, $browseType, $branchGroups, $modulePairs, $useDatatable, $canRecordEstimate, $projectClosed = false)
    {
        $this->loadModel('task');
        $customFields = $this->loadModel('datatable')->getSetting('execution');

        $i = 0;
        foreach($tasks as $child)
        {
            $class  = $i == 0 ? ' table-child-top' : '';
            $class .= ($i + 1 == !empty($task->children)) ? ' table-child-bottom' : '';
            echo "<tr class='table-children {$class} parent-{$task->id}' data-id='{$child->id}' data-status='{$child->status}' data-estimate='{$child->estimate}' data-consumed='{$child->consumed}' data-left='{$child->left}'>";
            foreach($customFields as $field) $this->task->printCell($field, $child, $users, $browseType, $branchGroups, $modulePairs, $useDatatable, true, $canRecordEstimate, $projectClosed);
            echo '</tr>';
            $i ++;

            if(!empty($child->children))
            {
                $this->echoChildren($child, $child->children, $field, $users, $browseType, $branchGroups, $modulePairs, $useDatatable, $canRecordEstimate, $projectClosed);
            }
        }
    }

    public function getExecutionName($executions){
        $executions = $this->dao->select('concat(t1.name,"/",t2.name) name')->from(TABLE_EXECUTION)->alias('t1')
            ->leftJoin(TABLE_EXECUTION)->alias('t2')
            ->on('t1.id = t2.parent')
            ->where('t1.deleted')->eq('0')
            ->andWhere('t2.id')->in($executions)
            ->fetchAll();
        return $executions;
    }
     ...
     * @return array
     */
    public function getPairsByID($executionIdList)
    {
        $executions = $this->dao->select('t1.id, concat_ws("/", t2.name, t1.name) as name')->from(TABLE_EXECUTION)->alias('t1')
            ->leftJoin(TABLE_EXECUTION)->alias('t2')
            ->on('t1.parent = t2.id')
