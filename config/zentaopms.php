...
$config->executionCommonList['zh-cn'][1] = '阶段';
...
define('TABLE_PLANLINKS',     '`' . $config->db->prefix . 'planlinks`');
define('TABLE_RELEASE',       '`' . $config->db->prefix . 'release`');
define('TABLE_RELEASE_BASELINE_LOG',       '`' . $config->db->prefix . 'release_baseline_log`');
define('TABLE_PUSHLOG',       '`' . $config->db->prefix . 'pushlog`');
...
define('TABLE_BUILD_BUG_PHOTO', '`' . $config->db->prefix . 'build_bug_photo`');
...
define('TABLE_CRONCONFIG',   '`' . $config->db->prefix . 'cronconfig`');
...
define('TABLE_FLOW_PUBLISH',           '`' . $config->db->prefix . 'flow_publish`');
define('TABLE_FLOW_SUPPORT',           '`' . $config->db->prefix . 'flow_support`');
define('TABLE_PUBLISHRECORD',          '`' . $config->db->prefix . 'publishrecord`');
define('TABLE_PRODUCTCODEINFO',        '`' . $config->db->prefix . 'productcodeinfo`');
define('TABLE_FLOWWORKLOAD',           '`' . $config->db->prefix . 'flowworkload`');
define('TABLE_FLOWCOSTWORKLOAD',       '`' . $config->db->prefix . 'flowcostworkload`');
define('TABLE_PARTICIPANTSWORKLOAD',   '`' . $config->db->prefix . 'participantsworkload`');
define('TABLE_REVIEWQZ',               '`' . $config->db->prefix . 'reviewqz`');
define('TABLE_REVIEWISSUEQZ',          '`' . $config->db->prefix . 'reviewissueqz`');
define('TABLE_DEPTORDER',              '`' . $config->db->prefix . 'deptorder`');

define('TABLE_REPORT_HISTORY_BUG', '`' . $config->db->prefix . 'report_history_bug`');
define('TABLE_REPORT_TASK_BUG',    '`' . $config->db->prefix . 'report_task_bug`');
