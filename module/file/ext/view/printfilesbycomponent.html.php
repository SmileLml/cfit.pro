...
<?php endif;?>
  ...
  <script>
  /* Delete a file. */
  function deleteFile(fileID, isAjaxDel = false)
  {
      if(!fileID) return;
      if(isAjaxDel == false){
          hiddenwin.location.href = createLink('file', 'delete', 'fileID=' + fileID+'&isAjaxDel='+isAjaxDel);
      }else {
          if(confirm('您确定删除该附件吗？')){
              $.get(createLink('file', 'delete', 'fileID=' + fileID+'&isAjaxDel='+isAjaxDel+'&confirm=yes'), function(data) {
                  $('#fileDel_'+fileID).parent().parent().remove();
              });
          }
      }
    ...
    <ul class="files-list">
      ...
      foreach($files as $file)
      {
          if(common::hasPriv('file', 'download'))
          {
              ...
              echo "<li title='{$file->title}'>" . html::a($this->createLink('file', 'download', "fileID=$file->id") . $sessionString, $fileTitle . " <span class='text-muted'>({$fileSize})</span>", '_blank', "onclick=\"return downloadFile($file->id, '$file->extension', $imageWidth, '$file->title')\"");
              ...
              if($canOperate == 'true')
              {
                  ...
                  if(common::hasPriv('file', 'delete')) echo html::a('###', $lang->delete, '', "class='text-primary' onclick='deleteFile($file->id, $isAjaxDel)' title='$lang->delete' id='fileDel_$file->id'");
                  echo '</span>';
              }
              echo '</li>';
          }
