        ...
        <form id='childrenForm' method='post' target='hiddenwin' action='<?php echo $this->createLink('tree', 'manageChild', "root=$rootID&viewType=$viewType");?>'>
          <table class='table table-form table-auto'>
            <tr>
              ...
              <?php $showPromptViewTypes = ['story', 'case', 'bug']; ?>
                <?php if(in_array($viewType, $showPromptViewTypes)):?>
                <td colspan="2">
                  <div style="color: #000;font-weight: bold;"><?php echo $lang->tree->prompt ?>：</div>
                  <?php if($viewType == 'story'):?>
                  <div><span style="color:red"> <?php echo $lang->tree->promptBrowseStory ?></span></div>
                  <?php elseif($viewType == 'bug'):?>
                  <?php foreach (($this->app->openApp == 'project'?$lang->tree->promptBrowseProjectBug:$lang->tree->promptBrowseBug) as $index => $prompt):?>
                  <div>
                  ...
                  <?php endforeach; ?>
                  <?php elseif($viewType == 'case'):?>
                  <?php foreach ($lang->tree->promptBrowseCase as $index => $prompt):?>
                  <div>
                  <span style="color:red">
                      <?php echo $index + 1 ?>：
                      <?php echo $prompt ?>
...
<!--                  $backLink-->
                <?php if(!isonlybody()) echo html::backButton($lang->goback, '', "class='btn btn-wide'");?>
