...
class customModel extends model
{
    ...
    public function getAllLang()
    {
        ...
        try
        {
            $sql  = $this->dao->select('*')->from(TABLE_LANG)->where('`lang`')->in("$currentLang,all")->orderBy('lang,`order`,id')->get();
    ...
    public function setItem($path, $value = '')
    {
        ...
        if($level == 5) list($lang, $module, $section, $key, $system, $order) = explode('.', $path);
        if($level == 6) list($lang, $module, $section, $key, $system, $order, $extendInfo) = explode('.', $path);
        ...
        $item->value   = $value;
        $item->system  = $system;
        $item->order   = isset($order) ? $order : 0;
        $item->extendInfo   = isset($extendInfo) ? $extendInfo : '';
    ...
    public static function mergeFeatureBar($module, $method)
    {
        ...
        if($app->openApp == 'project' and $module == 'testcase') $queryModule = 'projectTestcase'; 
     ...
     * @return array
     */
    public function getCustomSetList($module, $section, $lang = 'zh-cn'){
        $data = $this->dao->select('`key`,`value`')
            ->from(TABLE_LANG)
