...
$(document).ready(function()
{
   ...
   // var projectname = $('#project'+m).find("option:selected").text();
    $.get(createLink('workreport', 'ajaxGetTaskObject','app='+apps+"&index="+m+"&projectName="+project+"&task="+task+"&flag=all"), function(data)
    {
    ...
    var taskName = $('#objects'+m).find("option:selected").text();
    if(taskName.indexOf('已') >= 0){
      //  $('#beginDate\\[' + m + '\\]').attr('readonly','readonly');
        $('#project'+m).prop('disabled', true).trigger("chosen:updated");
        $('#activity'+m).prop('disabled', true).trigger("chosen:updated");
        $('#apps'+m).prop('disabled', true).trigger("chosen:updated");
        $('#objects'+m).prop('disabled', true).trigger("chosen:updated");
    }
    /*if(project){
        $.ajaxSettings.async = false;
