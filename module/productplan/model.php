...
class productplanModel extends model
{
     ...
     * @return object
     */
    public function getByProjectID($projectID, $setImgSize = false)
    {
        $plan = $this->dao->select('*')->from(TABLE_PRODUCTPLAN)->where('project')->eq($projectID)->fetch();
        if(!$plan) return false;
     ...
     * @return array
     */
    public function getByIDPairs($planIDList)
    {
        return $this->dao->select('id, title')->from(TABLE_PRODUCTPLAN)->where('id')->in($planIDList)->orderBy('begin desc')->fetchPairs('id', 'title');
    }
            ...
            foreach($plans as $plan)
            {
                if($product->type == 'normal')
                {
                    ...
                    foreach($stories as $story) {
                        $storyPairs[$story->id] = $story->estimate; //工时
                    }
                }
                else
                {
                ...
                $plan->stories   = count($storyPairs); //需求数
                $plan->bugs      = isset($bugs[$plan->id]) ? count($bugs[$plan->id]) : 0; //bug数
                $plan->hour      = array_sum($storyPairs); //需求的工时数
                $plan->projectID = $plan->project; //项目id
    ...
    {
        $date = date('Y-m-d');
        $plans = $this->dao->select('id,title,parent,begin,end,product')->from(TABLE_PRODUCTPLAN)
        ...
        $productInfo = $this->dao->select('id,name')->from(TABLE_PRODUCT)
        ->where('id')->in($product)
        ->andWhere('deleted')->eq(0)
        ->fetchPairs();
     ...
     * @return array
     */
    public function getPairsOfProductName($product = 0)
    {
        $date = date('Y-m-d');
        $plans = $this->dao->select('id,title,parent,product,begin,end')->from(TABLE_PRODUCTPLAN)
            ->where('product')->in($product)
            ->andWhere('deleted')->eq(0)
            ->orderBy('begin desc')
            ->fetchAll('id');
        ...
        foreach($plans as $plan)
        {
            ...
            if($plan->parent > 0 and $plan->parent == $parent) $plan->title = $parentTitle . ' /' . $plan->title;
            $planPairs[$plan->id] = $productInfo[$plan->product].'_'.$plan->title;
        }
        return array('' => '') + $planPairs;
    }
     ...
     * @return array
     */
    public function getProductPlanList($productIds = 0, $branch = 0, $expired = '', $skipParent = false)
    {
        ...
        $plans = $this->dao->select('id,title, product, parent,begin,end')->from(TABLE_PRODUCTPLAN)
            ->where('deleted')->eq(0)
            ->beginIF($productIds)->andWhere('product')->in($productIds)->fi()
            ->beginIF($branch)->andWhere("branch")->in("0,$branch")->fi()
            ->beginIF($expired == 'unexpired')->andWhere('end')->ge($date)->fi()
            ->beginIF($skipParent)->andWhere('parent')->ne(-1)->fi()
            ->orderBy('begin desc')
        ...
        $productIds = array_flip(array_flip(array_column($plans, 'product')));
        //商品信息
        ...
        foreach($plans as $plan)
        {
            ...
            if($plan->parent > 0 and $plan->parent == $parent){
                $plan->title = $parentTitle . ' /' . $plan->title;
            }
            $planPairs[$plan->id] = $plan->title . " [{$plan->begin} ~ {$plan->end}]";

            if($plan->begin == '2030-01-01' and $plan->end == '2030-01-01') {
                $planPairs[$plan->id] = $plan->title . ' ' . $this->lang->productplan->future;
            }
            $productId = $plan->product;
            $productName = isset($productList[$productId]) ? $productList[$productId]: '';
            if($productName){
                $planPairs[$plan->id] = $productName . '【'.$planPairs[$plan->id]. '】';
            }
        }
        return array('' => '') + $planPairs;
    }
     ...
     * @return array
     */
    public function  getPairsGroupProduct($productIds, $branch = 0, $expired = '', $skipParent = false){
        $date = date('Y-m-d');
        $plans = $this->dao->select('id,product,title,parent,begin,end')->from(TABLE_PRODUCTPLAN)
        ...
        foreach($plans as $plan)
        {
            ...
            if(!isset($planPairs[$product])){
                $planPairs[$product] = array('' => '');
            }
            if($plan->parent == '-1')
            {
                $parent      = $plan->id;
            ...
            if($plan->parent > 0 and $plan->parent == $parent) $plan->title = $parentTitle . ' /' . $plan->title;
            $planPairs[$product][$plan->id] = $plan->title . " [{$plan->begin} ~ {$plan->end}]";
            if($plan->begin == '2030-01-01' and $plan->end == '2030-01-01') $planPairs[$product][$plan->id] = $plan->title . ' ' . $this->lang->productplan->future;
        }
        return $planPairs;
     ...
     * @param int $branch
     * @param string $expired
     * @param false $skipParent
     * @return array|string[]
     */
    public function getPairsForRelation($product = 0, $branch = 0, $expired = '', $skipParent = false, $getTitle = false)
    {
        $date = date('Y-m-d');
        $plans = $this->dao->select('id,title,parent,begin,end')->from(TABLE_PRODUCTPLAN)
            ->where('product')->in($product)
     ...
     * @return array
     */
    public function getProductPlanNameList($productIds){
        $data = [];
        if(!$productIds){
            return $data;
        }
