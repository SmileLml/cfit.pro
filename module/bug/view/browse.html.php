...
if($taskID)
{
    $tempApplicationID = 'testtask'.$taskID;
    $pageMode = 'testtask-bug';
}
?>
<style>
...
<div id="mainMenu" class="clearfix">
  <div id="sidebarHeader">
    <div class="title">
      ...
      if($moduleID)
      {
          $removeLink = $browseType == 'bymodule' ? inlink('browse', "applicationID=$tempApplicationID&productID=$productID&branch=$branch&browseType=$browseType&param=0&orderBy=$orderBy&recTotal=0&recPerPage={$pager->recPerPage}") : 'javascript:removeCookieByKey("bugModule")';
    ...
    foreach($menus as $menuItem)
    {
        ...
        if($menuItem->name == 'my')
        {
            ...
            foreach($lang->bug->mySelects as $key => $value)
            {
                ...
                echo html::a($this->createLink('bug', 'browse', "applicationID=$tempApplicationID&productid=$productID&branch=$branch&browseType=$key"), $value,"data-app=".$this->app->openApp);
            }
            echo '</ul></li>';
        }
        elseif($menuItem->name == 'QUERY')
        {
            $searchBrowseLink = inlink('browse', "applicationID=$tempApplicationID&productID=$productID&branch=$branch&browseType=bySearch&param=%s");
        ...
        elseif($menuItem->name == 'more')
        {
            if(!empty($lang->bug->moreSelects))
            {
                ...
                foreach($lang->bug->moreSelects as $menuBrowseType => $label)
                {
                    ...
                    echo '<li>' . html::a($this->createLink('bug', 'browse', "applicationID=$tempApplicationID&productID=$productID&branch=$branch&browseType=$menuBrowseType"), "<span class='text'>{$label}</span>", '', "class='btn btn-link $active' data-app='{$this->app->openApp}'") . '</li>';
                }
                echo '</ul></div>';
            }
        }
        ...
        {
            echo html::a($this->createLink('bug', 'browse', "applicationID=$tempApplicationID&productID=$productID&branch=$branch&browseType=$menuBrowseType"), $label, '', "class='btn btn-link $active' data-app='{$this->app->openApp}'");
        }
    }
    ?>
  ...
  <div class="btn-toolbar pull-right">
    <?php if($this->app->openApp != 'project' && is_numeric($productID)) common::printIcon('bug', 'report', "applicationID=$applicationID&productID=$productID&browseType=$browseType&branchID=$branch&moduleID=$moduleID", '', 'button', 'bar-chart muted');?>
    <div class='btn-group'>
      ...
      <ul class='dropdown-menu' id='exportActionMenu'>
        ...
        $link  = common::hasPriv('bug', 'export') ? $this->createLink('bug', 'export', "applicationID=$applicationID&productID=$productID&orderBy=$orderBy&browseType=$browseType") : '#';
    ...
    <div class='btn-group dropdown-hover'>
      <?php
      $params = "applicationID=$applicationID&productID=$productID&branch=$branch&extra=moduleID=$moduleID";
      if($taskID) $params .= ",taskID=$taskID";
      if($this->app->openApp == 'project') $params .= ",projectID={$this->session->project}";
      $link     = $this->createLink('bug', 'create', $params);
      $disabled = '';
      ...
      echo html::a($link, "<i class='icon icon-plus'></i> {$lang->bug->create} </span><span class='caret'>", '', "class='btn btn-primary $disabled' data-app='{$this->app->openApp}'");
      ?>
      <ul class='dropdown-menu'>
        <?php $disabled = common::hasPriv('bug', 'batchCreate') ? '' : "class='disabled'";?>
        <li <?php echo $disabled?>>
        <?php
          $batchLink = $this->createLink('bug', 'batchCreate', "applicationID=$applicationID&productID=$productID&branch=$branch&executionID=0&moduleID=$moduleID");
          echo "<li>" . html::a($batchLink, "<i class='icon icon-plus'></i>" . $lang->bug->batchCreate, null, "data-app='{$this->app->openApp}'") . "</li>";
        ?>
        </li>
      </ul>
    </div>
    <?php
    if($pageMode == 'testtask-bug')
    {
        common::printIcon('testtask', 'linkBug', "taskID=$taskID", '', 'button', 'link');
    }
    ?>
  </div>
</div>
  ...
  <div class="side-col" id="sidebar">
    ...
    <div class="cell">
      ...
      <div class="text-center">
        <?php if(is_numeric($productID)) common::printLink('tree', 'browse', "productID=$productID&view=bug&currentModuleID=0&branch=0&from={$this->lang->navGroup->bug}", $lang->tree->manage, '', "class='btn btn-info btn-wide'");?>
        <hr class="space-sm" />
      </div>
    </div>
  </div>
  <div class="main-col">
    ...
    <div class="table-empty-tip">
      <p>
        ...
        if($this->app->openApp == 'project') $params .= ",projectID={$this->session->project}";
        ?>
        <?php echo html::a($this->createLink('bug', 'create', $params), "<i class='icon icon-plus'></i> " . $lang->bug->create, '', "class='btn btn-info' data-app='{$this->app->openApp}'");?>
        <?php endif;?>
      </p>
    </div>
    <?php else:?>
    ...
    <form class='main-table table-bug' method='post' id='bugForm' <?php if(!$useDatatable) echo "data-ride='table'";?>>
      ...
      <?php
      $vars = "applicationID=$tempApplicationID&productID=$productID&branch=$branch&browseType=$browseType&param=$param&orderBy=%s&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}";
      ...
      //$canBatchClose        = ($canBeChanged and common::hasPriv('bug', 'batchClose'));
      ...
      <table class='table has-sort-head<?php if($useDatatable) echo ' datatable';?>' id='bugList' data-fixed-left-width='<?php echo $widths['leftWidth']?>' data-fixed-right-width='<?php echo $widths['rightWidth']?>'>
        ...
        <tbody>
          ...
          <tr data-id='<?php echo $bug->id?>'>
            <?php foreach($setting as $value) $this->bug->printCell($value, $bug, $users, $builds, $branches, $modulePairs, $projects, $plans, $stories, $tasks, $typeTileList, $products, $useDatatable ? 'datatable' : 'table',$testtasks,$planInfo);?>
          </tr>
          <?php endforeach;?>
        </tbody>
      </table>
      ...
      <div class='table-footer'>
        ...
        <div class="table-actions btn-toolbar">
            ...
            if ($pageMode == 'testtask-bug')
            {
                $actionLink = $this->createLink('testtask', 'unlinkBug', "taskID=$taskID");
                $misc       = $canBatchEdit ? "onclick=\"setFormAction('$actionLink')\"" : "disabled='disabled'";
                echo html::commonButton('取消关联', $misc, 'btn'); 
            }
             ?>
        </div>
        <?php endif;?>
        <div class="table-actions btn-toolbar">
          <div class='btn-group dropup'>
            ...
            $misc       = $canBatchEdit ? "onclick=\"setFormAction('$actionLink')\"" : "disabled='disabled' style='background: rgb(120, 189, 245);'";
          ...
          <?php if($canBatchChangeModule and is_numeric($productID)):?>
          <div class="btn-group dropup">
...
<?php if($pageMode == 'testtask-bug'): ?>
<script>
    $('[data-id="testtask"]').addClass('active');
    $('[data-id="bug"]').removeClass('active');
