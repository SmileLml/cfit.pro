...
<div id="mainContent">
  ...
  <form id='myBugForm' class="main-table table-bug" data-ride="table" method="post" action='<?php echo $this->createLink('bug', 'batchEdit', "productID=0");?>'>
    ...
    $canBatchAction   = ($canBatchEdit or $canBatchConfirm or $canBatchAssignTo);
    ?>
    <table class="table has-sort-head table-fixed" id='bugList'>
      ...
      <thead>
        <tr>
          ...
          <th class='w-50px' title='<?php echo $lang->bug->pri;?>'>      <?php common::printOrderLink('pri',      $orderBy, $vars, $lang->bug->pri);?></th>
          <th>                <?php common::printOrderLink('title',       $orderBy, $vars, $lang->bug->title);?></th>
          <th class='w-150px'><?php common::printOrderLink('applicationID', $orderBy, $vars, $lang->bug->applicationID);?></th>
          <th class='w-100px'> <?php common::printOrderLink('type',        $orderBy, $vars, $lang->bug->type);?></th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class='text-left nobr'><?php if($bug->applicationName) echo html::a($this->createLink('application', 'view', "id=$bug->applicationID"), $bug->applicationName, '', "title={$bug->applicationName}");?></td>
          <td title="<?php echo zget($lang->bug->typeList, $bug->type, '');?>"><?php echo zget($lang->bug->typeList, $bug->type, '');?></td>
          <td title="<?php echo zget($typeTileList, $bug->childType, '');?>"><?php echo zget($typeTileList, $bug->childType, '');?></td>
          <?php if($type != 'openedBy'): ?>
          ...
          <td class='c-actions'>
            ...
            if($canBeChanged)
            {
                ...
                common::printIcon('bug', 'create',     "applicationID=$bug->applicationID&product=$bug->product&branch=$bug->branch&extra=$params", $bug, 'list', 'copy', '', 'iframe', true, "data-width='95%'", '', $bug->project);
            }
            ?>
          </td>
        </tr>
