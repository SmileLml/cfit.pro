...
<div id='mainContent' class='main-content'>
  ...
  <form method='post' enctype='multipart/form-data' id='dataform' data-type='ajax'>
    <table class='table table-bordered' style='word-break:break-all' id='steps'>
      ...
      <?php $resultIdList = array();?>
      <?php foreach($run->case->steps as $key => $step):?>
      <?php
      $resultIdList[] = $step->id;
      ...
      <tr class='step <?php echo $stepClass?>'>
        ...
        <td>
          <table class='w-p100'>
            <tr>
              <td class='no-padding bd-0'><?php echo html::textarea("reals[$step->id]", '', "rows=1 class='form-control' id='reals{$step->id}'");?></td>
...
<?php js::set('resultIdList', json_encode($resultIdList));?>
<script>
$(function()
{
...
setTimeout(function setEditor()
{
    var json = eval('(' + resultIdList + ')');
    for(var i = 0; i < json.length; i ++)
    {
        var editorID    = json[i];
        var realsObject = $('#reals' + editorID);

        $('#reals' + editorID).kindeditor();
    }
}, 500);
</script>
