...
class block extends control
{
    ...
    public function admin($id = 0, $module = 'my')
    {
        ...
        if($module == 'my')
        {
            ...
            if(strpos(",$closedBlock,", ",|workreport,") === false) $modules['workreport'] = $this->lang->block->workreport;
    ...
    public function dashboard($module, $type = '', $projectID = 0)
    {
        ...
        foreach($blocks as $key => $block)
        {
            ...
            if(isset($this->lang->block->modules[$source]->moreLinkList->{$blockID}))
            {
                ...
                elseif($moduleName == 'report' and $method == 'projectsummary')
                {
                    unset($block->moreLink);
                    $block->textMorelink = html::a($this->createLink('report', 'projectsummary', "projectID=$projectID"), '查看更多报表', '', 'style="color: blue;" data-app="project"');
                }
                elseif($moduleName == 'execution' and $method == 'task')
                {
                    unset($block->moreLink);
                    $block->textMorelink = html::a($this->createLink('execution', 'task'), '查看更多任务', '', 'style="color: blue;"');
                }
            }
    ...
    public function welcome()
    {
        ...
        // 获取审批的汇总数据
        $audits = $this->loadModel('my')->getAuditSummary();

        //获取我的评审的汇总数据 20220712 增加
        $myreview = $this->loadModel('my')->reviewAndMeetingCount();
        $this->app->loadClass('pager', $static = true);
        $recTotal = 0;$recPerPage = 20; $pageID = 1;
        $pager = pager::init($recTotal, $recPerPage, $pageID);
        $issuie = $this->loadModel('reviewproblem')->getList($projectID = 0, $reviewID = 0, $browseType = 'myNoclose', $param = 0, $orderBy = "id_desc", $pager);
        $reviewqzCount      = $this->loadModel('my')->getUserReviewqzCount();
        $this->view->myreview   = $myreview + count($issuie) + $reviewqzCount;

        $this->view->audits     = array_sum($audits);
    ...
    public function printBlock($id, $module = 'my')
    {
        ...
        elseif($block->block == 'workreport')
        {
            $html = $this->fetch('block', 'workreport');
        }
    ...
    {
        $this->session->set('bugList', $this->createLink('qa', 'index'), 'qa');
        if(preg_match('/[^a-zA-Z0-9_]/', $this->params->type)) die();
    ...
    {
        $this->session->set('caseList', $this->createLink('qa', 'index'), 'qa');
    ...
    public function printTesttaskBlock()
    {
        ...
        $this->session->set('testtaskList', $this->createLink('qa', 'index'), 'qa');
        if(preg_match('/[^a-zA-Z0-9_]/', $this->params->type)) die();

        $this->view->testtasks = $this->dao->select('*')->from(TABLE_TESTTASK)
            ->where('deleted')->eq('0')
            ->beginIF(!$this->app->user->admin)->andWhere('product')->in($this->app->user->view->products)->fi()
            ->beginIF($this->params->type != 'all')->andWhere('status')->eq($this->params->type)->fi()
            ->orderBy('id desc')
            ->limit((int)$this->params->count)
            ->fetchAll();
    }
    ...
    public function printWaterfallEstimateBlock()
    {
        ...
        /* 查询已消耗工时，删除的执行不计算。*/
        $consumed = $this->dao->select('sum(cast(consumed as decimal(10,2))) as consumed')->from(TABLE_TASK)
    ...
    public function printQaStatisticBlock()
    {
        ...
        $applicationList = $this->dao->select('id,name')->from(TABLE_APPLICATION)
            ->where('deleted')->eq(0)
            ->orderBy('id_desc')
            ->limit($count)
            ->fetchAll('id');

        $applicationIdList = array_keys($applicationList);
        $today         = date(DT_DATE1);
        $yesterday     = date(DT_DATE1, strtotime('yesterday'));

        $testtasks = $this->dao->select('*')->from(TABLE_TESTTASK)
            ->where('applicationID')->in($applicationIdList)
            ->andWhere('project')->ne(0)
            ->andWhere('deleted')->eq(0)
            ->orderBy('id')
            ->fetchAll('applicationID');

        $bugs = $this->dao->select("applicationID, count(id) as total,
        ...
        foreach($applicationList as $applicationID => $application)
        {
            $bug = isset($bugs[$applicationID]) ? $bugs[$applicationID] : '';
            $application->total              = empty($bug) ? 0 : $bug->total;
            $application->assignedToMe       = empty($bug) ? 0 : $bug->assignedToMe;
            $application->unclosed           = empty($bug) ? 0 : $bug->unclosed;
            $application->unresolved         = empty($bug) ? 0 : $bug->unresolved;
            $application->unconfirmed        = empty($bug) ? 0 : $bug->unconfirmed;

            $application->assignedRate    = $application->total ? round($application->assignedToMe  / $application->total * 100, 2) : 0;
            $application->unresolvedRate  = $application->total ? round($application->unresolved    / $application->total * 100, 2) : 0;
            $application->unconfirmedRate = $application->total ? round($application->unconfirmed   / $application->total * 100, 2) : 0;
            $application->unclosedRate    = $application->total ? round($application->unclosed      / $application->total * 100, 2) : 0;
            $application->testtask        = isset($testtasks[$applicationID]) ? $testtasks[$applicationID] : '';
        }

        $this->view->applicationList = $applicationList;
    }
    ...
    public function printRecentProjectBlock()
    {
        ...
        $projects = $this->loadModel('project')->getInfoList('all', 30, 'id_desc', $pager);
        $this->view->projects = $projects;
    }
    ...
    /**
     * Print project works  block.
     *
     * @access public
     * @return void
     */
    public function printProjectworksBlock()
    {
        $this->loadModel('project');
        $this->loadModel('dept');
        ...
        $lastMonthBegin = date('Y-m-d', strtotime('first day of -1 month'));
        $lastMonthEnd   = date('Y-m-d', strtotime('last day of -1 month'));
        $currentMonth   = date('Y-m-01');

        /* 获取用户最近7天的工作量。*/
        $workload7days = $this->project->getEffortByProject($projectID, $last7day, $today);

        foreach($workload7days as $user7d => $work7d){
            $key7d = explode('/',$user7d);
            $list7d[$key7d[0]][$key7d[1]] = $work7d;
        }

        /* 获取用户上个月的工作量。*/
        $workloadLastMonth = $this->project->getEffortByProject($projectID, $lastMonthBegin, $lastMonthEnd);
        foreach($workloadLastMonth as $userMon => $workMon){
            $keyMon = explode('/',$userMon);
            $listMon[$keyMon[0]][$keyMon[1]] = $workMon;
        }

        /* 获取用户本月的工作量。*/
        $workloadCurrentMonth = $this->project->getEffortByProject($projectID, $currentMonth, $today);
        foreach($workloadCurrentMonth as $userCRT => $workCRT){
            $keyCRT = explode('/',$userCRT);
            $listCRT[$keyCRT[0]][$keyCRT[1]] = $workCRT;
        }

        /* 获取用户本项目中所有的工作量。*/
        $workloadTotal = $this->project->getEffortByProject($projectID);
        ...
        // 只要报工过的人都算。
        $members = $this->dao->select('id,account,realname,dept')->from(TABLE_USER)->where('account')->in($accounts)->fetchAll('account');
        $deptMap = $this->dept->getDeptPairs();

        // 将项目团队成员和报工过的人数据合并。
        $members = array_merge($teamMembers, $members);

        $amount = array('count' => '合计','user' => 0,'last7day' => 0,'lastMonth' => 0,'currentMonth' => 0,'total' => 0,'perMonth' => 0);
        ...
        foreach($members as $user)
        {
            if(!empty($list[$user->account])){
                foreach($list[$user->account] as $dept => $workNum){
                    ...
                    $user->last7day     = !empty($list7d[$user->account][$dept])?$list7d[$user->account][$dept]:0;
                    $user->lastMonth    = !empty($listMon[$user->account][$dept])?$listMon[$user->account][$dept]:0;
                    $user->currentMonth = !empty($listCRT[$user->account][$dept])?$listCRT[$user->account][$dept]:0;
                    ...
                    $amount['last7day']     += $user->last7day;
                    $amount['lastMonth']    += $user->lastMonth;
                    $amount['currentMonth'] += $user->currentMonth;
    ...
    /**
     * Print delay task  block
     *
     * @access public
     * @return void
     */
    public function printDelaytaskBlock()
    {
        $this->loadModel('project');
        $this->app->loadLang('task');
        $this->app->loadLang('programplan');
        ...
        $uri = $this->app->getURI(true);
        $this->session->set('taskList', $uri, 'project');

        $tasks   = $this->project->getExecutionTaskByProject($projectID, 'delay');
        $stages  = $this->project->getStagesByProject($projectID, true);
    ...
    /**
     * Print WorkReportblock.
     *
     * @access public
     * @return void
     */
    public function printWorkReportBlock()
    {
