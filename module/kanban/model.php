...
class kanbanModel extends model
{
     ...
     * @return array
     */
    public function getKanbanData($kanbanID, $regionIDList = '',$browseType='all',$queryID=0)
    {
        $kanbanData   = array();
        ...
        $cardGroup   = $this->getCardGroupByKanban($kanbanID,$browseType,$queryID);
     ...
     * @return array
     */
    public function getCardGroupByKanban($kanbanID,$browseType='all',$queryID)
    {
        $kanbanQuery = '';
        if($browseType == 'bysearch') {
            $query = $queryID ? $this->loadModel('search')->getQuery($queryID) : '';
            if($query) {
                $this->session->set('kanbankQuery', $query->sql);
                $this->session->set('kanbanForm', $query->form);
            }
            if($this->session->kanbanQuery == false) $this->session->set('kanbanQuery', ' 1 = 1');
            $kanbanQuery = $this->session->kanbanQuery;
        }
        $isSearchFromId = false;
        if (strpos($kanbanQuery, 'fromID')) {
            $isSearchFromId = true;
            $kanbanQuery = str_replace('`fromID`','`fromType`="demandcollection" and `fromID`',$kanbanQuery);
        }
        /* Get card data.*/
        $cards = $this->dao->select('*')->from(TABLE_KANBANCARD)
            ...
            ->beginIF(!$isSearchFromId)->andWhere('fromID')->eq(0)
//            ->beginIF($browseType=='relatedtome')->andWhere('(FIND_IN_SET("'.$this->app->user->account.'",assignedTo) or createdBy="'.$this->app->user->account.'")')->fi()
            ->beginIF($browseType=='relatedtome')->andWhere('FIND_IN_SET("'.$this->app->user->account.'",assignedTo)')->fi()
            ->beginIF($browseType=='bysearch')->andWhere($kanbanQuery)->fi()
            ->fetchAll('id');
        foreach($this->config->kanban->fromType as $fromType)
        {
            $cards = $this->getImportedCards($kanbanID, $cards, $fromType,0,0,$browseType,$queryID);
        }
        ...
        $actions   = array('recordEstimate','editCard', 'archiveCard', 'deleteCard', 'moveCard', 'setCardColor', 'viewCard', 'sortCard', 'viewExecution', 'viewPlan', 'viewRelease', 'viewBuild', 'viewTicket');
        $cardGroup = array();
        foreach($cellList as $cell)
        {
     ...
     * @return array
     */
    public function getImportedCards($kanbanID, $cards, $fromType, $archived = 0, $regionID = 0,$browseType='all',$queryID=0)
    {
        $kanbanQuery = '';
        if($browseType == 'bysearch') {
            $query = $queryID ? $this->loadModel('search')->getQuery($queryID) : '';
...
//                $executions = $this->getFirstStats(0, 'all', 0, 0, 0, 'id_asc', null);
//                $executionProgress  =[];
//                foreach ($executions as $execution){
//                    $executionProgress[$execution->id] = $execution->hours;
//                }
//            }
            ...
            foreach($objectCards as $objectID => $cardsInfo)
            {
                foreach($cardsInfo as $cardID => $objectCard)
                {
                    ...
                    if($fromType =='execution')
                    {
                        ...
                        $execution = $this->dao->findByID($objectID)->from(TABLE_EXECUTION)->andWhere('deleted')->eq(0)->fetch();
                        $execution_path = explode(',',trim($execution->path,','));
                        $projectId = $execution_path[0];
//                        $objectCard->progress = isset($executionProgress[$objectID]->progress) ? $executionProgress[$objectID]->progress : 0;
                        $executions = $this->getFirstStats($projectId, 'all', 0, 0, 0, 'id_asc', null);
                        $hour = $executions[$objectID]->hours;
//                        $objectCard->progress = $hour->totalEstimate ? round($hour->totalConsumed / $hour->totalEstimate, 2) * 100 : ($hour->totalConsumed ? 100 :0);
                        $objectCard->progress = round($executions[$objectID]->progress,1);
//                        $objectCard->progress = isset($executions[$objectID]->hours->progress) ? $executions[$objectID]->hours->progress : 0;
                    }
    ...
    {
        $oldCard = $this->getCardByID($cardID);
        if ($oldCard->fromType == 'demandcollection'){
            $this->loadModel('demandcollection')->batchKanbanState($oldCard->fromID);
        }
     ...
     * @return array
     */
    public function getFirstStats($projectID = 0, $status = 'undone', $productID = 0, $branch = 0, $itemCounts = 30, $orderBy = 'id_asc', $pager = null, $defaultProjectID=0)
    {
        if (empty($projectID)) $projectID = $defaultProjectID;
        if(empty($productID))
        {
...
//                ->page($pager)
            ...
            //获取项目经理
            $PMs = $this->dao->select('*')->from(TABLE_EXECUTION)
                ->where('type')->in('project')
                ->beginIF($projectID != 0)->andWhere('id')->eq($projectID)->fi()
                ->andWhere('deleted')->eq('0')
                ->fetchAll('id');
            foreach ($executions as $key => $execution){
                if (!empty($PMs[$execution->project] -> PM))
                    $execution->PM = $PMs[$execution->project] -> PM;
            }
        }
        else
        {
...
//        $project = $this->getByID($projectID);
        $project = $this->dao->select('*')->from(TABLE_PROJECT)->where('id')->eq($projectID)->andWhere('`type`')->in('project')->fetch();
        if($project and $project->model == 'waterfall')
        {
            foreach($parents as $id => $execution)
            {
                ...
                foreach($execution->children as $childrenExecution)
                {
                    ...
                    $execution->hours->totalLeft =$execution->hours->totalEstimate - $execution->hours->totalConsumed;
                }
        ...
        reset($parents);
        if (!is_null($pager)){
            $pager->recTotal = count($parents);
            $parents = array_splice($parents,($pager->pageID-1)*$pager->recPerPage ,$pager->recPerPage);
        }
...
//        foreach($children as $child) $orphan = array_merge($child, $orphan);
     ...
     * liuyuhan
     */
    public function computeCard($cardID){
        $card = $this->dao->findByID($cardID)->from(TABLE_KANBANCARD)->andWhere('deleted')->eq(0)->fetch();
//        $task = $this->getByID($taskID);
        ...
        if (empty($card->estimate) && !empty($consumed)){
            $progress= 100.0;
        }else{
            $progress = round(((float)$consumed / (float)$card->estimate )* 100, 0);
        }

        $this->dao->update(TABLE_KANBANCARD)->set('consumed')->eq($consumed)->set('left')->eq($left)->set('progress')->eq($progress)->where('id')->eq($cardID)->exec();
     ...
     * liuyuhan
     */
    public function batchCreateEffort($cardID, $efforts)
    {
        $card = $this->dao->findByID($cardID)->from(TABLE_KANBANCARD)->andWhere('deleted')->eq(0)->fetch();
        $kanban = $this->dao->findByID($card->kanban)->from(TABLE_KANBAN)->andWhere('deleted')->eq(0)->fetch();
        $consumed = $this->dao->select('SUM(consumed) c')->from(TABLE_EFFORT)->where('objectType')->eq('kanban')
            ->andWhere('objectID')->eq($cardID)
            ->andWhere('deleted')->eq(0)
            ->fetch('c');
        $left     = $card->estimate - $consumed;
        //$progress = $task->progress;
        $depts    = $this->dao->select('account, dept')->from(TABLE_USER)->where('deleted')->eq(0)->fetchPairs();
        foreach($efforts as $effort)
        {
            $this->dao->insert(TABLE_EFFORT)->set('objectType')->eq('kanban')
                ->set('objectID')->eq($cardID)
                ->set('product')->eq(',,')
                ->set('project')->eq($kanban->space)
                ->set('execution')->eq($card->kanban)
                ...
                ->set('source')->eq(isset($effort->source) ? 1 : 0)
            ...
            if(!empty($kanban->team))
            {
                if(isset($kanban->team[$this->app->user->account]))
                {
                    $teamUser         = $kanban->team[$this->app->user->account];
                    ...
                    $this->dao->update(TABLE_TEAM)->set('left')->eq($teamUserLeft)->set('consumed')->eq($teamUserConsumed)
                        ->where('root')->eq((int)$cardID)
                        ->andWhere('type')->eq('kanban')
    ...
    public function checkConsumed(){
        ...
        {
            dao::$errors['consumed'] = sprintf($this->lang->kanban->emptyObject, $this->lang->kanban->consumed);
        }else if(!is_numeric($consumed)) {
            dao::$errors['consumed'] = sprintf($this->lang->kanban->noNumeric, $this->lang->kanban->consumed);
        }else if(!preg_match($reg, $consumed))
        {
            dao::$errors['consumed'] = sprintf($this->lang->kanban->consumedError, $this->lang->kanban->consumed);
        }
    }
     ...
     * liuyuhan
     */
    public function deleteCardConsumed($cardID){
        $card = $this->dao->findByID($cardID)->from(TABLE_KANBANCARD)->fetch();
        if ($card->fromType == '') {
            //自定义卡片删除工时
    ...
    /**
     * Project: chengfangjinke
     * Desc:删除一格中所有的卡片和卡片对应的工时
     * liuyuhan
     */
    public function deleteCellCardAndConsumed($cellID){
        $kanbanCell = $this->dao->findByID($cellID)->from(TABLE_KANBANCELL)->fetch();
        $cardIDs = explode(',', $kanbanCell->cards);
        foreach ($cardIDs as $cardID){
            $this->deleteCardConsumed($cardID);
            $this->dao->update(TABLE_KANBANCARD)->set('deleted')->eq(1)->where('id')->eq($cardID)->exec();
        }
    }

    /**
     * Project: chengfangjinke
     * Desc:删除一列中所有的卡片和卡片对应的工时
     * liuyuhan
     */
    public function deleteColumnCardAndConsumed($columnID){

        $this->dao->update(TABLE_KANBANCOLUMN)->set('deleted')->eq(1)->where('id')->eq($columnID)->exec();
        $kanbanCells = $this->dao->select('*')->from(TABLE_KANBANCELL)
            ->where('`column`')->eq($columnID)
            ->fetchAll('id');
        foreach ($kanbanCells as $kanbanCell){
            $this->deleteCellCardAndConsumed($kanbanCell->id);
        }
    }
     ...
     * 根据看板id批量获取卡片
     */
    public function getCardsBykanbanIds($kanbanIds,$fields = "*",$fromType = "",$fromID = 0){
        $cards = $this->dao->select($fields)->from(TABLE_KANBANCARD)
            ->where("kanban")->in($kanbanIds)
            ->andWhere("fromID")->eq($fromID)
            ->andWhere("fromType")->eq($fromType)
            ->fetchall();
        return $cards;
    }
    /**
     * Project: chengfangjinke
     * Desc: 只有空间的负责人和团队才能报工
     * liuyuhan
     */
    public function isCanRecordEstimate($kanbanID)
    {
        $kanban = $this->getByID($kanbanID);
        // 判断用户必须是空间的负责人和团队人才能报工才可以提交工时。
        $space = $this->dao->select('*')->from(TABLE_KANBANSPACE)->where('id')->eq($kanban->space)->fetch();
        $spaceTeam =  explode(',', $space->team);
        $kanbanTeam = explode(',', $kanban->team);
        if (($space->owner !=null && !empty($space->owner))&&(!in_array($space->owner, $spaceTeam))) $spaceTeam = array_push($spaceTeam, $space->owner);
        if (($kanban->owner !=null && !empty($kanban->owner))&&(!in_array($kanban->owner, $kanbanTeam))) $kanbanTeam = array_push($kanbanTeam, $kanban->owner);
        $team = array_merge($spaceTeam, $kanbanTeam);
        array_unique($team);
        $canRecordEstimate = false;
        if(!empty($team) and in_array($this->app->user->account, $team)){
            $canRecordEstimate = true;
        }
        return $canRecordEstimate;
    }
    //度量统计需要查看所有空间看板
    public function getAllSpaceCard($spaceParam=[]){
        $space = $this->dao->select("*")->from(TABLE_KANBANSPACE)->where('deleted')->eq(0)->fetchall('id');
        $spaceId = [];
        ...
        $kanbans = $this->dao->select("*")->from(TABLE_KANBAN)->where('deleted')->eq(0)->andWhere('space')->in($spaceId)->fetchall('id');
        foreach ($space as $k=>$v) {
            foreach ($kanbans as $kanban) {
                if ($kanban->space == $v->id){
                    $space[$k]->kanbans[] = $kanban;
                }
            }
        }
        return $space;
    }
    public function getLanePairsByGroups($groupIDS, $orderBy = '`order`_asc')
    {
        return $this->dao->select('id,name')->from(TABLE_KANBANLANE)
            ->where('deleted')->eq(0)
            ->andWhere('`group`')->in($groupIDS)
            ->orderBy($orderBy)
            ->fetchPairs();
    }
     ...
     * Desc: 同步需求收集
     */
    public function batchDemandcollection($ids,$fromType){
        if ($ids == ''){
            return false;
        }
        $data = fixer::input('post')->get();
        if (!isset($data->spaceId) || (int)$data->spaceId <= 0){
            dao::$errors[] = '请选择空间';
            return dao::$errors;
        }
        if (!isset($data->kanbanId) || (int)$data->kanbanId <= 0){
            dao::$errors[] = '请选择看板';
            return dao::$errors;
        }
        if (!isset($data->laneId) || (int)$data->laneId <= 0){
            dao::$errors[] = '请选择目标泳道';
            return dao::$errors;
        }
        $lane = $this->dao->select('*')->from(TABLE_KANBANLANE)->where('id')->eq($data->laneId)->fetch();
        //获取columnid，自定义默认位置
        $column = $this->getColumnGroupByRegions($lane->region);
        $columnId = "";
        foreach ($column[$lane->group] as $cv) {
            if (!isset($cv->asParent) || $cv->asParent != 1){
                if ($columnId == ''){
                    $columnId = $cv->id;
                }
            }
        ...
        $card = $this->checkCard($ids,$fromType,$data->kanbanId,'*');
        $cardFromID = array_column($card,'fromID');
        $cardID = array_column($card,'id');
        $demand = explode(',',$ids);
        $now         = helper::now();
        $objectCards = array();
        $style = "font-size: 14px;font-weight: 700;line-height: 20px;";
        foreach ($demand as $v){
            $info = $this->dao->findByID($v)->from(TABLE_DEMANDCOLLECTION)->fetch();

            if (in_array($v,$cardFromID)){
                foreach ($card as $ck=>$cv) {
                    $oldCard = $cv;
                    if ($cv->fromID == $v){
                        $newcard = new stdClass();
                        $newcard->lastEditedBy = $this->app->user->account;
                        $newcard->lastEditedDate = $now;
                        $newcard->name = $info->title;
                        $newcard->desc = '<div style="'.$style.'">需求描述：</div><div>'.$info->desc.'</div><div style="'.$style.'">需求分析：</div><div>'.$info->analysis.'</div>';
                        $this->dao->update(TABLE_KANBANCARD)->data($newcard)->where('id')->eq($cv->id)->exec();
                        $actionID = $this->loadModel('action')->create('kanbancard', $cv->id, 'edited');
                        $changes = common::createChanges($oldCard, $newcard);
                ...
                $cardData = new stdClass();
                $cardData->kanban      = $data->kanbanId;
                $cardData->name        = $info->title;
                $cardData->desc        = '<div style="'.$style.'">需求描述：</div><div>'.$info->desc.'</div><div style="'.$style.'">需求分析：</div><div>'.$info->analysis.'</div>';
                $cardData->region      = $lane->region;
                $cardData->group       = $lane->group;
                $cardData->fromID      = $v;
                $cardData->fromType    = $fromType;
                $cardData->createdBy   = $this->app->user->account;
                $cardData->assignedTo   = $this->app->user->account;
                $cardData->createdDate = $now;
                $cardData->pri = $info->priority;
                $this->dao->insert(TABLE_KANBANCARD)->data($cardData)->exec();
                $cardID = $this->dao->lastInsertID();
                $this->loadModel('action')->create('kanbancard', $cardID, 'created');
                $objectCards[$cardID] = $v;
            }
        }
        if($objectCards && !dao::isError())
        {
            $cards = implode(',', array_keys($objectCards));
            $this->addKanbanCell($data->kanbanId, $data->laneId, $columnId, 'common', $cards);
    ...
    /***
     * @param $ids string  fromID
     * @param $type array  fromType
     * @param $kanbanID string
     * @param $fields string 要查询的字段
     * Description：获取该看板下是否存在某个类型的卡片
     */
    public function checkCard($ids,$type,$kanbanID='',$fields="*"){
        $card = $this->dao->select($fields)->from(TABLE_KANBANCARD)
            ->where('fromID')->in($ids)
            ->andWhere('fromType')->eq($type)
            ->beginIF($kanbanID)->andWhere('kanban')->eq($kanbanID)->fi()
            ->andWhere('deleted')->eq('0')
            ->fetchAll();
        return $card;
    ...
    public function geteffortBycard($objectId,$objecttype='kanban'){
        $efforts = $this->dao->select('*')->from(TABLE_EFFORT)
     ...
     * desc:根据看板id获取分类
     */
    public function getTypeBykanban($kanban,$returnType = ''){
        $type = $this->dao->select("*")->from(TABLE_KANBANTYPE)
            ->where('kanban')->eq($kanban)
            ->andWhere('deleted')->eq(0)
            ->fetchall();
        if ($returnType == 'getKeyValue'){
            $data = [];
     ...
     * desc：创建分类
     */
    public function createKanbanType($kanban){
        $key = $_POST['key'];
        if(count(array_unique($key)) != count($key)){
            dao::$errors[] = 'key值不能重复';
        }
        $id = $_POST['id'];
        $status = $_POST['status'];
        ...
        foreach ($key as $k=>$v) {
            if ($id[$k] > 0){
                ...
                $update->name = $name[$k];
                $update->key = $v;
                $update->kanban = $kanban;
                $update->deleted = $status[$k];
                $this->dao->update(TABLE_KANBANTYPE)->data($update)->where('id')->eq($id[$k])->exec();
            }else{
                $data = new stdClass();
                $data->name = $name[$k];
                $data->key = $v;
                ...
                $data->deleted = $status[$k];
                $this->dao->insert(TABLE_KANBANTYPE)->data($data)->exec();
            }
        }
        return true;
    }
    public function buildSearchForm($queryID, $actionURL){
        $this->config->kanban->search['actionURL'] = $actionURL;
        $this->config->kanban->search['queryID']   = $queryID;
        $this->loadModel('search')->setSearchParams($this->config->kanban->search);
    }
}
