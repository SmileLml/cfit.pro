...
class reportModel extends model
{
    ...
    public function getBugs($begin, $end, $product, $execution)
    {
        ...
        foreach($bugs as $bug)
        {
            ...
            // 有效bug 状态是 已解决或已关闭时，解决方案是：已解决、延期处理、不予解决、转为研发需求、已挂起、无法重现 加上 激活但没有解决方案
            if((($bug->status == 'resolved' or $bug->status == 'closed') and ($bug->resolution == 'fixed' or $bug->resolution == 'postponed' or $bug->resolution == 'willnotfix' or $bug->resolution == 'tostory' or $bug->resolution == 'hangup' or $bug->resolution == 'notrepro')) or ($bug->status == 'active' and !$bug->resolution) )
            {
                $bugCreate[$bug->openedBy]['resolved'] = empty($bugCreate[$bug->openedBy]['resolved']) ? 1 : $bugCreate[$bug->openedBy]['resolved'] + 1;
            }
        }
        ...
        foreach($bugCreate as $account => $bug)
        {
            ...
            $bugCreate[$account]['validRate'] = (isset($bug['resolved']) and $bug['resolved'] and isset($bug['all']) and $bug['all']) ? ($bug['resolved'] / $bug['all']) : "0";
        }
