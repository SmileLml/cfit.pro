...
class treeModel extends model
{
    ...
    {
        if($this->isMergeModule(0, $type)) $type .= ',story';
    ...
    {
        /* Add for task #1945. check the module has case or no. */
        if((isset($extra['rootID']) and isset($extra['branch']) and $branch == 'null') or ($type == 'case' and is_numeric($extra)) or ($type == 'bug' and is_numeric($extra)))
        {
            ...
            if(empty($objects))
            {
                ...
                else
                {
                    ...
                    elseif($type == 'bug')
                    {
                        if($this->session->tempApplicationID)
                        {
                            $objects = $this->dao->select('module')->from(TABLE_BUG)
                            ->where("FIND_IN_SET($extra,linkTesttask)")
                            ->fetchAll('module');
                        }
                    }
                }
    ...
    public function createBugLink($type, $module)
    {
        ...
        return html::a(helper::createLink('bug', 'browse', "applicationID=$applicationID&root={$module->root}&branch=&type=byModule&param={$module->id}"), $module->name, '_self', "id='module{$module->id}'");
    }

    /**
     * Create link of a bug in testtask.
     *
     * @param  string  $type
     * @param  object  $module
     * @access public
     * @return string
     */
    public function createTesttaskBugLink($type, $module)
    {
        $applicationID = $this->session->tempApplicationID;
        return html::a(helper::createLink('bug', 'browse', "applicationID=$applicationID&root={$module->root}&branch=&type=byModule&param={$module->id}"), $module->name, '_self', "id='module{$module->id}'");
    }

     /**
     * Create link of a project-bug.
     *
     * @param  string  $type
     * @param  object  $module
     * @access public
     * @return string
     */
    public function createProjectBugLink($type, $module)
    {
        $projectID     = $this->session->projectID;
        $applicationID = $this->session->applicationID;
        $productID     = $this->session->productID;
        
        return html::a(helper::createLink('project', 'bug', "projectID=$projectID&applicationID=$applicationID&productID={$productID}&browseType=byModule&branch=0&build=0&orderBy=status,id_desc&param={$module->id}"), $module->name, '_self', "id='module{$module->id}'");
    }

    /**
     * Create link of a project-bug in testtask.
     *
     * @param  string  $type
     * @param  object  $module
     * @access public
     * @return string
     */
    public function createProjectTestTaskBugLink($type, $module)
    {
    ...
    public function createCaseLink($type, $module)
    {
        ...
        $applicationID = $this->session->applicationID;
        return html::a(helper::createLink($moduleName, $methodName, "applicationID=$applicationID&root={$module->root}&branch=&type=byModule&param={$module->id}"), $module->name, '_self', "id='module{$module->id}' data-app='{$this->app->openApp}'");
    }

    /**
     * Create link of a test case in project.
     *
     * @param  string  $type
     * @param  object  $module
     * @access public
     * @return string
     */
    public function createProjectCaseLink($type, $module)
    {
        $moduleName = 'project';
        $methodName = 'testcase';
        ...
        return html::a(helper::createLink($moduleName, $methodName, "projectID=$projectID&applicationID=$applicationID&productID=$productID&branch=&browseType=byModule&param={$module->id}"), $module->name, '_self', "id='module{$module->id}' data-app='{$this->app->openApp}'");
    }
    ...
    {
        return html::a(helper::createLink('testtask', 'cases', "taskID=$extra&type=byModule&module={$module->id}"), $module->name, '_self', "id='module{$module->id}' data-app='{$this->app->openApp}'");
    }
    ...
    {
        /* 不为用例库或者需求时，返回true，追加story模块。*/
        if($viewType != 'story' and $viewType != 'caselib') return true;
        return false;
