    ...
    public function mergeNotifications($notificationData)
    {
        ...
        foreach($notificationData as $userID => $userMessages)
        {
            ...
            foreach($userMessages as $message)
            {
                ...
                if($message->contentType == 'text' || isset($contentData->contentType) && $contentData->contentType == 'text')
                {
                ...
                if($contentType == 'text')
                {
                    $messageGroups[$contentType][] = $message;
                    continue;
                }
            ...
            foreach($messageGroups as $groupKey => $messages)
            {
                ...
                foreach($messages as $message)
                {
                    ...
                    $innerContent = json_decode($content);
                    $uniqueMessages[$innerContent->id] = $message;
                }
                $messageGroups[$groupKey] = array_values($uniqueMessages);
            }
