  ...
  <div class='side-col' id='sidebar'>
    ...
    <div class='cell'>
      ...
      <div class='text-center'>
        <?php if(is_numeric($productID)) common::printLink('tree', 'browse', "productID=$productID&view=case&currentModuleID=0&branch=0&from={$this->lang->navGroup->testcase}", $lang->tree->manage, '', "class='btn btn-info btn-wide'");?>
        <hr class="space-sm" />
      </div>
    </div>
  </div>
  <div class='main-col'>
    ...
    <div class="table-empty-tip">
      <p>
        ...
        <?php if(common::hasPriv('testcase', 'create') and $browseType != 'bysuite'):?>
        <?php $initModule = isset($moduleID) ? (int)$moduleID : 0;?>
        <?php echo html::a($this->createLink('testcase', 'create', "applicationID=$applicationID&productID=$productID&branch=$branch&moduleID=$initModule"), "<i class='icon icon-plus'></i> " . $lang->testcase->create, '', "class='btn btn-info' data-app='{$this->app->openApp}'");?>
        <?php endif;?>
    ...
    <form class='main-table table-case' id='caseForm' method='post' <?php if(!$useDatatable) echo "data-ride='table'";?>>
      ...
      <?php
      $vars = "applicationID=$applicationID&productID=$productID&branch=$branch&browseType=$browseType&param=$param&orderBy=%s&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}";
      ...
      $canConfirmLibcaseChange    = common::hasPriv('testcase', 'batchConfirmLibcaseChange');
      ...
      <div class='table-footer'>
        <?php if($canBatchAction and !empty($productID)):?>
