...
<div id="mainContent" class="main-content fade">
    <div class="center-block">
        <div class="main-header">
            <h2><?php echo $lang->environmentorder->edit; ?></h2>
        </div>
        <form class="load-indicator main-form form-ajax" method="post" enctype="multipart/form-data" id="dataform">
            <table class="table table-form" id="">
                ...
                <tr >
                    ...
                    <td colspan='3'><?php echo html::input('title', $info->title, "class='form-control'"); ?></td>

                </tr>
                <tr>
                    <th><?php echo $lang->environmentorder->priority; ?></th>
                    <td colspan='3'><?php echo html::select('priority', array('' => '') +$this->lang->environmentorder->priorityList,$info->priority, 'class="form-control chosen"'); ?></td>
                </tr>
                <tr>
                    <th><?php echo $lang->environmentorder->origin; ?></th>

                    <td colspan='3'><?php echo html::select('origin',array('' => '') + $this->lang->environmentorder->originList,$info->origin, 'class="form-control chosen"'); ?></td>
                </tr>
                <tr>
                    <th><?php echo $lang->environmentorder->content; ?></th>
                    <td colspan='3'><?php echo html::input('content', $info->content, "class='form-control'"); ?></td>
                </tr>
                <tr>
                    <th><?php echo $lang->environmentorder->finallytime; ?></th>
                    <td colspan='3'><?php echo html::input('finallytime', $info->finallytime, "class='form-control form-datetime'"); ?></td>
                </tr>
                <tr>
                    <th><?php echo $lang->environmentorder->description; ?></th>
                    <td colspan='3'>
                        <?php echo html::textarea('description', $info->description, "rows='10' class='form-control' ");?>
                    </td>
                    ...
                    <td colspan='3'>
                        ...
                        <table class="table table-form table-bordered">
                            ...
                            <tr v-for="(row,index) in rows" :key="row.id">
                                ...
                                <td >
                                    <input type="file" accept=".doc,.docx,.png,.jpg,.xls,.xlsx" :name="'files['+row.id+']'" class="form-control" @change="()=>{row.material=''}">
                                    <input type="hidden" :name="'material['+row.id+']'" v-model="row.material">
                                    <a  style="color: #0bd986" v-if="row.material" style="margin-left: 10px;width: 80px;height: 32px" :href="row.fileUrl">{{row.file?.title}}</a>
                                </td>
                ...
                <tr>
                    ...
                    <td class='form-actions text-center' colspan='3'>
                        ...
                        ?>
                        <?php if($source=="list"){
                            echo html::linkButton("返回",$this->session->common_back_url,'self','','btn btn-wide');
