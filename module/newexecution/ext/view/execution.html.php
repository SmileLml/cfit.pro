...
<div id='mainMenu' class='clearfix' style="min-width: 1200px">
  ...
  <div class='btn-toolbar pull-right'>
    ...
    <?php common::hasPriv('project', 'refresh') ?common::printLink('project', 'refresh', "projectID=$projectID&flag=2", "<i class='icon-refresh muted'> </i>" . $lang->project->refresh, '', "class='btn btn-link {$disabled}'") : ''?>
    <?php if(!empty($executionStats) and common::hasPriv('newexecution', 'deleteAll')) {

        if (strpos($projectname,'二线管理') !== false) {
            if($this->app->user->account == 'admin'){
                common::printLink('newexecution', 'deleteAll', "projectID=$projectID", "<i class='icon-trash muted'></i> " . '&nbsp;' . $lang->execution->deleteAll, 'hiddenwin', "class='btn btn-link {$disabled}'");
...
<div id='mainContent' class="main-row fade">
  ...
  <form class='main-table' id='executionsForm' method='post' action='<?php echo inLink('batchEdit');?>' data-ride='table' style="overflow: auto;min-width:1200px">
    <table class='table has-sort-head table-fixed' id='executionList'>
      ...
      <thead>
        <tr>
          <th class='w-700px '><?php common::printOrderLink('name', $orderBy, $vars, $lang->project->execName);?></th>
            <!-- 需求35 【计划开始】【计划完成】【计划工期（基准）】【实际开始】【实际完成】【实际工期】【工期偏差】【项目空间状态】隐掉 -->
         ...
         <!-- <th class='w-80px text-center'><?php /*echo $lang->project->progress;*/?></th>-->
        <!--  <th class='w-80px text-center'><?php /*echo $lang->project->status;*/?></th>-->
          <th class='w-80px'><?php echo $lang->project->action;?></th>
        </tr>
      </thead>
      <tbody id='executionTableList'>
        <?php $this->project->printStageNew($executionStats, $taskStats, 1, $projectStatus,$projectType);?>
      </tbody>
    </table>
  </form>
  <?php endif;?>
