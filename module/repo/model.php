...
class repoModel extends model
{
    ...
    public function getGitlabProjects($host, $token)
    {
        ...
        $allResults = array();
        $opts    = array('https'=>array('protocol_version'=>"1.1"));
        $context = stream_context_create($opts);
        for($page = 1; true; $page ++)
        {
            //$results = json_decode(file_get_contents($host . "?private_token=$token&page={$page}&per_page=100", false, $context));
            $url = $host . "?private_token=$token&page={$page}&per_page=100";
            $results = $this->loadModel('requestlog')->http($url, array(), 'GET');
