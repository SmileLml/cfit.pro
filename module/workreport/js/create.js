...
function addTaskItem(obj)
{
   ...
   /* relevantHtml.find('#codePlus0').attr({'id':'codePlus' + rand, 'data-id': rand});
    ...
    relevantHtml.find('.copyStage').attr({'id':'codeCopy' + rand, 'data-id': rand});
   ...
   /* $("#pk_workType0-search").parent().parent().remove();
    ...
    if($("#pk_workType"+ rand +"-search").parent().parent().next().length > 0){
        $("#pk_workType"+ rand +"-search").parent().parent().next().remove();
    }*/
   // $('#workType'+rand + ' option').val('request');
   // $('#pk_workType'+rand+'-search').next().children('.picker-selection-text').html('');
    $('#workType0_chosen').remove();
    if($('#workType'+rand+'_chosen').length > 0){
        $('#workType' + rand +'_chosen').remove();
    }
    $('#workType' + rand).chosen();
    if($('#workType'+ rand +'_chosen').next().attr('id')){
        if($('#workType'+ rand +'_chosen').next().attr('id').length > 0){
            $('#workType'+ rand +'_chosen').next().remove();
        }
    }
...
function delTaskItem(obj)
{
   ...
   /* $("input[id^=endDate]").each((index, item)=>
    ...
    $("select[id^=workType]").each((index, item)=>
    {
        ...
        $(item).parent().find('.chosen-container').attr({'id':'workType' + index+'_chosen'});
    })
    ...
    $("span[id^=codeCopy]").each((index, item)=>
    {
        index++;
        $(item).attr({'data-id': index+1,'id': 'codeCopy' +(index+1)});
    })
}

//同上，复制上一行
function copyTaskItem(obj)
{
    var objIndex = $(obj).attr('data-id');
    ...
    //复制上一个的数据
   if(index > 0){
       var begin = $('#beginDate\\[' + index + '\\]').val();
       var project =  $('#project'+index).val();
       var activity =  $('#activity'+index).val();
       var apps =  $('#apps'+index).val();
       var objects =  $('#objects'+index).val();
       var workType =  $('#workType'+index).val();
       var consumed = $('#consumed\\[' + index + '\\]').val();
      // var type =  $('#pk_workType'+index+'-search').next().children('.picker-selection-text').text();
     //  console.log(type)
       var workContent = $('#workContent\\[' + index + '\\]').val();

       $('#beginDate\\[' + objIndex + '\\]').val(begin);
       $('#project'+objIndex).val(project);
       $('#project'+objIndex).trigger('chosen:updated');
       getActivity(obj);
       $('#activity'+objIndex).val(activity);
       $('#activity'+objIndex).trigger('chosen:updated');
       getApps(obj);
       $('#apps'+objIndex).val(apps);
       $('#apps'+objIndex).trigger('chosen:updated');
       getTasks(obj);
       $('#objects'+objIndex).val(objects);
       $('#objects'+objIndex).trigger('chosen:updated');
       $('#workType'+objIndex).val(workType);
       $('#workType'+objIndex).trigger('chosen:updated');
       //$('#workType'+objIndex + ' option').val(workType);
       //$('#pk_workType'+objIndex+'-search').next().children('.picker-selection-text').html(type);
      // $('#workType' + objIndex).picker();
        $('#consumed\\[' + objIndex + '\\]').val(consumed);
       $('#workContent\\[' + objIndex + '\\]').val(workContent);
