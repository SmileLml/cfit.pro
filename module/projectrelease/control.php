...
class projectrelease extends control
{
    ...
    public function commonAction($projectID = 0, $productID = 0, $branch = 0)
    {
        ...
        $this->products = array('' => '','99999' =>'无') + $this->product->getProductPairsByProject($projectID);
            ...
            if($this->server->ajax)
            {
                ...
                else
                {
                   ...
                   /* $release = $this->release->getById($releaseID);
                    ...
                    //删除发布单也同步安全资产平台
                    if(!empty($release->app)){
                        $release = $this->loadModel('projectrelease')->getByID($releaseID);
                        $this->loadModel('projectrelease')->pushSafeAsset($release);
                    }
                }
                $this->send($response);
            }
     ...
     * @param string $status
     */
    public function deal($releaseID,  $version = 0, $status  = ''){
        if($_POST) {
            $logChanges = $this->projectrelease->deal($releaseID);
            if(dao::isError()) {
                $response['result']  = 'fail';
                $response['message'] = dao::getError();
                $this->send($response);
            ...
            $actionID = $this->loadModel('action')->create($objectType, $releaseID, 'deal', $this->post->comment);
            if($logChanges) {
        ...
        $release = $this->projectrelease->getByID($releaseID);
        //是否允许处理
        $checkRes = $this->projectrelease->checkIsAllowDeal($release, $version, $status, $this->app->user->account);
        if($checkRes['result']){
            $viewSuffix = $this->lang->projectrelease->viewSuffixList[$release->status];
            $this->view->view = 'deal'. $viewSuffix.'.html.php';
        }
        $this->view->title      = $this->lang->projectrelease->deal;
        $this->view->position[] = $this->lang->projectrelease->deal;
        $this->view->release  = $release;
