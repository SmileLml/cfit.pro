...
class myBuild extends build
{
     ...
     * @return void
     */
    public function edit($buildID,  $productID = 0, $projectID = 0)
    {
        if (!empty($_POST)) {
            $desc = $this->post->desc;
            if($_POST['issubmit'] == 'submit'){
                ...
                $changes = $this->build->updateSave($buildID);
            }
            ...
            if ($changes or $files or $this->post->desc) {
                $fileAction = '';
                if (!empty($files)) $fileAction = $this->lang->addFiles . join(',', $files) . "\n";
                $actionID = $this->loadModel('action')->create('build', $buildID, 'edited',$desc);
