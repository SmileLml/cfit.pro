...
.expand-demand, .more a {color: #0c60e1; font-size: 11px;}
.expand .icon, .more a .icon {font-size: 14px; transform: rotate( 90deg); display: inline-block;}
.expand-demand .icon, .more a .icon {font-size: 14px; transform: rotate( 90deg); display: inline-block;}
.pullback.icon {color: #0c60e1; font-size: 14px; transform: rotate( -90deg); display: inline-block; float: right; margin-top: 8px;}
.pullback-demand.icon {color: #0c60e1; font-size: 14px; transform: rotate( -90deg); display: inline-block; float: right; margin-top: 8px;}
...
<div id="mainMenu" class="clearfix">
  <div class="btn-toolbar pull-left">
    <?php echo html::a($this->createLink('requirement', 'matrix', "status=noclosed"), '<span class="text">' . $lang->requirement->noClosed . '</span>', '', "class='btn btn-link noclosedTab' data-app='platform'");?>
    <?php echo html::a($this->createLink('requirement', 'matrix', "status=closed"), '<span class="text">' . $lang->requirement->closed . '</span>', '', "class='btn btn-link closedTab' data-app='platform'");?>
    <div class='input-group input-group-sm' style='width: 400px' id='conditions'>
      <span class='input-group-addon'><?php echo $lang->requirement->beginAndEnd;?></span>
      ...
      <span class='input-group-addon fix-border'><?php echo $lang->requirement->to;?></span>
      ...
      <table class='table table-bordered' id="trackList">
        <thead>
          <tr class='text-center'>
            ...
            <th><?php echo $lang->demand->common;?></th>
            <th><?php echo $lang->application->common;?></th>
            <th class='w-120px'><?php echo $lang->product->line;?></th>
            <th><?php echo $lang->requirement->productCommon;?></th>
            <th><?php echo $lang->product->code;?></th>
            <th><?php echo $lang->requirement->projectCommon;?></th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <tr class="hidden r-<?php echo $opinion->id;?>">
              <td id="r-<?php echo $requirement->id;?>" oid="<?php echo $opinion->id;?>" count="<?php echo count($requirement->children);?>" class="text-ellipsis" title="<?php echo $requirement->name;?>">
                ...
                <i class="pullback-demand icon icon-angle-double-right hidden"></i>
              </td>
              <td class="expand-demand hd-<?php echo $requirement->id;?>" oid="<?php echo $opinion->id;?>" r-id="<?php echo $requirement->id;?>" >
                <?php if(!empty($requirement->children)):?>
                  <?php echo sprintf($this->lang->requirement->demandCount, count($requirement->children));?>
                  <a class="task-toggle collapsed"><i class="icon icon-angle-double-right"></i></a>
                <?php endif;?>
              </td>
              ...
              <?php foreach($requirement->children as $demand): ?>
                <tr class="hidden d-<?php echo $requirement->id;?>">
                  <td title="<?php echo $requirement->title;?>"><?php echo html::a($this->createLink('demand', 'view', "opinionID=$demand->id"), $demand->title, '');?></td>
                  <td>
                  <?php
                  $appList = explode(',', str_replace(' ', '', $demand->app));
                  foreach($appList as $appID) 
                  {   
                      if($appID) echo '<div class="text-ellipsis" title="' . zget($apps, $appID, '') . '">' . html::a($this->createLink('application', 'view', "appID=$appID"), zget($apps, $appID)). '</div>';
                  }
                  ...
                  <td>
                  <?php 
                    $linkedProducts = explode(',', str_replace(' ', '', $demand->product));
                    $i = 0;
                    foreach($linkedProducts as $productID)
                    {
                        if($i == 2) echo '<div class="more">More<a class="task-toggle collapsed"><i class="icon icon-angle-double-right"></i></a></div>';
                  ...
                  <td>
                  <?php
                  $codeList = zget($codes, $demand->project, array());
                  $i = 0;
                  foreach($codeList as $code)
                  {
                      if($i == 2) echo '<div class="more">More<a class="task-toggle collapsed"><i class="icon icon-angle-double-right"></i></a></div>';
                  ...
                  <td>
                  <?php
                  $projectProgress = isset($progress[$demand->project]) ? $progress[$demand->project] : 0;
                  echo "<div class='c-progress'>";
                  echo "<div class='progress-pie' data-doughnut-size='90' data-color='#00da88' data-value='{$projectProgress}' data-width='24' data-height='24' data-back-color='#e8edf3'>";
                  echo "<div>{$projectProgress}%</div>";
                  echo "</div>";
                  echo "</div>";
                  echo $demand->project ? html::a($this->createLink('projectplan', 'view', "planID=$demand->project"), $name) : '';
                  ?>
                  </td>
                </tr>
              <?php endforeach;?>
...
$('.pullback').click(function() {
    ...
    console.log($('.r-' + id + ' .expand-demand'))
    $('.r-' + id + ' .pullback-demand').trigger('click');
    $('.h-' + id).removeClass('hidden');
    $('.r-' + id).addClass('hidden');
    $(this).parent('td').attr('rowspan', 1);
})

$('.expand-demand').click(function(){
  ...
  $('#r-' + id + ' .pullback-demand').removeClass('hidden');//一级关闭二级按钮展示
  $('.hd-' + id).addClass('hidden');//二级第一排隐藏
})
$('.pullback-demand').click(function(){
  $(this).addClass('hidden');
  var id = $(this).parent('td').attr('id').split('-')[1];
  ...
  $('#o-' + oid).attr('rowspan',Number(rowspan) - Number(count));//扩展一级别
  $('#r-' + id + ' .pullback-demand').addClass('hidden');//一级关闭二级按钮展示
