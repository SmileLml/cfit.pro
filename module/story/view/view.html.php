    ...
    <div class='main-actions'>
      <div class="btn-toolbar">
        ...
        if($story->parent >= 0 and $story->type != 'requirement' and (common::hasPriv('testcase', 'create', $story) or common::hasPriv('testcase', 'batchCreate', $story)))
        {
            ...
            if(isonlybody()) $misc = '';
            $link = $this->createLink('testcase', 'create', "applicationID=$product->app&productID=$story->product&branch=$story->branch&moduleID=0&from=&param=0&storyID=$story->id", '', true);
            if(common::hasPriv('testcase', 'create', $story)) echo "<li>" . html::a($link, $lang->testcase->create, '', $misc) . "</li>";

            $link = $this->createLink('testcase', 'batchCreate', "applicationID=$product->app&productID=$story->product&branch=$story->branch&moduleID=0&storyID=$story->id", '', true);
