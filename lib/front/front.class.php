...
class html extends baseHTML
{
     ...
     * @return string
     */
    static public function select($name = '', $options = array(), $selectedItems = "", $attrib = "", $append = false, $maxLength = 0)
    {
        $options = (array)($options);
        ...
        foreach($options as $key => $value)
        {
            ...
            $newValue = $maxLength > 0 && $maxLength < mb_strlen($value) ? mb_substr($value, 0, $maxLength) . '...' : $value;
            $string  .= "<option value='$key'$selected title='{$value}' data-keys='{$optionPinyin}'>$newValue</option>\n";
        }
