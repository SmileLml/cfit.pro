...
function renderCount($count, count, column)
{
    ...
    {
        $count.parents('.title').parent('.kanban-header-col').css('background-color', 'transparent');
        var objParent = $count.parents('.title').parent('.kanban-header-col').parent() ;
        var _length = objParent.children().length;
        var childreWitth = 100 / _length;
        for (var i = 0;i < _length;i++){
            var _left = objParent.children().eq(i).css("left");
            if (i > 0 && _left == "0%"){
                var _left = objParent.children().eq(i).css("left",childreWitth+"%");
            }
        }
        // console.log($count.parents('.title').parent('.kanban-header-col').parent().children("div").length);
...
function renderKanbanItem(item, $item)
{
    ...
    else if(item.fromType == 'demandcollection')
    {
        renderDemandcollectionItem(item, $item);
    }
    else
    {
        if(!$title.length)
        ...
        $title.html('<span class="label label-id">'+item.id+'</span>  '+item.name).attr('title', item.name);
    ...
    if(kanban.performable == 1 && (item.fromType == '' || item.fromType == 'execution' || item.fromType == 'demandcollection'))
    {
    ...
    if (item.fromType == 'execution'){
        $title = $item.children('.header').children('.executionName').children('.title');
    }else if (item.fromType == 'demandcollection'){
        $title = $item.children('.header').children('.demandName').children('.title');
    }else{
...
function renderExecutionItem(item, $item)
{
    ...
    if(!$title.length)
    {
        ...
        if(privs.includes('viewExecution') && item.deleted == '0') $title = $('<a class="title"><span class="label label-id">'+item.id+'</span><i class="icon icon-' + icon + '"></i>' + name + '</a>').appendTo($titleBox).attr({'href':'javascript:void(0);','onclick': "locateView('execution'," + item.fromID + ")"});
...
function renderDemandcollectionItem(item, $item) {
    /* Output header information. */
    var privs = item.actions;
    if(privs.includes('sortCard')) $item.parent().addClass('sort');
    ...
    var $titleBox = $header.children('.demandName');
    if(!$titleBox.length) $titleBox = $(
        [
    ...
    if(!$title.length)
    {
        ...
        var belongStr = '';
        if (item.belongModel != ''){
            belongStr = '【'+belongModel[item.belongModel]+'】';
        }
        var viewMethod = item.execType == 'kanban' ? 'kanban' : 'view';
        // if(privs.includes('viewExecution') && item.deleted == '0') $title = $('<a class="title"><i class="icon icon-' + icon + '"></i>' + name + '</a>').appendTo($titleBox).attr('href', createLink('execution', viewMethod, 'executionID=' + item.fromID));
        if(privs.includes('viewExecution') && item.deleted == '0') $title = $('<a class="title iframe" data-toggle="modal" data-width="80%"><span class="label label-id">'+item.id+'</span>&nbsp;&nbsp;' + name + '【'+item.fromID+'】'+belongStr+'</a>').appendTo($titleBox).attr('href', createLink('kanban', 'viewCard', 'cardID=' + item.id, '', true));
        // if(privs.includes('viewCard')) $title = $('<a class="title iframe" data-toggle="modal" data-width="80%"></a>').appendTo($item).attr('href', createLink('kanban', 'viewCard', 'cardID=' + item.id, '', true));
    ...
    $title.attr('title', name+'【'+item.fromID+'】');
    ...
    // $info.children('.fromID').empty().append('<span>【'+item.fromID+'】</span>');
    var $statusBox = $info.children('.execStatus');
    if(!$statusBox.length)
    {
        if(item.deleted == '0')
...
function renderReleaseItem(item, $item)
{
    ...
    if(!$title.length)
    {
        if(privs.includes('viewRelease') && item.deleted == '0') $title = $('<a class="releaseTitle title"><span class="label label-id">'+item.id+'</span><i class="icon icon-publish"></i>' + name + '</a>').appendTo($item).attr('href', createLink('release', 'view', 'releaseID=' + item.fromID));
...
function renderProductplanItem(item, $item)
{
    ...
    if(!$title.length)
    {
        if(privs.includes('viewPlan') && item.deleted == '0') $title = $('<a class="productplanTitle title"><span class="label label-id">'+item.id+'</span><i class="icon icon-delay"></i>' + name + '</a>').appendTo($item).attr('href', createLink('productplan', 'view', 'productplanID=' + item.fromID));
...
function renderBuildItem(item, $item)
{
    ...
    if(!$title.length)
    {
        if(privs.includes('viewBuild') && item.deleted == '0') $title = $('<a class="buildTitle title" data-app="project"><span class="label label-id">'+item.id+'</span><i class="icon icon-ver"></i>' + name + '</a>').appendTo($item).attr('href', createLink('build', 'view', 'buildID=' + item.fromID));
...
function renderTicketItem(item, $item)
{
    ...
    {
        var icon = 'ticket';
        if(privs.includes('viewTicket') && item.deleted == '0') $title = $('<a class="title"><span class="label label-id">'+item.id+'</span><i class="icon icon-' + icon + '"></i>' + name + '</a>').appendTo($titleBox).attr('href', createLink('ticket', 'view', 'ticketID=' + item.fromID));
...
function moveCard(cardID, fromColID, toColID, fromLaneID, toLaneID, kanbanID, regionID)
{
    ...
    var url = createLink('kanban', 'moveCard', 'cardID=' + cardID + '&fromColID='+ fromColID + '&toColID=' + toColID + '&fromLaneID='+ fromLaneID + '&toLaneID=' + toLaneID + '&kanbanID=' + kanbanID + '&browseType='+browseType+'&param='+param);
    return $.ajax(
    {
        method:   'post',
        dataType: 'json',
...
function setCardColor(cardID, color, kanbanID, regionID)
{
    if(!cardID) return false;
    color = color.replace('#', '');
    var url = createLink('kanban', 'setCardColor', 'cardID=' + cardID + '&color=' + color + '&kanbanID=' + kanbanID + '&browseType='+browseType+'&param='+param);
    return $.ajax(
    {
        method:   'post',
        dataType: 'json',
...
{
    if(!cardID) return false;
    var url = createLink('kanban', 'finishCard', 'cardID=' + cardID + '&kanbanID=' + kanbanID + '&browseType='+browseType+'&param='+param);
    return $.ajax(
    {
        method:   'post',
        dataType: 'json',
...
function createCardMenu(options)
{
    ...
    var items = [];
    if(privs.includes('recordEstimate') && (card.fromType == '' || card.fromType == 'demandcollection') && isCanRecordEstimate) items.push({label: kanbanLang.recordEstimate, icon: 'time', url: createLink('kanban', 'recordEstimate', 'cardID=' + card.id, '', 'true'), className: 'iframe', attrs: {'data-toggle': 'modal', 'data-width': '80%'}});
    if(privs.includes('editCard') && (card.fromType == '' || card.fromType == 'demandcollection')) items.push({label: kanbanLang.editCard, icon: 'edit', url: createLink('kanban', 'editCard', 'cardID=' + card.id, '', 'true'), className: 'iframe', attrs: {'data-toggle': 'modal', 'data-width': '80%'}});
    if(privs.includes('deleteCard')) items.push({label: card.fromType == '' ? kanbanLang.deleteCard : kanbanLang.removeCard, icon: card.fromType == '' ? 'trash' : 'unlink', url: createLink('kanban', 'deleteCard', 'cardID=' + card.id + '&confirm=no' + '&browseType='+browseType+'&param='+param), attrs: {'target': 'hiddenwin'}});
    if(kanban.performable == 1 && (card.fromType == '' || card.fromType == 'demandcollection'))
    {
        if(card.status == 'done')
        {
    ...
    if(privs.includes('archiveCard') && kanban.archived == '1') items.push({label: kanbanLang.archiveCard, icon: 'card-archive', url: createLink('kanban', 'archiveCard', 'cardID=' + card.id + '&confirm=no' + '&browseType='+browseType+'&param='+param), attrs: {'target': 'hiddenwin'}, onClick:function(){$('#archivedCards').replaceWith("<div id='archivedCards'></div>");}});
...
function createColumnCreateMenu(options)
{
    ...
    if(kanban.object && kanban.object != 'cards') items.push({className: 'parentDivider'});
    if(kanban.object && kanban.object != 'cards') items.push({label: kanbanLang.importAB, className: 'import'});
    return items;
}
...
$(function()
{
    ...
    $('#kanban').on('click', '[data-contextmenu]', function(event)
    {
        ...
        var importSubmenu       = '<ul class="dropdown-menu">' + importExecutionLink + '</ul>';
