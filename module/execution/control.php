...
class execution extends control
{
    ...
    public function __construct($moduleName = '', $methodName = '')
    {
        ...
        if(!in_array($this->methodName, array('computeburn', 'ajaxgetdropmenu','view')))
        {
            $this->executions = $this->execution->getPairs($this->session->project, 'all', 'nocode');
            if(!$this->executions and $this->methodName != 'index' and $this->methodName != 'create') $this->locate($this->createLink('project', 'execution', array('status' => 'all', 'project' => $this->session->project)));
        }
        $this->objectType = $this->config->systemMode == 'classic' ? 'project' : 'execution';

        $projectID = $this->session->project;
        if(!empty($projectID)) $this->project->setMenu($projectID);
    }
    ...
    public function task($executionID = 0, $status = 'unclosed', $param = 0, $orderBy = '', $recTotal = 0, $recPerPage = 100, $pageID = 1)
    {
        ...
        $this->app->session->set('taskList', $uri, $this->app->openApp);
        ...
        $this->view->canRecordEstimate = $this->project->isCanRecordEstimate($execution->project);
        $this->session->set('common_back_url', $this->app->getURI(true),'backlog');
        $this->view->projectName = $this->loadModel('project')->getByID($execution->project);
        $this->display();
    }
    ...
    public function importBug($executionID = 0, $browseType = 'all', $param = 0, $recTotal = 0, $recPerPage = 30, $pageID = 1)
    {
        ...
        $execution = $this->execution->getByID($executionID);
        $users  = $this->loadModel('user')->getTeamMemberPairs($execution->project, 'project', 'nodeleted');
    ...
    public function manageProducts($executionID, $from = '')
    {
        ...
        if(!empty($_POST))
        {
            ...
            if($diffProducts) $this->loadModel('action')->create($this->objectType, $executionID, 'Managed', $comment, $comment);
    ...
    public function dynamic($executionID = 0, $type = 'today', $param = '', $recTotal = 0, $date = '', $direction = 'next')
    {
        ...
        $this->session->set('common_back_url',    $uri, 'backlog');
    ...
    public function ajaxGetDropMenu($executionID, $module, $method, $extra)
    {
        ...
        foreach($executions as $execution)
        {
            ...
            if(empty($executions[$execution->parent]))
            {
                $orderedExecutions[$execution->id] = $execution;
            }
            else
            {
                // 汇总查询各个阶段下的任务总数（只算可执行的任务）。
                $execution->tasks = $this->execution->getExecutionTaskCount($execution->id);
                $children[$execution->parent][$execution->id] = $execution;
            }
        }

        // 考虑一级阶段下面也挂任务，汇总的时候把任务总数为一级阶段下的任务总数 + 所有子阶段的任务总数。
        foreach($orderedExecutions as $id => $execution)
        {
            if(isset($children[$id]))
            {
                foreach($children[$id] as $childrenExecution)
                {
                    $execution->tasks += $childrenExecution->tasks;
                }
            }
        }
        ...
        $this->display('execution', 'ajaxgetdropmenunew');
    }
