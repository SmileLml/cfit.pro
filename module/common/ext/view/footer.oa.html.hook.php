...
?>
<?php if(empty($this->app->user->signed)) $this->attend->signIn($this->app->user->account);?>
<script>
<?php if(!empty($this->app->user->signed) and $this->app->user->mustSignOut == 'no'):?>
window.onunload = function()
{
    $.get(createLink('attend', 'signOut'));
}
<?php endif;?>
<?php if($this->app->user->mustSignOut == 'yes'):?>
...
window.onload = function (ev) {
    //工作流配制出的模块名称
    var $flowModuleName = "<?php echo $flowModuleName?>";
    var workFlowArray = ['handbook','publish','permission'];
    if ($.inArray($flowModuleName,workFlowArray) >= 0){
        $(".deleter").each(function () {
            var _href = $(this).attr('href');
            if (_href != ''){
                $(this).attr('data-link',_href);
                $(this).attr('onclick',"newDelete('"+_href+"',$(this))");
