...
class im extends control
{
    ...
    public function sysGetServerInfo($account, $password, $apiVersion = '', $userID = 0, $version = '', $device = 'desktop')
    {
        ...
        if(isset($user->token) && $device == 'zentaoweb') $outputData->authToken = $user->token;
    ...
    public function chatSetCommitters($gid = '', $committers = '', $userID = 0)
    {
        ...
        if($committers == '$ADMINS')
        {
            if($chat->type == 'system')
            {
                ...
                ->fetch('admins');
                $admins = $this->im->userGetList('', array_filter(explode(',', $admins)));
                $chat->committers = implode(',', array_keys($admins));
            }
