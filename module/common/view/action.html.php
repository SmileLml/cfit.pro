...
<style>.histories-list > li{word-break: break-word; word-wrap: break-word;}</style>
  ...
  <?php endif;?>
    <ol class='histories-list'>
      ...
      <?php $canEditComment = ((!isset($canBeChanged) or !empty($canBeChanged)) and end($actions) == $action and trim($action->comment) != '' and ( in_array($this->methodName, array('view', 'reviewview', 'meetingview'))) and $action->actor == $this->app->user->account and common::hasPriv('action', 'editComment'));?>
      <li value='<?php echo $i ++;?>'>
        <?php
        $action->actor = zget($users, $action->actor);
        if($action->action == 'assigned' || $action->action == 'assignedtoframeworked') $action->extra = zget($users, $action->extra);
        ...
        <?php if(isset($defaultComment)):?>
        <div class='history-changes article-content comment'>
