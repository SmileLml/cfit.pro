<?php
/**
 * The diff view file of doclib module of ZenTaoPMS.
 *
...
<div id='mainMenu' class='clearfix'>
  <div class='btn-toolbar pull-left'>
    <?php
    $backURI = $this->session->doclibView ? $this->session->doclibView : $this->session->doclibList;
    if($backURI)
    {
    ...
    <div class="page-title">
      <?php
      echo html::a($this->projectdoc->createLink('browse', "libID=$libID"), $lib->name, '', "data-app='{$app->openApp}'");
      $paths= explode('/', $entry);
      ...
      {
          $postPath .= $pathName . '/';
          echo '/' . ' ' . html::a($this->projectdoc->createLink('browse', "libID=$libID&branchID=$branchID&path=" . $this->projectdoc->encodePath($postPath)), trim($pathName, '/'), '', "data-app='{$app->openApp}'");
      }
      echo '/' . ' ' . $fileName;
      if($lib->SCM == 'Git')
      {
...
<?php endif;?>
<div class="doclib panel">
  <div class='panel-heading'>
    <form method='post'>
      <div class='btn-group pull-right'>
        <?php echo html::commonButton($lang->projectdoc->viewDiffList['inline'], "id='inline'", $arrange == 'inline' ? 'active btn btn-sm' : 'btn btn-sm')?>
        <?php echo html::commonButton($lang->projectdoc->viewDiffList['appose'], "id='appose'", $arrange == 'appose' ? 'active btn btn-sm' : 'btn btn-sm')?>
      </div>
      <div class='btn-toolbar'>
        <div class='btn-group'>
          <?php if(common::hasPriv('doclib', 'download')) echo html::a($this->projectdoc->createLink('download', "libID=$libID&path=" . $this->projectdoc->encodePath($entry) . "&fromRevison=$oldRevision&toRevision=$newRevision&type=path"), $lang->projectdoc->downloadDiff, 'hiddenwin', "class='btn btn-sm btn-download'");?>
          <div class='btn-group'>
            <?php echo html::commonButton(zget($lang->projectdoc->encodingList, $encoding, $lang->projectdoc->encoding) . "<span class='caret'></span>", "data-toggle='dropdown'", 'btn dropdown-toggle btn-sm')?>
  ...
  <div class='doclibCode'>
    <table class='table diff' id='diff'>
      ...
      <tr data-line='<?php echo $line->newlc ?>'>
        ...
        $line->line = $lib->SCM == 'Subversion' ? htmlspecialchars($line->line) : $line->line;
      ...
      <tr data-line='<?php echo $line->newlc ?>'>
        ...
        $content->old[$oldlc] = $lib->SCM == 'Subversion' ? htmlspecialchars($content->old[$oldlc]) : $content->old[$oldlc];
        ...
        $content->new[$newlc] = $lib->SCM == 'Subversion' ? htmlspecialchars($content->new[$newlc]) : $content->new[$newlc];
