...
class tree extends control
{
    ...
    public function browse($rootID, $viewType, $currentModuleID = 0, $branch = 0, $from = '')
    {
        ...
        else if($this->app->openApp == 'qa' and $viewType != 'caselib')
        {
            ...
            $product = $this->dao->select('*')->from(TABLE_PRODUCT)->where('id')->eq($rootID)->fetch();
            $applicationID   = $this->rebirth->saveState($applicationList, $product->app, $product->id);
            $this->rebirth->setMenu($applicationID, $product->id, 0, $viewType);
        }
        ...
        elseif($viewType == 'bug')
        {
            ...
            {
                $this->lang->waterfall->menu->qa['subMenu']->bug['subModule'] = 'tree';
            }
            else
            {
        ...
        elseif($viewType == 'case')
        {
            if($this->app->openApp == 'project')
            {
                $this->lang->waterfall->menu->qa['subMenu']->testcase['subModule'] = 'tree';
            }
            else
            {
                $this->app->loadConfig('qa');
    ...
    public function browseTask($rootID, $productID = 0, $currentModuleID = 0)
    {
        ...
        unset($this->lang->waterfall->menu->story['subModule']);
        $this->lang->waterfall->menu->task['subModule'] .= ',tree';
