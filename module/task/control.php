...
class task extends control
{
    ...
    public function __construct($module = '', $method = '')
    {
        ...
        if($this->app->openApp == 'project') $this->project->setMenu($this->session->project);
    }
    ...
    public function batchCreate($executionID = 0, $storyID = 0, $moduleID = 0, $taskID = 0, $iframe = 0)
    {
        ...
        // 判断该任务是否有工时消耗(根据消耗表判断)，如果有则需要创建同名子任务。
        $taskConsumed = 0;
        //if($taskID) $taskConsumed = $this->dao->select('consumed')->from(TABLE_TASK)->where('id')->eq($taskID)->fetch('consumed');
        if($taskID)
        {
        ...
        if($taskConsumed)
        {
            $taskInfo = $this->task->getByID($taskID);
            $oldTaskID = $this->dao->select('id')->from(TABLE_TASK)->where('parent')->eq($taskID)->andWhere('name')->eq($taskInfo->name)->fetch('id');
            if($oldTaskID) $taskConsumed = 0;
        }

        if(!empty($_POST))
        {
            $mails = $this->task->batchCreate($executionID,$execution->dataVersion);
    ...
    public function commonAction($taskID)
    {
        ...
        $this->view->members   = $this->loadmodel('user')->getteammemberpairs($this->view->task->project, 'project', 'nodeleted');
    ...
    public function batchEdit($executionID = 0)
    {
        ...
        $executionTeams = $this->loadModel('user')->getTeamMemberPairs($this->session->project, 'project', 'nodeleted');
    ...
    public function report($executionID, $browseType = 'all', $chartType = 'default')
    {
        ...
        $this->view->title         = $this->lang->task->report->common;
