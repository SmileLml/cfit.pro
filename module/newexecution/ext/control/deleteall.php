...
class myNewExecution extends newexecution
{
    ...
    public function deleteAll($projectID, $confirm = 'no')
    {
        if($confirm == 'no')
        {
            echo js::confirm($this->lang->execution->confirmDeleteAll, $this->createLink('newexecution', 'deleteAll', "projectID=$projectID&confirm=yes"));
            exit;
        }
        else
        {
            ...
            {
                $this->action->create('newexecution', $execution, 'deleted', '', ACTIONMODEL::CAN_UNDELETED);
                $this->newexecution->updateUserView($execution);
            }
            ...
            die(js::locate($this->createLink('newexecution', 'execution', "browseType=all&projectID=$projectID"), 'parent'));
        }
    }
}
