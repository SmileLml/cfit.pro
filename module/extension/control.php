...
class extension extends control
{
    ...
    public function upload()
    {
        ...
        if($_FILES)
        {
            ...
            /* 判断是否在站点基础目录下。 */
            if(strpos($dest, $this->app->basePath) === false) die(js::alert($this->lang->noBasicDirectory));
            ...
            if(isset($info->code) and $info->code != $extension)
            {
                ...
                /* 判断是否在站点基础目录下。 */
                $oldFileName = $this->app->getTmpRoot() . "/extension/$fileName";
                if(strpos($oldFileName, $this->app->basePath) === false) die(js::alert($this->lang->noBasicDirectory));

                rename($oldFileName, $this->app->getTmpRoot() . "/extension/{$info->code}.zip");
