  ...
  <td style='padding: 10px; border: none;'>
    <table cellpadding='0' cellspacing='0' style='width: 100%; border: 1px solid #e5e5e5; margin-bottom: 15px; border-collapse: collapse; font-size: 13px; text-align: left;'>
      <tr>
        ...
        <td style='padding: 5px; border: 1px solid #e5e5e5;'><?php echo $change->id;?></td>

      </tr>
      <tr>
          <th style='width: 100px;border: 1px solid #e5e5e5; background-color: #f5f5f5; padding: 5px;'><?php echo $this->lang->custommail->changeCode;?></th>
          <td style='padding: 5px; border: 1px solid #e5e5e5; text-align: left;'>
        <?php
        $detailsURL = zget($this->config->mail, 'domain', common::getSysURL()) . helper::createLink('change', 'view', "id=$change->id", 'html');
        echo html::a($detailsURL, $change->code, '', 'style="color:blue;"');
        ?>
        </td>
      </tr>
      <tr>
          ...
          <td style='padding: 5px; border: 1px solid #e5e5e5;'><?php echo zget($this->lang->change->statusList, $change->status);?></td>
      </tr>
      <tr>
          ...
          <?php
          if($change->status == 'reject'){
              $reviewerUsersSubStr = zget($users,$change->createdBy);
          }else{
          ...
          ?>
          <td style='padding: 5px; border: 1px solid #e5e5e5;'><?php echo $reviewerUsersSubStr;?></td>
      </tr>
      <tr>
          <th style='width: 100px; border: 1px solid #e5e5e5; background-color: #f5f5f5; padding: 5px;'><?php echo $this->lang->custommail->changedReason;?></th>
          <td style='padding: 5px; border: 1px solid #e5e5e5;' class="textOVerThree"><?php echo $change->reason;?></td>
      </tr>
      <tr>
