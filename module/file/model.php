...
//            ->andWhere('objectID')->eq((int)$objectID)
     ...
     * @return array
     */
    public function getByObjectExtra($objectType, $objectID, $extra = '')
    {
        $files = $this->dao->select('*')->from(TABLE_FILE)
            ->where('objectType')->eq($objectType)
            ->andWhere('objectID')->eq((int)$objectID)
            ->andWhere('extra')->ne('editor')
            ->andWhere('deleted')->eq('0')
            ->beginIF($extra)->andWhere('extra')->ne($extra)
            ->orderBy('id')
            ->fetchAll('id');
        foreach($files as $file)
        {
    ...
    //自定义excel width
    public function setExcelWidth($width){
        self::$_setExcelWidth = (int)$width;
    }
    /**
     * Get info of a file.
     *
    ...
    public function saveUpload($objectType = '', $objectID = '', $extra = '', $filesName = 'files', $labelsName = 'labels')
    {
        ...
        foreach($files as $id => $file)
        {
            if($objectType == 'review'){
                if($file['size']  > $this->config->review->fileSize->fileSize*1024*1024){
                    return dao::$errors['files'] = sprintf($this->lang->file->fileOverSize, $this->config->review->fileSize->fileSize);
                }
            }
            if($file['size'] == 0) continue;
            if($objectType=='environmentorder'){
                $file['pathname']   =  $file['title'];
            }
            if($objectType=='environmentorder'){
            ...
            $file['extra']   =$extra;
            if($objectType=='environmentorder'){
                $file['extra']   = hash_file('md5',$this->savePath . $file['pathname']);
            }
            unset($file['tmpname']);
    ...
    public function setSavePath()
    {
        ...
        if(!file_exists($savePath))
        {
            ...
            if(PHP_SAPI == 'cli'){
                chmod($savePath,0777);
                chmod($savePath . 'index.html',0777);
                @chown($savePath,'apache');
                @chown($savePath . 'index.html','apache');
                @chgrp($savePath,'apache');
                @chgrp($savePath . 'index.html','apache');
            }
        }
    ...
    public function pasteImage($data, $uid = '')
    {
        ...
        {
            //$data = fixer::stripDataTags(urldecode($data)); //urldecode 转+ 号会变成空格
    ...
    public function replaceImgURL($data, $fields)
    {
        ...
        $url = isset($_SERVER['HTTP_HOST']) ? 'http://'.$_SERVER['HTTP_HOST'] : ''; //cli模式下没有 报错
        foreach($fields as $field)
        {
            if(empty($field) or empty($data->$field)) continue;
            $link = $url . helper::createLink('file', 'read', "fileID=$1", "$3");
            if(strpos($link, 'mobileapi.php') !== false){
                $link = str_replace($url,'',$link);
                $addr = isset($_SERVER['REMOTE_ADDR']) ? 'http://'.$_SERVER['REMOTE_ADDR'] : '';
                // 绝对路径 ip替换为空
                $data->$field = str_replace($url,'',str_replace($addr,'',$data->$field));
                // 生产环境域名替换为空
                $data->$field = str_replace('http://dpmp.cfit.cn','',$data->$field);

                $data->$field = str_replace('img src="/js','img src="/cfitpms/js',$data->$field);
                if (strpos($link, 'cfitpms') === false){
                    $link = str_replace('/mobileapi.php','/cfitpms/mobileapi.php',$link);
                }
                if(strpos($data->$field, 'file-read') !== false && strpos($data->$field, 'cfitpms/file-read') === false){
                    $data->$field = str_replace('/file-read','/cfitpms/file-read',$data->$field);
                }
            }

            $data->$field = preg_replace('/ src="{([0-9]+)(\.(\w+))?}" /', ' src="' . $link . '" ', $data->$field);
    ...
    public function sendDownTmpHeader($fileName, $fileType, $content, $type = 'content')
    {
        /* Clean the ob content to make sure no space or utf-8 bom output. */
        $obLevel = ob_get_level();
        for($i = 0; $i < $obLevel; $i++) ob_end_clean();
     ...
     * @return bool
     */
    public function saveApiFile($objectType, $objectID, $url, $filename)
    {
        //        if(filter_var($url, FILTER_VALIDATE_URL) === false){ //url不正确
//            return false;
//        }
        $pathName = $this->getUrlFile($url); //下载文件 //已经crc32后的文件
     ...
     * @return bool
     */
    public function deleteFile($objectType, $objectID){
        if(!($objectType && $objectID)){
            return false;
        }
        return  $this->dao->update(TABLE_FILE)->set('deleted')->eq(1)
            ->where('objectType')->eq($objectType)
     ...
     * @return mixed
     */
    public function getFileInfo($objectType,$objectID)
    {
        if(!($objectType && $objectID)){
            return false;
        }
     ...
     * @Date: 2023/8/23
     * @Time: 17:58
     * @Interface deleteAllFile
     * @param $ids
     */
    public function deleteAllFile($ids)
    {
        if(!($ids)){
            return false;
        }
     ...
     * @Date: 2023/8/23
     * @Time: 18:12
     * @Interface updateFileObjectType
     * @param $requirementID
     * @param $newObjectType
     * @param $ids
     * @return false
     */
    public function updateFileObjectType($requirementID,$newObjectType, $ids)
    {
        if(!($requirementID && $ids)){
            return false;
        }
        return  $this->dao->update(TABLE_FILE)
            ->set('objectType')->eq($newObjectType)
            ->set('objectID')->eq($requirementID)
            ->where('id')->in($ids)
            ->exec();
    }
     ...
     * @Date: 2023/8/23
     * @Time: 18:20
     * @Interface getByObjectHaveDelete
     * @param $ids
     * @return mixed
     */
    public function getByObjectHaveDelete($ids)
    {
        $files = $this->dao->select('*')->from(TABLE_FILE)
            ->where('id')->in($ids)
            ->fetchAll('id');
     ...
     * @return array|bool
     */
    public function getFileListByObjectIds($objectType, $objectIds){
        $data = [];
        if(!($objectType && $objectIds)){
            return false;
        }
        $ret = $this->dao->select('*')->from(TABLE_FILE)
            ->where('objectType')->eq($objectType)
