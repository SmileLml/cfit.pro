...
{
    const WHITELIST_LEADER_TYPE = 1001;
    /**
    ...
    {
        return $this->dao->select('t1.id,t1.account, t1.reason, t1.subObjectID, t2.realname,t2.dept,t2.role,t2.phone,t2.qq,t2.weixin,t2.email')->from(TABLE_ACL)->alias('t1')
            ->leftjoin(TABLE_USER)->alias('t2')->on('t1.account = t2.account')
    ...
    {
        //历史报名单
        $oldWhitelist = $this->dao->select('account,objectType,objectID,type,source,reason')->from(TABLE_ACL)->where('objectID')->eq($objectID)->andWhere('objectType')->eq($objectType)->fetchAll('account');
    ...
    public function deleteProjectWhitelist($objectID = 0, $account = '')
    {
        ...
        //是否项目领导
        $isLeader  = $this->dao->select('*')->from(TABLE_ACL)->where('objectID')->eq($projectID)->andWhere('reason')->eq(self::WHITELIST_LEADER_TYPE)->andWhere('objectType')->eq('project')->fetch();

        /* Determine if the user exists in other sprints in the project set. */
        if(empty($whitelist) && empty($isLeader))  //tongyanqi 2022-04-19
        {
