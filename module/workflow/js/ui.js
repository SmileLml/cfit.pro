...
$(function()
{
    ...
    var updateField = function(field, relativeField, relativeAsNext)
    {
        ...
        $field.attr('data-order', field.order)
            ...
            .toggleClass('field-required', (',' + field.layoutRules + ',').indexOf(',' + notEmptyRule + ',') > -1);
    ...
    var validateField = function(field, includeHiddenField)
    {
        ...
        if(!errors.name)
        {    
            if(typeof field.name !== 'string' || !field.name.length) errors.name = validateMessages.nameRequired;
            else 
            {    
                for(var i = 0; i < fields.length; ++i) 
                {    
                    ...
                    {    
                        errors.name = validateMessages.nameDuplicated;
                        break;
                    }    
                }    
            }    
        ...
        if(!buildin)
        {
            ...
            {
                if(typeof field.length !== 'string' || !field.length.length) errors.length = validateMessages.lengthRequired;
            }
    ...
    var updateFieldTypeTip = function(field)
    {
        ...
        switch(field.type)
        {
            ...
            case 'int':
                ...
                tip = tips.number.replace(/MAX/, max).replace(/MIN/, min);
            ...
            case 'char':
                ...
                tip = tips.string.replace(/LENGTH/, length);
                break;
            case 'varchar':
                var length = field.length;
                length = length ? parseInt(length) : 0;
    ...
    $filedEditForm.on('change', function(e)
    {
        ...
        if(oldValue !== controlValue)
        {
            ...
            if(field.errors)
            {
                ...
                if(controlName === 'decimalDigits') delete field.errors.decimalDigits;
            }
        }
        validateField(field);
        updateField(field);
    ...
    $('a[href=#confirmReleaseModal]').on('click', function()
    {    
        if(saveFields(function(result)
        {    
            if(result) $('a[href=#confirmReleaseModal]').click();
        })) return false;
    });
