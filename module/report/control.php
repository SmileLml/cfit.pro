...
class report extends control
{
    ...
    public function projectDeviation($begin = 0, $end = 0)
    {
        ...
        $executions = array();
        if($begin or $end) $executions = $this->report->getExecutions($begin, $end);
    ...
    public function productSummary($conditions = '')
    {
        ...
        $products = array();
        if($conditions) $products = $this->report->getProducts($conditions);
    ...
    public function bugCreate($begin = 0, $end = 0, $product = 0, $execution = 0)
    {
        ...
        $bugs = array();
        if($product or $execution) $bugs = $this->report->getBugs($begin, $end, $product, $execution);
     ...
     * @return void
     */
    public function bugAssign($query = 0)
    {
        $assigns = array();
        if($query) $assigns = $this->report->getBugAssign();
