...
class datatableModel extends model
{
    ...
    public function getFieldList($module, $method = '')
    {
        ...
        if($module == 'project')
        {
            if($method == 'testcase')  $fromModule = 'testcase'; 
            if($method == 'bug')       $fromModule = 'bug'; 
            if($method == 'testsuite') $fromModule = 'testsuite';  
            if($method == 'testtask')  $fromModule = 'testtask';
        }

        if(!isset($this->config->$module))   $this->loadModel($module);
        if(!isset($this->config->$fromModule)) $this->loadModel($fromModule);
        ...
        foreach($this->config->$module->datatable->fieldList as $field => $items)
        {
            ...
            $title = zget($this->lang->$fromModule, $items['title'], null);

            if(empty($title)) $title = zget($this->lang->$module, $items['title'], null);
            if(empty($title)) $title = zget($this->lang, $items['title'], $items['title']);

            $this->config->$module->datatable->fieldList[$field]['title'] = $title;
        }
        if(isset($this->config->bizVersion))
        {
            $fields = $this->loadModel('workflowfield')->getList($fromModule);
     ...
     * @return object
     */
    public function getSetting($module, $method = '')
    {
    ...
    public function printHead($col, $orderBy, $vars, $checkBox = true)
    {
        ...
        if($col->show)
        {
            ...
            if($id == 'id' and (int)$width < 90) $width = '90px'; //todo 2022-04-19 改这个会有问题
    ...
    public function printHeadFixed($col, $orderBy, $vars, $checkBox = true)
    {
        $id = $col->id;
        if($col->show)
        {
            ...
            $title = isset($col->title) ? "title='$col->title'" : '';
            $title = (isset($col->name) and $col->name) ? "title='$col->name'" : $title;
            if($id == 'id' and (int)$width < 90) $width = '60px';
            $align = $id == 'actions' ? 'text-center' : '';
            $align = in_array($id, array('budget', 'teamCount', 'estimate', 'consume')) ? 'text-right' : $align;
            $style  = '';
            $data   = '';
