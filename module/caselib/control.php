...
class caselib extends control
{
    ...
    public function browse($libID = 0, $browseType = 'all', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        /* 获取固定排序字段。 */
        if(isset($this->config->caselib->browse->fixedSort)) $orderBy = $this->config->caselib->browse->fixedSort;
    ...
    public function batchEditCase($libID = 0, $branch = 0)
    {
        $this->loadModel('testcase');
        if($this->post->title)
        {
            $this->config->testcase->edit->requiredFields = 'title,type';
            ...
            die(js::locate($this->createLink('caselib', 'browse', "libID=$libID&browseType=byModule&param=0"), 'parent'));
        }
    ...
    public function export($libID, $browseType = 'all', $param = 0, $orderBy)
    {
        if($_POST)
        {
            $browseType = strtolower($browseType);
            ...
            $caselibLang   = $this->lang->caselib;
            $caselibConfig = $this->config->caselib;

            /* Create field lists. */
            $fields = $this->post->exportFields ? $this->post->exportFields : explode(',', $caselibConfig->exportFields);
            foreach($fields as $key => $fieldName)
            {
                $fieldName          = trim($fieldName);
                $fields[$fieldName] = isset($caselibLang->$fieldName) ? $caselibLang->$fieldName : $fieldName;
                unset($fields[$key]);
            }
            ...
            $cases = $this->caselib->getLibCases($libID, $browseType, $queryID, $moduleID, $sort);
            ...
            $showModule  = !empty($this->config->datatable->caselibBrowse->showModule) ? $this->config->datatable->caselibBrowse->showModule : '';
            $modulePairs = $this->loadModel('tree')->getModulePairs($libID, 'caselib', $showModule);

            foreach ($cases as $key => $case)
            {
                $cases[$key]->module = isset($modulePairs[$case->module]) ? $modulePairs[$case->module] : '';
                $cases[$key]->priAB  = zget($this->lang->testcase->priList, $case->pri, $case->pri);
                $cases[$key]->type   = zget($this->lang->testcase->typeList, $case->type, $case->type);
                ...
                if($case->stage)
                {
                    ...
                    foreach($stags as $stage)
                    {
                        ...
                        $stageList[] = isset($this->lang->testcase->stageList[$stage]) ? $this->lang->testcase->stageList[$stage] : $stage;
                    }
                }
                $case->stage  = implode(',', $stageList);
                $categoryList = [];
                if($case->categories)
                {
                    $categories = explode(',', $case->categories);
                    foreach ($categories as $category)
                    {
                        ...
                        $categoryList[] = isset($this->lang->testcase->categoryList[$category]) ? $this->lang->testcase->categoryList[$category] : $category;
                    }
                }
                $case->categories = implode(',', $categoryList);
                ...
                if(isset($relatedSteps[$case->id]))
                {
                    ...
                    foreach($relatedSteps[$case->id] as $step)
                    {
                        ...
                        if($step->version != $case->version)
                        {
                            continue;
                        }
                        $sign = (in_array($this->post->fileType, ['html', 'xml'])) ? '<br />' : "\n";
        ...
        $fileName = $this->lang->caselib->common;

        $case = $this->caselib->getByID($libID);

        $this->view->fileName        = $fileName . '-' . $case->name;
