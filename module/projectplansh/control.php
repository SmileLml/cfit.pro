<?php
class projectPlansh extends control
{
    /**
    ...
    public function browse($browseType = 'all', $param = 0, $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1, $isSecondline = 0,$shanghaipart = 0)
    {
        ...
        $actionURL = $this->createLink('projectplansh', 'browse', "browseType=bySearch&param=myQueryID&order=$orderBy&recTotal=$recTotal&recPerPage=$recPerPage&pageID=$pageID&secondline=$isSecondline");
        $this->projectplansh->buildSearchForm($queryID, $actionURL);
        /* Load pager. */
        $this->app->loadClass('pager', $static = true);
        $pager = new pager($recTotal, $recPerPage, $pageID);
        $this->session->set('projectplanshList', $this->app->getURI(true));
        $userDept = $this->loadModel('user')->getUserDeptName($this->app->user->account);
        $plans = $this->projectplansh->getList($browseType, $queryID, $orderBy, $pager, $isSecondline,'projectplansh',$shanghaipart);
...
//            if(in_array($plan->status,array("yearreject","yearstart","yearpass","start"))){
        ...
        $this->view->title      = $this->lang->projectplansh->common;
    ...
    private function setSearchList()
    {
        ...
        $this->config->projectplansh->search['params']['outsideProject']['values'] += $outside;
        $this->loadModel('opinion');
        $this->config->projectplansh->search['params']['category']['values'] = $this->lang->opinion->categoryList;

        $this->config->projectplansh->search['params']['app']['values'] = array(0 => '') + $this->loadmodel('application')->getapplicationNameCodePairs();
        $this->config->projectplansh->search['params']['bearDept']['values'] = $this->loadModel('dept')->getOptionMenu();
        ...
        $this->config->projectplansh->search['params']['outsideProject']['values'] = $vlist;
        ...
        $this->config->projectplansh->search['params']['outsideSubProject']['values'] = $vlist;
        ...
        $this->config->projectplansh->search['params']['outsideTask']['values'] = $vlist;
    }
    /**
     * Project: chengfangjinke
     * Method: create
    ...
    public function create($secondline  = 0)
    {
        ...
        {
            $this->checkPost();//校验字段
            $planID = $this->projectplansh->create();
        ...
        $this->view->title    = $this->lang->projectplansh->create;
...
//        $this->view->outsideProject = $this->loadModel('outsideplan')->getPairsHavingTasks();
    ...
    public function editPlanOpinion($planID,$opinionID){
        ...
        if($_POST)
        {
            ...
            $changes = $this->projectplansh->editPlanOpinion();
            if(dao::isError())
            {
                $response['result']  = 'fail';
                $response['message'] = dao::getError();
        ...
        $plan = $this->projectplansh->getSimpleByID($planID);
        $planRequirementList = [];
        if($plan->requirement){
    ...
    public function editPlanRequirement($planID,$requirementID){
        ...
        if($_POST)
        {
            ...
            $changes = $this->projectplansh->editPlanRequirement();
            if(dao::isError())
            {
                $response['result']  = 'fail';
                $response['message'] = dao::getError();
    ...
    {
        $this->view->plan     = $this->projectplansh->getByID($planID);
        if($_POST)
        {
            if(array_key_exists($_POST['isDelayPreYear'],$this->lang->projectplansh->isDelayPreYearList)){
                $this->dao->update(TABLE_PROJECTPLAN)->data(['isDelayPreYear'=>$_POST['isDelayPreYear']])->where('id')->eq($planID)->exec();
            }else{
                dao::$errors['realRelease'] = $this->lang->projectplansh->isDelayPreYearError;
    ...
    public function edit($planID = 0, $confirm = 'no')
    {
        ...
        {
            $this->checkPost();//校验字段
            $changes = $this->projectplansh->update($planID);
        ...
        $products = $this->loadModel('product')->getProductInfo();
        $this->view->title    = $this->lang->projectplansh->edit;
        $this->view->users    = $this->loadModel('user')->getPairs('noclosed');
        $this->view->plan     = $this->projectplansh->getByID($planID);
        $this->view->lines    = array('' => '') + $this->loadModel('product')->getLinePairs();
        $this->view->products = [''] + $products;
...
//        $this->view->outsideSubProject  = $this->loadModel('outsideplan')->getSubProjectPairs($this->view->plan->outsideSubProject);
    ...
    public function view($planID = 0)
    {
        ...
        $plan = $this->projectplansh->getByID($planID);
        $creationID = $plan->creation->id ?? 0;
        $this->view->creationID  = $creationID;
        $this->view->plan  = $plan;
        $this->view->title = $this->lang->projectplansh->view;
        ...
        $this->view->plans    = $this->projectplansh->getPairs();
        ...
        if($yearNodes){
            ...
            if($yearNodes[0]->nodeCode == 'builtPerson'){
                for ($i=$yearVersion-1; $i>=0; $i--){
                    $beforeNode = $this->loadModel('projectplansh')->getNodesByWhere('projectplanyear', $planID, $i,'pass','deptLeader');
                    if($beforeNode){
...
//        a($this->view->plan->changeVersion);
        ...
        if($mainRelationInfo || $slaveRelationInfo){
            ...
            $this->view->relationProjectplanList = array_column($this->projectplansh->getByIDMultipleList(array_unique($planArr),"id,name"),'name','id');

        }

        $allow = $this->loadModel('projectplansh')->isBuildDeptPerson($planID);
        //发起人看到详情所有节点
...
//        $this->view->relatedObject = $relatedObjectList;
        ...
        $this->view->products  = $this->projectplansh->getCreationProducts($plan->project);
...
//        a($changeYearNodes[1]);exit();
        ...
        if($plan->oldview == -1){ //旧版的demand都更新为-1了 旧版显示旧的页面
            $this->display('projectplansh', 'oldview');
        } else {
            $this->display();
        }
    ...
    public function ajaxshowdiffchange($changeID=0){
        $this->app->loadLang('opinion');
        $this->app->loadConfig('execution');
        $res = $this->projectplansh->ajaxshowdiffchange($changeID);
...
//        $this->view->projectplanChange = $res;
        ...
        $this->view->title    = $this->lang->projectplansh->edit;
        $this->view->users    = $this->loadModel('user')->getPairs('noclosed');
//        $this->view->plan     = $this->projectplansh->getByID($res->planID);
...
//        a($this->view->planStages);
    ...
    public function initProject($planID = 0)
    {
        ...
        if($_POST)
        {
            $data = $this->projectplansh->initProject($planID);
            ...
            $response['result']  = 'success';
            $response['message'] = $this->lang->saveSuccess;
            $response['locate']  = $this->createLink('projectplansh','view',"planID=$planID");

            $this->send($response);
        }

        $this->view->title    = $this->lang->projectplansh->initProject;
        $this->view->actions  = $this->loadModel('action')->getList('projectplan', $planID);
        $this->view->plan     = $this->projectplansh->getByID($planID);
        $this->view->plans    = $this->projectplansh->getPairs();
        $this->view->creation = $this->projectplansh->getCreationByID($planID);
        $this->view->users    = $this->loadModel('user')->getPairs('noclosed|noletter');
        $this->view->lines    = array('' => '') + $this->loadModel('product')->getLinePairs();
    ...
    {
        $oldCreation =  $this->projectplansh->getCreationByID($planID);
        ...
        if($_POST)
        {
            $data = $this->projectplansh->initProject($planID, 1);
    ...
    public function submit($planID = 0)
    {
        if($_POST)
        {
            $this->projectplansh->submit($planID);
            ...
            //$this->projectplansh->sendmail($planID, $actionID);
        ...
        if($isInerProject){
            ...
            //外部项目过滤  产创和架构
            $filterDepts = $this->lang->projectplansh->submitFilterDept;
            // 如果 补齐 项目负责人 的部门
            if(!in_array(zget($this->view->deptParent,$this->app->user->dept),$this->lang->projectplansh->submitFilterDept)){
                $filterDepts[] = zget($this->view->deptParent,$this->app->user->dept);
            }
        }
        ...
        $this->view->users = $this->loadModel('user')->getPairs('noclosed|noletter');
        //上海分公司会签需要显示二级部门
        $isShanghai = $this->projectplansh->isShangHai($planID);
        $this->view->depts = $this->projectplansh->getTopDepts($isShanghai );
        //过滤 产创/架构/本部门
    ...
    public function yearReview($planID = 0)
    {
        if($_POST)
        {
            $this->projectplansh->yearReview($planID);
        ...
        $planPerson = $this->loadModel('projectplansh')->isBuildDeptPerson($planID);
        if(!empty($planPerson)){
        ...
        $this->view->title = $this->lang->projectplansh->yearReview;
        $this->view->plan  = $plan;
        $this->view->users = $this->loadModel('user')->getPairs('noclosed|noletter');
        $this->view->leaders = $this->loadModel('user')->getLeaders();
        $this->view->depts = $this->projectplansh->getAllDepts();
    ...
    {
        $projectplan = $this->projectplansh->getByID($planID);
        if($_POST)
        {
            ...
            $result = $this->projectplansh->review($planID);
            ...
            if($result['mailsend']){
                $this->projectplansh->sendPlanProjectedMail($planID, $actionID);
            }
...
//        $this->view->curPendingNode = $this->review->getFirstPendingNode('projectplan', $planID, $projectplan->version);
    ...
    public function addPlanAction($actionResult){
        ...
        $this->projectplansh->addPlanAction($actionResult);
        //触发邮件
        $this->projectplansh->sendActionmail($actionResult['planID']);
    ...
    public function yearReviewing($planID = 0)
    {
        ...
        if($_POST)
        {
            $result = $this->projectplansh->yearReviewing($planID);
        ...
        //防止在地盘待处理和年度计划页面重复审批
        $repeatVerify = $this->projectplansh->selectActionInfo($planID,$projectplan->yearVersion,'projectplanyear');
        if(!$repeatVerify){
            die(js::alert($this->lang->projectplansh->approvalEmpty) .js::closeModal('parent'));
        }
        ...
        if($projectplan->reviewStage == 2 && $projectplan->rejectStatus == 3 ){
            $nodeCode = $this->lang->projectplansh->nodeCode[3];
        ...
        unset($leadersMergeInfo['maoting']);
        $leaderCN = array_flip($leadersMergeInfo);
        foreach ($leaderCN as $name){
            $leader[$name] = $users[$name];
        }

        $cto = array('maoting'=>'茅廷');
        $title = $this->lang->projectplansh->nodeCodeDesc[$this->lang->projectplansh->nodeCode[$projectplan->reviewStage]];
...
//        a($projectplan);
        if($_POST)
        {
            $result = $this->projectplansh->yearBatchReviewing($planID);
    ...
    {
        $projectplan = $this->projectplansh->getByID($planID);
        $changeReviewInfo = $this->projectplansh->getChangePlanInfo($planID,$projectplan->changeVersion);
        $changeReviewInfo = $this->loadModel('file')->replaceImgURL($changeReviewInfo, 'planRemark');
        if($_POST)
        {
            $result = $this->projectplansh->changeReview($planID);
        ...
        $title = $this->lang->projectplansh->nodeCodeDesc[$this->lang->projectplansh->changeNodeCode[$projectplan->changeStage]];
        $this->view->plan      = $projectplan;
        $this->view->title     = $title;
...
//        $this->view->changePlaInfo = $changeReviewInfo;
    ...
    public function delete($planID, $confirm = 'no')
    {
        if($confirm == 'no')
        {
            echo js::confirm($this->lang->projectplansh->confirmDelete, $this->createLink('projectplan', 'delete', "planID=$planID&confirm=yes"), '');
            exit;
        }
        else
        {
            $this->projectplansh->delete(TABLE_PROJECTPLAN, $planID);
//            $this->dao->update(TABLE_PROJECTPLAN)->set('status')->eq('deleted')->where('id')->eq($planID)->exec();
    ...
    public function exec($planID = 0)
    {
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            /*if(empty($this->post->opinion[0])){
                dao::$errors['opinion'] = $this->lang->projectplansh->opinionEmpty.'!!';
                $response['result']  = 'fail';
                $response['message'] = dao::getError();
                $this->send($response);
            }
            if(empty($this->post->requirement[0])){
                dao::$errors['requirement'] = $this->lang->projectplansh->requirementEmpty2;
                $response['result']  = 'fail';
                $response['message'] = dao::getError();
                $this->send($response);
            }
            if(empty($this->post->demand[0])){
                dao::$errors['demand'] = $this->lang->projectplansh->demandEmpty.'!!';
                $response['result']  = 'fail';
                $response['message'] = dao::getError();
                $this->send($response);
            }*/
            ...
            $this->projectplansh->exec($planID);
    ...
    public function relationExec($projectID = 0)
    {
         ...
         if(empty($_POST) or !implode('', $this->post->products))
            {
                $response = array();
                $response['result']  = 'fail';
                $response['message'] = $this->lang->projectplansh->productEmpty;
                $this->send($response);
                return;
            }

            $plans = $_POST['plans'];
            $list = $this->projectplansh->getProductInfo($_POST['products']);
            ...
            $this->projectplansh->addRelation($projectID, json_encode($relations)); //记录详细产品版本关系内容
            $this->projectplansh->recordRelationPlan($projectID, $_POST['products'], $plans); //更新产品版本关系型数据表
        ...
        $this->view->plan     = $this->projectplansh->getByProjectID($projectID);
        $this->view->users    = $this->loadModel('user')->getPairs('noclosed|noletter');
    ...
    public function execEdit($planID = 0)
    {
        ...
        {
            $this->checkPost();//校验字段
            $changes = $this->projectplansh->execEdit($planID);
        ...
        $this->view->title    = $this->lang->projectplansh->execEdit;
        $this->view->plan     = $plan;
...
//        $this->view->outsideTask        = $this->loadModel('outsideplan')->getTaskPairs($this->view->plan->outsideTask);
    ...
    public function planChange($planID = 0)
    {
        ...
        {
            $this->checkPost();//校验字段
            $changes = $this->projectplansh->planChange($planID);
...
//        $this->view->outsideSubProject  = $this->loadModel('outsideplan')->getSubProjectPairs($this->view->plan->outsideSubProject);
    ...
    public function export($orderBy = 'id_desc', $browseType = 'all')
    {
        ...
        if($_POST)
        {
            ...
            $this->projectplansh->setListValue();

            $this->loadModel('file');
            $projectplanLang   = $this->lang->projectplansh;
            $projectplanConfig = $this->config->projectplansh;
            ...
            /* Get projectplans. */
            $projectplans = array();
            if($this->session->projectplanshOnlyCondition)
            {
                $projectplans = $this->dao->select('*')->from(TABLE_PROJECTPLAN)->where($this->session->projectplanshQueryCondition)
                    ->beginIF($this->post->exportType == 'selected')->andWhere('id')->in($this->cookie->checkedItem)->fi()
                    ->orderBy($orderBy)->fetchAll('id');
            }
            else
            {
                $stmt = $this->dbh->query($this->session->projectplanshQueryCondition . ($this->post->exportType == 'selected' ? " AND $field IN({$this->cookie->checkedItem})" : '') . " ORDER BY " . strtr($orderBy, '_', ' '));
            ...
            //获取所有年度计划
            $projectplanList = $this->projectplansh->getAllList("id,mark");
            ...
            foreach($projectplans as $projectplan)
            {
                ...
                if(isset($projectplanLang->statusList[$projectplan->status])){
                    ...
                    if($projectplan->status==$this->lang->projectplansh->statusEnglishList['yearpass'] && $projectplan->changeStatus == $this->lang->projectplansh->ChangestatusEnglishList['pending']){
                        $projectplanstatusstr = $this->lang->projectplansh->changeing;
                    }else{
                        $projectplanstatusstr = zget($this->lang->projectplansh->statusList, $projectplan->status, '');
                        if($projectplan->changeStatus == $this->lang->projectplansh->ChangestatusEnglishList['pass']){
                            $projectplanstatusstr .= $this->lang->projectplansh->changePass;
                        }else if($projectplan->changeStatus == $this->lang->projectplansh->ChangestatusEnglishList['reject']){
                            $projectplanstatusstr .= $this->lang->projectplansh->changeReject;
                        }
                    }
//                    $projectplan->status      = $projectplanLang->statusList[$projectplan->status];
                ...
                $projectplan->platformowner = zmget($this->lang->projectplansh->platformownerList,$projectplan->platformowner);
...
//                $creation = $this->dao->select('id,dept')->from(TABLE_PROJECTCREATION)->where('plan')->eq($projectplan->id)->fetch();
            ...
            $this->post->set('kind', 'projectplansh');
            $this->fetch('file', 'export2' . $this->post->fileType, $_POST);
        }

        $this->view->fileName        = $this->lang->projectplansh->common;
        $this->view->allExportFields = $this->config->projectplansh->list->exportFields;
    ...
    public function exportTemplate()
    {
        if($_POST)
        {
            ...
            foreach($this->config->projectplansh->export->templateFields as $field) $fields[$field] = $this->lang->projectplansh->$field;
    ...
    public function exportHistorybak($orderBy = 'id_desc', $browseType = 'all')
    {
        ...
        $this->view->fileName        = $this->lang->projectplansh->changeHistory;
        $this->view->allExportFields = $this->config->projectplansh->list->exportFields;
        $this->view->customExport    = false;
        $this->display();
    }
...
//            a($actionList);exit();
    ...
    {
        $this->app->loadLang('opinion');
        $this->lang->projectplansh->categoryList = $this->lang->opinion->categoryList;
        ...
        if($_POST)
        {
            $this->projectplansh->createFromImport();
        ...
        $this->view->title      = $this->lang->projectplansh->common . $this->lang->colon . $this->lang->projectplansh->showImport;
        $this->view->position[] = $this->lang->projectplansh->showImport;
    ...
    private function checkPost()
    {
        ...
        {
            dao::$errors['end'] = [$this->config->projectplansh->endError];
            $response['result']  = 'fail';
            $response['message'] = dao::$errors;
            $this->send($response);
        }
        if(!$this->post->owner)
        {
            dao::$errors['owner'] = [$this->config->projectplansh->ownerEmpty];
            $response['result']  = 'fail';
            $response['message'] = dao::$errors;
            $this->send($response);
        }
        ...
        for ($i = 0; $i<count($_POST['stageBegin']); $i++)
        {
            ...
            if ($_POST['stageBegin'][$i] > $_POST['stageEnd'][$i])   {
                dao::$errors['stageEndError2'] = [sprintf($this->config->projectplansh->stageEndError2, $i + 1)];
                $response['result']  = 'fail';
                $response['message'] = dao::$errors;
                $this->send($response);
            }
