...
<div id="mainMenu" class="clearfix table-row">
  ...
  <div class="btn-toolbar pull-right">
    ...
    <?php if(common::canModify('execution', $execution) and empty($disabled)):?>
    <div class="btn-group">
    ...
    if(common::hasPriv('task', 'create', $checkObject)) echo html::a($link, "<i class='icon icon-plus'></i> {$lang->task->create}", '', "class='btn btn-primary'" . $disabled);
    ?>
  </div>
</div>
    ...
    <tbody>
      ...
      <tr data-id='<?php echo $groupIndex?>' <?php if($groupIndex > 1 and $i == 0) echo "class='divider-top'";?>>
        ...
        <td class="c-actions">
          ...
          <?php common::printIcon('task', 'assignTo', "executionID=$task->execution&taskID=$task->id", $task, 'list', '', '', 'iframe', true, $disabled);?>
          <?php common::printIcon('task', 'edit', "taskid=$task->id", '', 'list', '', '', '', '', $disabled);?>
          <?php common::printIcon('task', 'delete', "executionID=$task->execution&taskid=$task->id", '', 'list', 'trash', 'hiddenwin', '', '', $disabled);?>
          <?php endif;?>
        </td>
      </tr>
      <?php $i++;?>
