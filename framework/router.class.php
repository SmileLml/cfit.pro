...
class router extends baseRouter
{
    ...
    public function setCommonLang()
    {
        ...
        $config->secondLink = 'modify-browse';
        $config->backlogLink = 'demand-browse';

        /* Get user preference. */
        $account     = isset($this->session->user->account) ? $this->session->user->account : '';
        if($this->dbh and !empty($this->config->db->name)) $userSetting = $this->dbh->query('SELECT `key`, value FROM' . TABLE_CONFIG . "WHERE `owner`='{$account}' AND `module`='common' and `key` in ('programLink', 'productLink', 'projectLink', 'executionLink', 'URSR', 'secondLink')")->fetchAll();
        foreach($userSetting as $setting)
        {
             ...
             if($setting->key == 'secondLink')   $config->secondLink   = $setting->value;
             if($setting->key == 'projectLink')  {
                 //$config->projectLink   = $setting->value;
                 //需求:当用户个性化设置为'默认进入最近一个项目仪表盘，可以查看当前项目概况'时，若无该项目权限，则跳到项目列表页
                 if(strpos($setting->value,'index') !== false){ //project-index
                     if(isset($app->user->view->projects)){
                         $userview = explode(',',$app->user->view->projects);
                         $projectid = $this->session->project;
                         if(in_array($projectid,$userview)){ //有权限
                             $config->projectLink   = $setting->value;
                         }else{
                             $config->projectLink   = 'project-browse';
                         }
