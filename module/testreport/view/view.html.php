  ...
  <div class='tab-content'>
    <div class='tab-pane' id='basic'>
      <table class='table table-form'>
        <tr>
          <th class='w-100px'><?php echo $lang->testreport->project;?></th>
          <td class='w-p50'><?php if(!empty($projectPlan)) common::printLinks('projectplan', 'view', "id={$projectPlan->id}", $projectPlan->name, '', 'class="text-primary" data-app="platform"');?></td>
          <td></td>
        </tr>
        <tr>
          <th><?php echo $lang->testreport->applicationID;?></th>
          <td>
          <?php $applicationName = zget($applicationList, $report->applicationID, '');
          common::printLinks('application', 'view', "applicationID={$report->applicationID}", $applicationName, '', 'data-app="application" class="text-primary"');
          ?>
          </td>
        </tr>
        <tr>
          <th><?php echo $lang->testreport->product;?></th>
          <td>
          <?php
          $productName = zget($products, $report->product, $this->lang->naProduct);
          common::printLinks('product', 'browse', "id={$report->product}", $productName, '', 'data-app="product" class="text-primary"');
          ?>
          </td>
        </tr>
        <tr>
...
<div id='mainActions' class='main-actions'>
  ...
  <div class='btn-toolbar'>
    ...
    {
        $extra = '';
        if(common::hasPriv('testreport', 'create')) echo html::a(inLink('create', "applicationID=$report->applicationID&productID=$report->product&objectType=$report->objectType&objectID=$report->objectID" . $extra),  "<i class='icon-refresh'></i>", '', "class='btn' title='{$lang->testreport->recreate}' data-app='{$this->app->openApp}'");
