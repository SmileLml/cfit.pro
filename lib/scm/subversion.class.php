...
class Subversion
{
     ...
     * @return string
     */
    public function cat($entry, $revision = 'HEAD', $output = false)
    {
        if(!scm::checkRevision($revision)) return false;

        // 判断是否将二进制流内容输入到指定文件中，在tmp目录下创建一个个人目录，每次下载都进行清空再下载。
        if($output)
        {
            // 定义临时文件存放目录。
            global $app;
            //$tmpFilePath = $app->getAppRoot() . "www/data/upload/{$app->company->id}/repo/" . $app->user->account;
            $tmpFilePath = $app->getTmpRoot() . "repo/" . $app->user->account;

            // 清空一下个人临时目录下的文件。
            $this->deleteDir($tmpFilePath);
    ...
    public function info($entry, $revision = 'HEAD')
    {
        ...
        if($info->kind == 'file')
        {
            // 兼容性处理SVN查询文件大小。
            $entry   = '"' . $this->root . '/' . $resourcePath . '"';
            $svnInfo = $this->replaceAuth(escapeCmd($this->buildCMD($entry, 'list', '--xml')));
            $svninfo = execCmd($svnInfo, 'string', $result);
            ...
            if(isset($parsedSvnInfo->list->entry->size)) $info->size = current($parsedSvnInfo->list->entry->size);
        }
        return $info;
    }
