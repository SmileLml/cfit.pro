...
class measurementModel extends model
{
     ...
     * @return object
     */
    public function getTemplates($pager = null)
    {
     ...
     * @return array
     */
    public function getReports($type, $pager = null)
    {
     ...
     * @return array
     */
    public function getSqlVars($sql)
    {
     ...
     * @return string
     */
    public function getBasicMeasValue($measurementID, $params = '')
    {
     ...
     * @return void
     */
    public function createMeasQueue($meas, $program = 0, $product = 0, $project = 0)
    {
     ...
     * @return void
     */
    public function createBasicMeasRecord($meas, $params)
    {
        $record = new stdclass();
     ...
     * @return void
     */
    public function createBasicMeasRecordByAction($meas, $record, $pp = '')
    {
        $params = json_decode($meas->params);
        if(empty($params))
        {
    ...
    {
        return $this->dao->select('id')->from(TABLE_PROJECT)->where('type')->eq('project')->andWhere('deleted')->eq('0')->fetchAll();
    }
     ...
     * @return string
     */
    public function buildParamControl($name, $controlType, $value = '', $optionType = '', $controlID = '', $mode = 'design')
    {
        ...
        elseif($controlType == 'select')
        {
            ...
            $control = html::select($name, $options, $value, "class='form-control' data-name=$name data-control=$controlID");
        }
        else
        {
     ...
     * @return string
     */
    public function showParam($param, $setting)
    {
    ...
    /**
     * Get params of an obeject parsed from holder.
     *
    ...
    public function getParams($sysData)
    {
        if($sysData->type == 'basic')
        {
            ...
            return json_decode($measurement->params, true);
        }
     ...
     * @return array
     */
    public function getTemplatePairs()
    {
        return $this->dao->select('id, name')->from(TABLE_MEASTEMPLATE)->where('deleted')->eq(0)->fetchPairs();
    }
    ...
    public function createSqlFunction($sql, $measurement)
    {
        ...
        if(!$postFunction) return array('result' => 'fail', 'errors' => $this->lang->measurement->tips->nameError);
    ...
    public function execSqlMeasurement($measurement, $vars, $action = 'save')
    {
        ...
        {
            if(!is_numeric($param)) $vars[$key] = $this->dbh->quote((string)$param);
        }
