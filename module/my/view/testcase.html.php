    ...
    <table class="table has-sort-head" id='caseList'>
      ...
      <thead>
        ...
        <th class="<?php echo $canBatchAction ? 'w-100px' : 'w-50px';?>">
          ...
          <th class='w-50px'>  <?php common::printOrderLink('pri',      $orderBy, $vars, $lang->testcase->pri);?></th>
          <th>                 <?php common::printOrderLink('title',    $orderBy, $vars, $lang->testcase->title);?></th>
          <th class='w-200px'> <?php common::printOrderLink('applicationID',    $orderBy, $vars, $lang->testcase->applicationID);?></th>
        ...
        <tr>
          <td class="c-id">
            ...
            <div class="checkbox-primary">
              <input type='checkbox' name='caseIDList[]' value='<?php echo $case->case;?>' <?php if(!$canBeChanged) echo 'disabled';?> />
              <label></label>
            </div>
            <?php endif;?>
            <?php echo sprintf('%03d', $caseID); ?>
          </td>
          ...
          <?php $applicationName = zget($applicationPairs, $case->applicationID, '');?>
          <td class='text-left'><?php if($applicationName) echo html::a($this->createLink('application', 'view', "id=$case->applicationID"), $applicationName, '', "title={$applicationName}");?></td>
          <?php if($type == 'assigntome'):?>
          ...
          <td class='c-actions'>
            ...
            if($canBeChanged)
            {
                common::printIcon('testcase', 'createBug', "applicationID=$case->applicationID&product=$case->product&branch=$case->branch&extra=caseID=$caseID,version=$case->version,runID=$runID", $case, 'list', 'bug', '', 'iframe', 'true', "data-app='qa' data-toggle=''");
                common::printIcon('testcase', 'create',  "applicationID=$case->applicationID&productID=$case->product&branch=$case->branch&moduleID=$case->module&from={$app->rawMethod}&param=$caseID", $case, 'list', 'copy', '', 'iframe', true, "data-width='95%'");
