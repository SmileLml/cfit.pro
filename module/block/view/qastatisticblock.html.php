  ...
  <div class="table-row">
    <div class="col col-nav">
      <ul class="nav nav-stacked nav-secondary" id='<?php echo $blockNavId;?>'>
        ...
        <?php $index = 1;?>
        <?php foreach($applicationList as $applicationID => $application):?>
        <li <?php if($index == 1) echo "class='active'";?> applicationID='<?php echo $application->id;?>'>
          <a href="javascript:;" data-target="#tabQaProduct<?php echo $application->id;?>" data-toggle="tab" title='<?php echo $application->name;?>'><?php echo $application->name;?></a>
        <?php echo html::a(helper::createLink('bug', 'browse', "applicationID=$application->id"), "<i class='icon-arrow-right text-primary'></i>", '', "class='btn-view' title={$lang->bug->browse}");?></li>
        <?php $index++;?>
        <?php endforeach;?>
    ...
    <div class="col tab-content">
      ...
      <div class="tab-pane fade <?php if($index == 1) echo 'active';?> in" id="tabQaProduct<?php echo $application->id;?>">
        <div class="statistic-menu input-group space w-400px">
          <div class="input-group-cell">
          <?php if($application->testtask):?>
            <?php echo $application->testtask->name;?>
            <span class="text-muted small"><a href="<?php echo $this->createLink('testtask', 'browse', "applicationID={$application->id}");?>" class="text-primary"><?php echo $lang->block->lblTesttask;?> <i class="icon icon-right-circle icon-sm"></i></a></span>
          <?php endif;?>
          </div>
        </div>
        <div class="table-row" id='bugBox<?php echo $application->id;?>'>
          <div class="col-5 text-middle text-center">
            <div class="progress-pie inline-block space progress-pie-100" data-value="<?php echo $application->assignedRate;?>" data-doughnut-size="80">
              <canvas width="100" height="100" style="width: 100px; height: 100px;"></canvas>
              <div class="progress-info">
                <small><?php echo $lang->bug->allBugs;?></small>
                <strong><?php echo empty($application->total) ? 0 : html::a($this->createLink('bug', 'browse', "applicationID={$application->id}&product=all&branch=&type=all"), $application->total);?></strong>
              </div>
            </div>
            <div class="text-center small with-padding">
              <span class="label label-dot label-primary"></span> &nbsp; <?php echo $lang->bug->assignToMe;?> &nbsp; <strong><?php echo empty($application->assignedToMe) ? 0 : html::a($this->createLink('bug', 'browse', "applicationID={$application->id}&product=all&branch=&browseType=assigntome"), $application->assignedToMe);?></strong>
            </div>
          </div>
          <div class="col-7">
            <div class="product-info">
              <div class="progress">
                <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo $application->unresolvedRate;?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $application->unresolvedRate;?>%"></div>
              </div>
              <div class="type-info">
                ...
                <div class="type-value">
                  <strong><?php echo empty($application->unresolved) ? 0 : html::a($this->createLink('bug', 'browse', "applicationID={$application->id}&product=all&branch=&browseType=unresolved"), $application->unresolved);?></strong>
                </div>
              </div>
            </div>
            <div class="product-info">
              <div class="progress">
                <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo $application->unconfirmedRate;?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $application->unconfirmedRate;?>%"></div>
              </div>
              <div class="type-info">
                <div class="type-label">
                ...
                <div class="type-value">
                  <strong><?php echo empty($application->unconfirmed) ? 0 : html::a($this->createLink('bug', 'browse', "applicationID={$application->id}&product=all&branch=&browseType=unconfirmed"), $application->unconfirmed);?></strong>
                </div>
              </div>
            </div>
            <div class="product-info">
              <div class="progress">
                <div class="progress-bar" role="progressbar" aria-valuenow="<?php echo $application->unclosedRate;?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $application->unclosedRate;?>%"></div>
              </div>
              <div class="type-info">
                <div class="type-label">
                ...
                <div class="type-value">
                  <strong><?php echo empty($application->unclosed) ? 0 : html::a($this->createLink('bug', 'browse', "applicationID={$application->id}&product=all&branch=&browseType=unclosed"), $application->unclosed);?></strong>
                </div>
              </div>
            </div>
          </div>
