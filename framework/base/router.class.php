...
class baseRouter
{
   ...
   /**
    ...
    public function setSuperVars()
    {
        ...
        if($this->config->framework->filterCSRF)
        {
            ...
            if((!defined('RUN_MODE') or RUN_MODE != 'api'  or RUN_MODE != 'mobileapi') and strpos($this->server->http_referer, "$httpType://$httpHost") !== 0) $_FILES = $_POST = array();
        }
   ...
   //------ 客户端环境有关的函数(Client environment related functions) ------//
    ...
    {
        if(isset($_COOKIE['openApp']) and $_COOKIE['openApp'] and !preg_match('/[^\w]/', $_COOKIE['openApp']))
        {
   ...
   /**
    ...
    public function mergeParams($defaultParams, $passedParams)
    {
        ...
        foreach($passedParams as $param => $value)
        {
            ...
            if($param == 'filename') continue; //下载文件不限制
    ...
    public function connectByPDO($params)
    {
        ...
        try
        {
            $dbKey = md5(json_encode($params));

            if(isset(self::$_DBH[$dbKey])){
                return self::$_DBH[$dbKey];
            } else {
