    ...
    <form class='form-indicator main-form form-ajax' method='post' target='hiddenwin' id='dataform'>
      <table class='table table-form'>
        <tr>
          ...
          <td class="col-main"><?php echo html::input('name', $project->name, "class='form-control' required");?></td><td><?php echo html::hidden('parent', 0)?></td><td></td>
        </tr>
        <tr class='hide'>
        ...
        <tr>
          ...
          <tr>
              <th class='w-120px'><?php echo $lang->project->planWorkload;?></th>
              <td><?php echo html::input('planWorkload', $project->planWorkload, "class='form-control'");?></td>
              <td></td>
              <td></td>
          </tr>
         <tr>
              <th><?php echo $lang->project->realBeginAndEnd;?></th>
              <td>
                  <div class='input-group'>
                      <?php echo html::input('realBegan', strpos('0000-00-00',$project->realBegan) !== false ? '':$project->realBegan, "class='form-control form-date' onchange='computeWorkDays();' placeholder='" . $lang->project->realEnd . "'");?>
                      <span class='input-group-addon'><?php echo $lang->project->to;?></span>
                      <?php
                      $disabledEnd = $project->realEnd == LONG_TIME ? 'disabled' : '';
                      $end         = $project->realEnd == LONG_TIME ? $lang->project->longTime : $project->realEnd;
                      $end         = strpos('0000-00-00',$end) !== false ? '': $end;
                      echo html::input('realEnd', $end, "class='form-control form-date' onchange='computeWorkDays();' $disabledEnd placeholder='" . $lang->project->realBegan . "' ");
                      ?>
                  </div>
              </td>
          </tr>
          <tr>
              <th class='w-120px'><?php echo $lang->project->workHoursTitle;?></th>
              <td><?php echo html::input('workHours', $project->workHours, "class='form-control' required");?></td>
              <td></td>
              <td></td>
          </tr>
          <tr>
              <th class='w-120px'><?php echo $lang->project->belong;?></th>
              <td><?php echo html::select('isShangHai', $this->lang->project->isShangHaiList,$project->isShangHai, "class='form-control chosen' required");?></td>
              <td></td>
              <td></td>
          </tr>
          <?php if((in_array($this->app->user->account,explode(',',$deptObj->qa))) || ($this->app->user->account == 'admin') || in_array($this->app->user->account,explode(',',$project->switchUser))):?>
              <tr>
                  <th><?php echo $lang->project->workreportSwitch;?></th>
                  <td colspan= <?php echo $this->app->user->account == 'admin' ? '3' :'';?>>
                      <div class='input-group '>
                          <?php echo html::select('switch',   $this->lang->project->workreportSwitchList,$project->switch, "class='form-control  w-200px'  ");?>

                          <?php if($this->app->user->account == 'admin'):?>
                              <span class='input-group-addon'><?php echo $lang->project->maintenanceStaff;?></span>
                              <?php
                              echo html::select('switchUser[]', $users, $project->switchUser,"class='form-control chosen w-200px' multiple");
                              ?>
                          <?php endif;?>
                      </div>
                  </td>
          ...
          <?php endif;?>
          <?php if((in_array($this->app->user->account,explode(',',$deptObj->qa))) || ($this->app->user->account == 'admin') || in_array($this->app->user->account,explode(',',$project->maintenanceStaff))):?>
          <tr>
              <?php $today = date(DT_DATE1);?>
              <th><?php echo $lang->project->dateAllowCycle;?></th>
              <td colspan= <?php echo $this->app->user->account == 'admin' ? '3' :'';?>>
                  <div class='input-group '>
                      <?php echo html::input('allowBegin', $project->allowBegin !='0000-00-00' ? $project->allowBegin: '', "class='form-control form-date w-200px'  placeholder='" . $lang->project->allowBegin . "' ");?>
                 <!-- <span class='input-group-addon'><?php /*echo $lang->project->to;*/?></span>
                       --><?php
                  /*  echo html::input('allowEnd', $project->allowEnd !='0000-00-00' && (!empty($project->allowEnd)) ? $project->allowEnd : '', "class='form-control form-date w-200px'   placeholder='" . $lang->project->allowEnd . "' ");
                       */?>
                      <?php if($this->app->user->account == 'admin'):?>
                      <span class='input-group-addon'><?php echo $lang->project->maintenanceStaff;?></span>
                      <?php
                      echo html::select('maintenanceStaff[]', $users, '',"class='form-control chosen w-200px' multiple");
                      ?>
                      <?php endif;?>
                  </div>
              </td>
        ...
        <tr>
          ...
          <tr>
              <th><?php echo $lang->project->isSafetyTest;?></th>
              <td><?php echo html::select('isSafetyTest', $lang->project->isSafetyTestList, $project->isSafetyTest, 'class="form-control chosen"');?></td>
              <td colspan='2'>
                  <div class='input-group'>
                      <span class='input-group-addon'><?php echo $lang->project->isPerformanceTest;?></span>
                      <?php echo html::select('isPerformanceTest', $lang->project->isPerformanceTestList, $project->isPerformanceTest, 'class="form-control chosen"');?>
                  </div>
              </td>
          </tr>
...
<?php
js::set('maintenanceStaff',$project->maintenanceStaff ? $project->maintenanceStaff :'');
js::set('bearDept',$projectPlan->bearDept ? $projectPlan->bearDept :'');
?>
<script>
 $(function()
    {
        $('input[name="allowEnd"]').attr('disabled','true');
        /*时间*/
        $('input[name="allowEnd"]').datetimepicker( 'setStartDate', '<?php echo date(DT_DATE1)?>');
        /*结束时间*/
        $('input[name="allowEnd"]').datetimepicker( 'setEndDate', '<?php echo date(DT_DATE1)?>');
        /*开始时间*/
        $('input[name="allowBegin"]').datetimepicker( 'setEndDate', '<?php echo date(DT_DATE1)?>');
    })
    $('#allowBegin').change(function()
    {
        var allowBegin = $('input[name="allowBegin"]').val();
        if(allowBegin != ''){
            $('input[name="allowEnd"]').val('<?php echo date(DT_DATE1)?>');
        }else{
            $('input[name="allowEnd"]').val('');
        }
    });
</script>
