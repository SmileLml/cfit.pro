...
?>
<?php
$pathInfo = '&root=' . $this->projectdoc->encodePath(empty($path) ? '/' : $path);
...
<form id='logForm' class='main-table' data-ride='table' method='post'>
  <table class='table table-fixed'>
    <thead>
      <tr>
        <th class='w-40px'></th>
        <th class='w-80px'><?php echo $lang->projectdoc->revisionA?></th>
        <th class='w-80px'><?php echo $lang->projectdoc->time?></th>
        <th class='w-100px'><?php echo $lang->projectdoc->committer?></th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class='versions'><span class="revision"><?php echo html::a($this->projectdoc->createLink('revision', "projectID=$projectID&libID=$libID&revision={$log->revision}" . $pathInfo), $log->revision, '', "data-app='{$this->app->openApp}'");?></span></td>
  ...
  <div class='table-footer'>
    <?php if(common::hasPriv('doclib', 'diff')) echo html::submitButton($lang->projectdoc->diff, '', count($revisions) < 2 ? 'disabled btn btn-primary' : 'btn btn-primary')?>
    <?php //echo html::a($this->projectdoc->createLink('log', "libID=$libID&entry=" . $this->projectdoc->encodePath($path) . "&revision=HEAD&type=$logType"), $lang->projectdoc->allLog, '', "class='allLogs'");?>
    <div class='pull-right'>
      <div class='btn-group'>
        <?php
        $prePage  = $pager->pageID == 1 ? 1 : $pager->pageID - 1;
        $nextPage = $pager->pageID == $pager->pageTotal ? $pager->pageID : $pager->pageID + 1;
        $params   = "libID=$libID&path=" . $this->projectdoc->encodePath($path) . "&type=$logType&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}";
        $preLink  = $this->projectdoc->createLink('ajaxSideCommits', "$params&pageID=$prePage");
        $nextLink = $this->projectdoc->createLink('ajaxSideCommits', "$params&pageID=$nextPage");
