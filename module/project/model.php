...
class projectModel extends model
{
    ...
    public function accessDenied()
    {
        ...
        die(js::locate(helper::createLink('project', 'browse')));
    }
    ...
    /**
     * Desc:①实现方式是二线实现时，所属项目只能选择二线的虚拟项目
     *      ②实现方式是项目实现时，所属项目是非二线项目
     *      ③默认获取全部
     *
     * Date: 2022/3/21
     * Time: 16:40
     ...
     *
     */
    public function getProjects($secondLine = 'project')
    {
        return $this->dao->select('id,name')->from(TABLE_PROJECT)
            ->where('deleted')->eq(0)
            ->beginIF($secondLine == 'project')->andWhere('type')->eq('project')->fi()
            ->beginIF($secondLine == 'second')->andWhere('type')->eq('stage')->fi()
            ->orderBy('id_desc')
            ->fetchPairs();
    }
    ...
    public function getByIDNoType($projectID,$field='*')
    {
        $project = $this->dao->select($field)->from(TABLE_PROJECT)->where('id')->eq($projectID)->fetch();
        if(!$project) return false;

        return $project;
    }

    public function getInIDs($projectIDs)
    {
        $projects = $this->dao->select('*')->from(TABLE_PROJECT)->where('id')->in($projectIDs)->andWhere('deleted')->ne(1)->fetchAll();
        return $projects;
    }
...
//            ->fetchAll('project');
        ...
        $estimates = $this->dao->select('project, sum(consumed) as estimate')->from(TABLE_EFFORT)
            ->where('project')->in($projectIdList)
        ...
        foreach($projects as $projectID => $project)
        {
            ...
            $project->manMonth   =  round($project->estimate/(8 * $project->workHours) , 2);
    ...
    public function getWorkhour($projectID)
    {
        ...
        $workhour->totalHours    = $this->dao->select('sum(days * hours) AS totalHours')->from(TABLE_TEAM)->where('root')->eq($projectID)->andWhere('type')->eq('project')->fetch('totalHours');
        $workhour->totalEstimate = $total->totalEstimate;
        $workhour->totalConsumed = $total->totalConsumed;
    ...
    public function update($projectID = 0)
    {
        ...
        if(isset($project->allowBegin) && $project->allowBegin == '') {
            $project->allowEnd = '';
        }
        if(isset($project->allowBegin) && $project->allowBegin != ''){
            $project->allowEnd = helper::today();
        }
        $this->lang->project->workHours = "人月工时计算参数";
        $this->dao->update(TABLE_PROJECT)->data($project)
            ...
            ->checkIF(isset($project->allowBegin) && $project->allowBegin != '', 'allowBegin', 'date')
          /*  ->checkIF(isset($project->allowEnd) && $project->allowEnd != '', 'allowEnd', 'date')
            ->checkIF(isset($project->allowEnd) && $project->allowEnd != '', 'allowEnd', 'gt', isset($project->allowBegin) ? $project->allowBegin :'')
            ->checkIF(isset($project->allowBegin) && $project->allowBegin != '', 'allowEnd', 'notempty')
            ->checkIF(isset($project->allowEnd) && $project->allowEnd != '', 'allowBegin', 'notempty')*/
            ->check('name', 'unique', "id != $projectID and deleted='0' and `type`='project'")
            ->where('id')->eq($projectID)
            ->exec();

        //项目管理，在设置里面修改项目名称时，需要联动将年度计划里面的名称也修改
        if($oldProject->name != $project->name)
        {
            $this->dao->update(TABLE_PROJECTPLAN)->set('name')->eq($project->name)->where('project')->eq($projectID)->exec();
            //更新projectcreation 表name
            $projectplanInfo = $this->loadModel('projectplan')->getPlanMainInfoByProjectID($projectID, $select = 'id,project');
            if($projectplanInfo){
                $planId = $projectplanInfo->id;
                $this->dao->update(TABLE_PROJECTCREATION)->set('name')->eq($project->name)->where('plan')->eq($planId)->exec();
            }
        }
        ...
        if(!dao::isError())
        {
            ...
            //保存计划工时的差异
            if(empty($oldProject->planWorkload)){
                ...
                if(empty($creation->workload)){
                    $oldProject->planWorkload = '';
                }else{
                    $oldProject->planWorkload = $creation->workload;
                }
            }
    ...
    public function suspend($projectID)
    {
        ...
        if(!dao::isError()){
            //增加年度计划内部项目状态联动
            $projectplan = $this->loadModel("projectplan")->getPlanByProjectID($projectID,"id,oldInsideStatus,insideStatus");
            if($projectplan){
                $updata = [
                ...
                $this->dao->update(TABLE_PROJECTPLAN)->data($updata)->where("id")->eq($projectplan->id)->exec();
            }
    ...
    public function close($projectID)
    {
        ...
        {
            // 项目关闭时调用人力接口
            $this->sendProjectInfoApi($oldProject);
    ...
    // 向人力接口发送项目信息
    public function sendProjectInfoApi($projectInfo){
        $result = '';
        if($this->config->global->pushProjectInfoEnable == 'enable'){
            $users         = $this->dao->select('`account`, `realname`')->from(TABLE_USER)->fetchPairs();
            $url           = $this->config->global->pushProjectInfoUrl;
            $Members       = $this->loadModel('project')->getTeamMembers($projectInfo->id);//查询项目关联用户
            $ProjectNo     = $this->dao->select('code')->from(TABLE_PROJECTPLAN)->where('project')->eq($projectInfo->id)->fetch('code');
            foreach($Members as $member){
                $roleArr = explode(',',$member->role);
                if(in_array('1', $roleArr)){ // 1为项目主管
                    $ProtyRoles = $this->lang->project->projectLeader;
                }elseif(in_array('2', $roleArr)){ // 2为项目经理
                    $ProtyRoles = $this->lang->project->projectManger;
                }else{ // 其余为项目团队成员
                    $ProtyRoles = $this->lang->project->projectMember;
                }
                $pushData[] = array(
                    'EmpName'        => $member->account,
                    'Title'          => zget($users, $member->account),
                    'ProjectName'    => $projectInfo->name,
                    'ProjectNo'      => $ProjectNo,
                    'ProBeginDate'   => $projectInfo->realBegan,
                    'ProEndtime'     => $projectInfo->realEnd,
                    'ProtyRoles'     => $ProtyRoles,
                );
            }

            $headers = array();
            $headers[] = 'App-Id: ' . $this->config->global->pushProjectInfoAppId;
            $headers[] = 'App-Secret: ' . $this->config->global->pushProjectInfoAppSecret;
            ...
            $this->requestlog->saveRequestLog($url, "project", $this->lang->project->sendProjectInfoApi, 'POST', $pushData, $result, $status, '');
            return $pushStatus;
        }
        return $result;
    }
    ...
    public function manageMembers($projectID)
    {
        ...
        //a($accounts);a($roles);die();
        foreach($roles as $key => $role)
        {
            if(!empty($accounts[$key]) && sizeof($roles[$key]) == 1)
            {
                return dao::$errors[] = $this->lang->project->rolesEmpty;
            }
        }
    ...
    {
        if(!$projectID){
            $this->loadModel('action')->create('project', $projectID, 'Managed', "维护产品时获取项目编号为 :{$projectID}，维护失败！".'<br>'.'产品编号：'.implode(',',$products));
            return;
        }
        $this->loadModel('user');
        ...
        foreach($products as $i => $productID)
        {
            ...
            $data->plan    = (isset($plans[$productID]) && $plans[$productID]) ? $plans[$productID] : $oldPlan; // 20220627增加判断
            $data->createdTime = helper::now();
            $this->dao->insert(TABLE_PROJECTPRODUCT)->data($data)->exec();
            $existedProducts[$productID] = true;

            /*处理关联文字信息*/
            $relation['productIds'][] = $productID;
            $relation['planIds'][$productID] = $data->plan;
        }
        ...
        //20220627 增加创建年度计划 更新用户视图
        if($projectID) $this->user->updateUserView($projectID, 'project', $members);
    }
   ...
   /**
     ...
     * @return void
     */
    public function updateBindProducts($projectID, $products = '')
    {
        if(!$projectID){
            $this->loadModel('action')->create('project', $projectID, 'Managed', "维护产品时获取项目编号为 :{$projectID}，维护失败！".'<br>'.'产品编号：'.implode(',',$products));
            return;
    ...
    // 获取所有项目空间下-所有管理评审-创建时间最晚的时间
    public function getClosingTimeAll($projectIds)
    {
        return $this->dao->select("project, left(max(createdDate),10) as proposedClosingTime, left(max(meetingRealTime),10) as closingMeetingTime")->from(TABLE_REVIEW)
            ->where('project')->in($projectIds)
            ->andWhere('type')->eq('manage')
            ->andWhere('deleted')->eq('0')
            ->groupBy('project')
            ->fetchAll('project');
    }
    ...
    // 获取所有项目成员
    public function getTeamMembersAll($projectIds)
    {
        return $this->dao->select("t1.root,group_concat(t1.account) as members")->from(TABLE_TEAM)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')->on('t1.account = t2.account')
            ->where('t1.root')->in($projectIds)
            ->andWhere('t2.deleted')->eq('0')
            ->groupBy('t1.root')
            ->fetchAll('root');
    }
    ...
    public function setMenu($objectID, $params = array())
    {
        ...
        common::setMenuVars('project', $objectID, $params);
    }
     ...
     * @return array
     */
    public function getProjectRelations($projectId)
    {
        $obj = $this->dao->select('*')->from(TABLE_PROJECTPLANRELATION)->where('projectId')->eq($projectId)->fetch();
        $relation = empty($obj->planRelation) ? [] :json_decode($obj->planRelation, 1);
        $list = [];
        foreach ($relation as  $item){
            ...
            $item['os']         = $item['os'] ?? "";
            $item['arch']       = $item['arch'] ?? "";
            $list[] = $item['code'] . '-' . $item['planTitle'] . '-For-'. $item['os'] . '-'. $item['arch'];
        }
        return $list;
    }
    /**
     ...
     * @return array
     */
    public function getProjectRelationList($projectId)
    {
        $obj = $this->dao->select('*')->from(TABLE_PROJECTPLANRELATION)->where('projectId')->eq($projectId)->fetch();
        $list =  empty($obj) ? [] : json_decode($obj->planRelation, 1) ;
        return $list;
    }
    /**
     * 添加项目的产品计划
     ...
     * @return array|void
     */
    public function getProjectListByIds($projectIds, $select = '*'){
        $data = [];
        if(!$projectIds){
            return $data;
        }
    ...
    public function getExecutionByAvailable($projectID, $orderBy = 'begin_asc,id_asc')
    {
        ...
        foreach($executions as $execution)
        {
            if(isset($execution->children))
            {
                ...
                //过滤二线工单
                if(strstr($execution->name,$gd) !== false){
                   continue;
                }*/
     ...
     * @return mixed
     */
    public function getCodeNamePairs()
    {
        return $this->dao->select("id,CONCAT(code,'（',name,'）')")->from(TABLE_PROJECT)
            ->where('deleted')->eq(0)
            ->andWhere('type')->eq('project')
            ->orderBy('id_desc')
            ->fetchPairs();
     ...
     * @return mixed
     */
    public function getProjectOneStage($project = null,$type = null){
     ...
     * @return mixed
     */
    public function getProjectTwoStage($parent = null){

        $res =   $this->dao->select("id,name")->from(TABLE_EXECUTION)
            ->where('deleted')->eq(0)
            ->andWhere('type')->eq('stage')
            ->andWhere('grade')->eq('2')
    ...
    public function printCellDefect($col, $defect, $users, $mode = 'datatable', $projects = [], $products = [])
    {
        ...
        if($col->show)
        {
            ...
            {
                case 'id':
                    echo common::hasPriv('defect', 'view') ? html::a(helper::createLink('defect', 'view', "defectID=$defect->id", 'html#app=project'),$defect->code) : $defect->code;
                    break;
                case 'title':
                    echo $defect->title;
                    break;
     ...
     * @return void
     */
    public function printCellTesttask($col, $task, $users, $mode = 'datatable', $products = [])
    {
        $id = $col->id;
        if($col->show)
        {
            ...
            switch($id)
            {
                ...
                case 'oddNumber':
                    ...
                    break;
                case 'name':
                    echo html::a(helper::createLink('testtask', 'cases', "taskID=$task->id"), $task->name,null, "data-app='{$this->app->openApp}' title='{$task->name}'");
                    break;
                case 'product':
                    $productName = zget($products, $task->applicationID . '-' . $task->product, '');
                    echo $productName;
                    break;
                case 'build':
                    echo $buildData;
                ...
                case 'end':
                    ...
                    break;
                case 'progress':
                    $progress = $this->loadModel('testtask')->processProgress($task->id);
                    echo $progress;
                    break;
                case'status':
                    $status = $this->processStatus('testtask', $task);
                    echo  "<span class='status-testtask status-$task->status'>$status</span>";
                    break;

                case 'actions':
                    echo '<div id="action-divider">';
                    common::printIcon('bug', 'browse', "projectID=testtask{$task->id}", $task, 'list', 'bug');
                    echo '</div>';
                    common::printIcon('testtask', 'cases', "taskID=$task->id", $task, 'list', 'sitemap');
                    common::printIcon('testtask', 'linkCase', "taskID=$task->id", $task, 'list', 'link');
                    if(common::hasPriv('execution', 'testreport'))
                    {
                        echo html::a(helper::createLink('testreport', 'browse', "applicationID=$task->applicationID&objectID=$task->product&objectType=product&extra=$task->id"), '<i class="icon-testreport-browse icon-flag"></i>', '', 'class="btn " title="' . $this->lang->testreport->browse . '" data-app="project"');
                    }
     ...
     * @return void
     */
    public function printCellTestsuite($col, $suite, $users, $products)
    {
        $viewLink = helper::createLink('testsuite', 'view', "suiteID=$suite->id");
        ...
        if($col->show)
        {
            ...
            switch($id)
            {
                ...
                case 'name':
                    ...
                    echo html::a($viewLink, $suite->name, null, "data-app='{$this->app->openApp}'");
                    break;
                case 'product':
                    echo zget($products, $suite->applicationID . '-' . $suite->product, '');
                    break;
                case 'desc':
    ...
    */
    ...
    public function getPairsCodeName()
    {
        return $this->dao->select('id,IF(code = "",name,CONCAT(code,"_",name))')->from(TABLE_PROJECT)
            ->where('deleted')->eq(0)
            ->andWhere('type')->eq('project')
            ->orderBy('id_desc')
            ->fetchPairs();
     ...
     * @return bool
     */
    public function getProjectInfoById($id, $select = '*'){
        if(!$id){
            return false;
        }
        $ret = $this->dao->select($select)
     ...
     * @return bool
     */
    public function updateProjectInfo($projectId){
        $postData = fixer::input('post')->get();
        $postDataArray = (array) $postData;
        $fields = implode(',', array_keys($postDataArray));
        $projectInfo = $this->getProjectInfoById($projectId, $fields);

        //更新信息
        $updateParams = new stdClass();
        foreach ($postDataArray as $key => $value){
            if($value != $projectInfo->$key){
                $updateParams->$key = $value;
            }
        }
        if(empty( (array) $updateParams)){
            dao::$errors[''] = $this->lang->project->noChangeError;
            return false;
        }
        $ret = $this->updateProject($projectId, $postData);
        return $ret;
    }
     ...
     * @return bool
     */
    public function updateProject($projectId, $updateParams, $comment = ''){
        if(!($projectId && $updateParams)){
            return false;
        }
        $fields = implode(',', array_keys((array) $updateParams));
        $projectInfo = $this->getProjectInfoById($projectId, $fields);
        $this->dao->update(TABLE_PROJECT)
            ...
            ->exec();
        if(!dao::isError()){
           $changes =  common::createChanges($projectInfo, $updateParams);
           if($changes){
               $actionID = $this->loadModel('action')->create('project', $projectId, 'edited', $comment);
               $this->action->logHistory($actionID, $changes);
           }
        }
        return true;
    ...
    /**
     * Get product list of a project.
     *
     * @param  int    $projectID
     * @access public
     * @return array
     */
    public function getProductList($projectID){
        $data = [];
        if(!$projectID){
            return $data;
        }
        $ret = $this->dao->select('t2.id, t2.name')->from(TABLE_PROJECTPRODUCT)->alias('t1')
            ->leftJoin(TABLE_PRODUCT)->alias('t2')->on('t1.product = t2.id')
            ->where('t1.project')->eq((int)$projectID)
            ->andWhere('t2.deleted')->eq(0)
            ->fetchPairs('id', 'name');
     ...
     * @return array
     */
    public function getProjectTeamUsers($projectID, $exWhere = ''){
        $data = [];
        if(!$projectID){
            return $data;
        }
        $ret = $this->dao->select('account')
            ->from(TABLE_TEAM)
     ...
     * @return string
     */
    public function getProjectDeptManagerUsers($projectID){
        $managerUsers = '';
        if(!$projectID){
            return $managerUsers;
        }
        ...
        $deptInfo = $this->loadModel('dept')->getFieldByDeptId('manager1', $bearDept);
        $managerUsers = $deptInfo->manager1;
        return $managerUsers;
    }
     ...
     * @return int
     */
    public function getProjectBearDept($projectID){
        $bearDept = 0;
        $projectPlanInfo = $this->loadModel('projectplan')->getPlanMainInfoByProjectID($projectID, 'bearDept');
        if($projectPlanInfo){
            $bearDept = $projectPlanInfo->bearDept;
