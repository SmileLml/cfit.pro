...
<div id='mainMenu' class='clearfix'>
  <div class='btn-toolbar pull-left'>
    <?php include '../../process/view/menu.html.php';?>
    <a class="btn btn-link querybox-toggle" id='bysearchTab'><i class="icon icon-search muted"></i> <?php echo $lang->activity->byQuery;?></a>
  </div>
  <div class='btn-toolbar pull-right'>
    <?php common::printLink('activity', 'batchCreate', "", "<i class='icon icon-plus'></i>" . $lang->activity->batchCreate, '', "class='btn btn-secondary'");?>
  ...
  <div class="main-col">
    ...
    <form id='activityForm' data-ride="table" method='post' class="main-table">
        ...
        <?php $vars = "&browseType=$browseType&param=$param&orderBy=%s&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}";?>
          <thead>
            <tr>
              ...
              <th class="text-left w-180px">   <?php common::printOrderLink('process',     $orderBy, $vars, $lang->activity->process);?></th>
              <th class="text-left w-180px">   <?php common::printOrderLink('name',        $orderBy, $vars, $lang->activity->name);?></th>
              <th class="text-left w-80px">    <?php common::printOrderLink('optional',    $orderBy, $vars, $lang->activity->optional);?></th>
              <th class="c-assignedTo w-120px"><?php common::printOrderLink('assignedTo',  $orderBy, $vars, $lang->activity->assignedTo);?></th>
              <th class="text-left w-120px">   <?php common::printOrderLink('createdBy',   $orderBy, $vars, $lang->activity->createdBy);?></th>
              <th class="text-left w-100px">   <?php common::printOrderLink('createdDate', $orderBy, $vars, $lang->activity->createdDate);?></th>
              <th class="text-center w-150px"> <?php echo $lang->activity->actions;?></th>
              <th class="sort-default text-left w-60px"><?php echo $lang->activity->sort;?></th>
            </tr>
          </thead>
          <tbody class="sortable" id="orderTableList" style="position: static;">
            ...
            <tr data-id=<?php echo $activity->id;?> data-order=<?php echo $activity->order;?>>
              <td calss="c-id" title="<?php echo $activity->id;?>">
                <?php echo html::checkbox('dataIDList[]', array($activity->id => ''));?>
                <?php echo html::a($this->createLink('activity', 'view', "activityID={$activity->id}"), $activity->id);?>
              </td>
              <td class="c-process">    <?php echo zget($processes, $activity->process, '');?></td>
              ...
              <td class="c-optional">   <?php echo zget($lang->activity->optionalOptions, $activity->optional, '');?></td>
              <td class="c-assignedTo"> <?php echo $this->activity->printAssignedHtml($activity, $users);?></td>
              <td class="c-createdBy">  <?php echo $activity->createdBy;?></td>
              <td class="c-createdDate"><?php echo substr($activity->createdDate, 0, 11);?></td>
              <td class='c-actions text-center'>
                <?php
                    common::printIcon('zoutput',  'batchCreate', '',                           $activity, 'list', 'treemap-alt', '',          '',       '',    '', $lang->activity->output);
      ...
      <div class='table-footer'>
        <div class="checkbox-primary check-all"><label><?php echo $lang->activity->selectAll;?></label></div>
