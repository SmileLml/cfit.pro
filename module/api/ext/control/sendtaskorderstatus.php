...
class myApi extends api
{
    ...
    public function sendTaskOrderStatus()
    {
        $logID = $this->loadModel('requestlog')->insideSaveRequestLog('secondorder' , 'sendTaskOrderStatus');
        $this->checkApiToken();
        ...
        if(!empty($secondorder->id)){
            ...
            if($this->post->taskListStatus == 'pass'){
                ...
                if(
                    (($secondorder->type == 'consult' && 'sectransfer' == $secondorder->handoverMethod) || $secondorder->type != 'consult') &&
                     $secondorder->type != 'support' &&
                    (($secondorder->type == 'other' && 'sectransfer' == $secondorder->handoverMethod) || $secondorder->type != 'other')
                ){
                //if($secondorder->type != 'consult' and $secondorder->type != 'support'){
                    ...
                    $this->loadModel('consumed')->record('secondorder', $secondorderID, 0, 'guestcn', $secondorder->status, 'indelivery', array(), '');
                    //查询对外移交关联
                    ...
                    if($next)
                    {
                        ...
                        $this->dao->update(TABLE_REVIEWER)->set('status')->eq('reject')->set('comment')->eq($this->post->rejectReason)->set('reviewTime')->eq(helper::now())->where('node')->eq($next)->exec();
                    }
                    ...
                    $this->loadModel('consumed')->record('sectransfer', $sectransfer->id, 0, 'guestcn', $sectransfer->status, $sectransferData->status, array());
                ...
                if($secondorder->type != 'consult' and $secondorder->type != 'support'){
                    ...
                    if($next)
                    {
                        ...
                        $this->dao->update(TABLE_REVIEWER)->set('status')->eq('pass')->set('comment')->eq($updateData->rejectReason)->set('reviewTime')->eq(helper::now())->where('node')->eq($next)->exec();
                    }
                    $sectransferData = new stdClass();
