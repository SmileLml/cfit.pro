...
function submitData() {
    ...
    if (parseInt(responseid) >0 ){
        //编辑
        url = createLink('outwarddelivery', 'edit', 'id='+responseid+'&source=copy')
    }else{
        //添加
        url = createLink('outwarddelivery', 'reissue', 'id='+id)
    }
    var abnormalId = $("#abnormalCode option:selected").val()
    $("#abnormalCode").removeAttr('disabled');
    if (abnormalId != ''){
    ...
    $.post(url,formdata,function (response) {
        ...
        if (response.result == 'fail'){
            ...
            //     $("#abnormalCode").attr('disabled','true');
            ...
            $.each(response.message, function(key, value)
            {
                ...
                else
                {
                    $.each(value, function(subKey, subValue)
                    {
                        ...
                        alertMSG = subKey != value.length - 1 ? subValue.replace(/[\。|\.]/, ';') : subValue;
                    })
                }
