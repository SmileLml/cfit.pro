...
<?php $browseLink = $app->session->caseList ? $app->session->caseList : $this->createLink('testcase', 'browse', "applicationID=$applicationID&productID=$case->product");?>
...
<div id='mainMenu' class='clearfix'>
    ...
    <div class="page-title">
      ...
      <span class='dropdown'>
        ...
        for($i = $case->version; $i >= 1; $i --)
        {
            ...
            echo '<li' . $class .'>' . html::a(inlink('view', "caseID=$case->id&version=$i"), '#' . $i, '_self', "data-app='{$this->app->openApp}'") . '</li>';
        }
        ?>
        </ul>
      </span>
  ...
  <div class='btn-toolbar pull-right'>
    ...
    <?php common::printLink('testcase', 'create', "applicationID=$applicationID&productID={$case->product}&branch={$case->branch}&moduleID={$case->module}", "<i class='icon icon-plus'></i>" . $lang->testcase->create, '', "class='btn btn-primary'"); ?>
  </div>
  <?php endif;?>
</div>
...
<div id="mainContent" class="main-row">
  <div class='main-col col-8'>
    <div class='cell' style='word-break:break-all'>
      ...
      <div class='detail'>
        <div class='detail-title'><?php echo $lang->testcase->intro;?></div>
        <div class="detail-content">
        <?php echo nl2br(!empty($case->intro)?$case->intro:'无');?>
        </div>
      </div>
    ...
    <div class='main-actions'>
      <div class="btn-toolbar">
        <?php echo html::a($browseLink, '<i class="icon-goback icon-back"></i> ' . $lang->goback, '', "id='back' class='btn' title='{$lang->goback}{$lang->backShortcutKey}' data-app='{$this->app->openApp}'");?>
        <?php if(!$case->deleted):?>
        ...
        if(!$isLibCase)
        {
            ...
            if($caseFails > 0) common::printIcon('testcase', 'createBug', "applicationID=$case->applicationID&product=$case->product&branch=$case->branch&extra=caseID=$case->id,version=$case->version,runID=$runID", $case, 'button', 'bug', '', 'iframe', '', "data-width='90%'");
        }
        ...
        if(!$isLibCase and $case->auto != 'unit') common::printIcon('testcase', 'create', "applicationID=$case->applicationID&productID=$case->product&branch=$case->branch&moduleID=$case->module&from=testcase&param=$case->id", $case, 'button', 'copy');
      ...
      <details class="detail" open>
        ...
        <div class="detail-content">
          <table class='table table-data'>
            ...
            <?php else:?>
            <tr>
              <th class='thWidth'><?php echo $lang->testcase->project;?></th>
              <td><?php if(!empty($projectPlan)) common::printLinks('projectplan', 'view', "id={$projectPlan->id}", $projectPlan->name, '', 'data-app="platform"');?></td>
            </tr>
            <tr valign='middle'>
              <th class='thWidth'><?php echo $lang->testcase->applicationID;?></th>
              <td>
              <?php $applicationName = zget($applicationList, $case->applicationID, '');
              common::printLinks('application', 'view', "applicationID={$case->applicationID}", $applicationName, '', 'data-app="application"');
              ?>
              </td>
            </tr>
            <tr>
              <th><?php echo $lang->testcase->product;?></th>
              <td><?php common::printLinks('product', 'view', "productID={$case->product}", $product->name, '', 'data-app="product"');?></td>
            </tr>
            <tr>
              <th><?php echo $lang->testcase->execution;?></th>
              <td><?php if(!empty($execution)) common::printLinks('execution', 'view', "id={$execution->id}", $execution->name, '', 'class="iframe"', true, true);?></td>
            </tr>
            <?php endif;?>
            <tr>
                ...
                else
                {
                    foreach($modulePath as $key => $module)
                    {
                        if($this->app->openApp == 'qa' || $this->app->openApp == 'ops')
                        {
                            ...
                            {
                                if(!common::printLink('testcase', 'browse', "applicationID=$applicationID&productID=$case->product&branch=$module->branch&browseType=byModule&param=$module->id", $module->name)) echo $module->name;
                            }
                        }
                        else if($this->app->openApp == 'project')
                        {

                            if(!common::printLink('project', 'testcase', "project=$case->project&applicationID=$case->applicationID&productID=$case->product&branch=$module->branch&browseType=byModule&param=$module->id", $module->name)) echo $module->name;
                        } 
            ...
            <?php endif;?>
            <tr>
              <th><?php echo $lang->testcase->categories;?></th>
              <td><?php echo $this->testcase->getCategoriesValueByKeys($case->categories);?></td>
            </tr>
          </table>
        </div>
      </details>
