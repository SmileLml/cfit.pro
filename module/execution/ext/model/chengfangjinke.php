...
public function getTaskExecutionName($executionID)
{
    $execution = $this->dao->select('name')->from(TABLE_EXECUTION)->where('id')->eq($executionID)->fetch();
    return isset($execution->name) ? $execution->name : '';
}
