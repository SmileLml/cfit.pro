...
class storyModel extends model
{
    ...
    function batchReview($storyIdList, $result, $reason)
    {
        ...
        foreach($storyIdList as $storyID)
        {
            ...
            //20200420 如果是平台需求收集同步过来的需求，返填评审结果到平台需求收集
            if($oldStory->source == 'tb' )
            {
                $demandcollection = new stdClass();
                $demandcollection->feedbackResult = zget($this->lang->story->reviewResultList,$result);
                if($reason){
                    $demandcollection->feedbackResult = $demandcollection->feedbackResult . ':' . zget($this->lang->story->reasonList,$reason);
                }
                $demandcollection->feedbackDate = helper::now();

                $this->dao->update(TABLE_DEMANDCOLLECTION)->data($demandcollection)->where('id')->eq($oldStory->sourceNote)->exec();
            }
        }
    ...
    public function batchChangePlan($storyIdList, $planID, $oldPlanID = 0)
    {
        ...
        foreach($storyIdList as $storyID)
        {
            ...
            //0420如果是平台需求收集同步过来的需求，返填结果到平台需求收集
            if($oldStory->source == 'tb')
            {
                $demandcollection = new stdClass();
                $demandcollection->Actual = $plan->id;
                $demandcollection->scheduledDate = helper::now();
                $this->dao->update(TABLE_DEMANDCOLLECTION)->data($demandcollection)->where('id')->eq($oldStory->sourceNote)->exec();
            }
    ...
    public function batchChangeStage($storyIdList, $stage)
    {
        ...
        foreach($storyIdList as $storyID)
        {
            ...
            //0420 如果是平台需求收集同步过来的需求，返填结果到平台需求收集
            if($oldStory->source == 'tb')
            {
                $demandcollection = new stdClass();
                $demandcollection->developstate = zget($this->lang->story->stageList,$stage);
                $this->dao->update(TABLE_DEMANDCOLLECTION)->data($demandcollection)->where('id')->eq($oldStory->sourceNote)->exec();    
            }
            $this->dao->update(TABLE_STORY)->data($story)->autoCheck()->where('id')->eq((int)$storyID)->exec();
    ...
    public function batchAssignTo()
    {
        ...
        foreach($storyIdList as $storyID)
        {
            ...
            {
                $demandcollection = new stdClass();
                $demandcollection->Developer = $assignedTo;
                $this->dao->update(TABLE_DEMANDCOLLECTION)->data($demandcollection)->where('id')->eq($oldStory->sourceNote)->exec();    
            }
            $story = new stdclass();
            $story->lastEditedBy   = $this->app->user->account;
