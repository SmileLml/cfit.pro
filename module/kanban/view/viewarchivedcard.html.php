                ...
                <div class='card-item' data-card='<?php echo $card->id;?>'>
                    <div class='col-xs-10'>
                        ...
                        <div class="kanban-item <?php echo $nameColor;?> <?php echo $color;?>" data-id="<?php echo $card->id;?>">
                            <?php if(empty($card->fromType) || $card->fromType == 'demandcollection'):?>
                        <?php echo "<div class='cardName'>" . html::a($this->createLink('kanban', 'viewCard', "cardID=$card->id", '', true), $finishLabel . $card->name, '', "class='iframe' data-toggle='modal' data-width='80%' title='$card->name'") . '</div>';?>
                            <div class="info">
                                ...
                                <?php else:?>
                                    ...
                                    {
                                        $delayed = (!empty($card->delay) or (!helper::isZeroDate($card->end) and helper::now() > $card->end)) ? "<span class='delayed label label-danger label-badge'>{$lang->execution->delayed}</span>" : '';
                                    }
                                    if(common::hasPriv($card->fromType, 'view'))
//            echo "<div class='cardName {$card->fromType}Name'>" . html::a($this->createLink($card->fromType, 'view', "id=$card->fromID"), $name, '', " title='$name'") . "$delayed</div>";
                                        echo "<div class='cardName {$card->fromType}Name'>" . html::a('javascript:void(0);', $name, '', " title='$name' onclick=locateView('execution','$card->fromID')") . "$delayed</div>";
                                ...
                                <?php endif;?>
                                <?php if(!helper::isZeroDate($card->begin) and !helper::isZeroDate($card->end) and $card->begin != '2030-01-01' and $card->end != '2030-01-01' and $card->fromType != ''):?>
                                    <span class="time label label-light" data-str="<?php echo $card->fromType;?>"><?php echo date("m-d", strtotime($card->begin)) . ' ' . $lang->{$card->fromType}->to . ' ' . date("m-d", strtotime($card->end));?></span>
                                <?php endif;?>
                                ...
                                if($card->fromType == 'demandcollection')          $assignedToList = explode(',', $card->assignedTo);
                                $count          = 0;
                                $members        = '';
                                ?>
                            ...
                            <?php if($kanban->performable and ($card->fromType == 'execution' or empty($card->fromType))):?>
                                <div class='progress-box'>
                                    ...
                                    <?php if($card->progress>100) $card->progress = 100;?>
...
$(function()
{
    ...
    $(".parentiframe").each(function () {
        ...
        $(this).attr('onclick','parentIframe("'+_href+'")')
        $(this).removeClass('iframe')
    })

})
function parentIframe(url){
    var obj = $(window.parent.document).find('#agency');
    obj.attr("href",url);
    parent.$("#agency",parent.document).trigger('click');
}
</script>
