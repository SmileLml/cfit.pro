    ...
    <table class='table table-form table-fixed' style='margin-bottom:20px;border:1px solid #ddd;'>
      ...
      <tbody>
        ...
        <tr class='text-top'>
          ...
          <td><?php echo html::input("dates[$i]", $beginAndEnd->flag ? $beginAndEnd->end : $today, "class='form-control form-date' readonly='readonly'");?></td>
          ...
          <td>
          <?php
          echo html::hidden("objectType[$i]", 'task'); 
          echo html::hidden("objectID[$i]", $taskID); 
      ...
      <tfoot>
        <tr>
          <?php $colspan = $objectType == 'task' ? '4' : '3';?>
              ...
              <?php foreach($efforts as $effort):?>
                  <tr class='main'>
                      ...
                      <td align='center' class='actions'>
                          <?php
                          if(empty($task->team) or  $task->assignedTo == $this->app->user->account) common::printIcon('task', 'editEstimate', "effortID=$effort->id", '', 'list', 'edit', '', 'showinonlybody', true);
                          if(empty($task->team) or  $task->assignedTo == $this->app->user->account) common::printIcon('task', 'deleteEstimate', "effortID=$effort->id", '', 'list', 'trash', 'hiddenwin', 'showinonlybody');
                          ?>
                      </td>
                  </tr>
              <?php endforeach;?>
...
<?php js::set('projectType',$projectType->secondLine ? $projectType->secondLine :'')?>
