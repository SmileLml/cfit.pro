<?php
class projectplanshModel extends model
{
    static $_fields = '*';
    /**
     ...
     * @return mixed
     */
    public function getList($browseType, $queryID, $orderBy, $pager, $secondLine = 0, $modelName = 'projectplansh',$shanghaipart = 0)
    {
        $projectplanQuery = '';
                    ...
                    if(strpos($v, '`pending`')){
                        if(strpos($v, '%')){
                            ...
                            if(strpos($v,"NOT LIKE") != false){
                                ...
                                if(isset($queryArray[$k+1])){
                                    if(strpos($v,")") != false){
                                        $queryArray[$k] = " (`id` ". helper::dbIN($planIds)." OR (status in ( ".$this->lang->projectplansh->browseSearchAllowStatus.") AND changeStatus != 'pending' AND find_in_set('{$user}' ,owner)) ))";
        ...
        $shDept = array_filter(explode(',',$this->lang->projectplansh->shProjectPlanDeptList['shDeptList'])); //后台配置的可查看项目的上海部门
        $projectplans = $this->dao->select(self::$_fields)->from(TABLE_PROJECTPLAN)
            ->where(1)
            ->andWhere('deleted')->ne('1')
            ->beginIF($browseType == 'all' )->andWhere('secondLine')->eq($secondLine)->andWhere('bearDept')->in($shanghaiDeptList)->fi() //各tab，其他部门可以看到所有，包含上海
            ->beginIF($browseType !== 'all')->andWhere('secondLine')->eq($secondLine)->andWhere('bearDept')->in($shanghaiDeptList)->fi() //各tab上海只能看到上海的项目
            //迭代33透出  deleted状态数据
        ...
        $this->loadModel('common')->saveQueryCondition($this->dao->get(), 'projectplansh', $browseType != 'bysearch');
    ...
    {
        $this->config->projectplansh->search['actionURL'] = $actionURL;
        $this->config->projectplansh->search['queryID'] = $queryID;
        $this->config->projectplansh->search['params']['line']['values'] = array('' => '') + $this->loadModel('product')->getLinePairs();
        $this->config->projectplansh->search['params']['app']['values'] = array('' => '') + $this->loadModel('application')->getapplicationNameCodePairs();
        $this->config->projectplansh->search['params']['bearDept']['values'] = array('' => '') + $this->loadModel('dept')->getOptionMenu();
        $this->config->projectplansh->search['params']['type']['values'] = array('' => '') + $this->lang->projectplansh->typeList;

        $this->loadModel('search')->setSearchParams($this->config->projectplansh->search);
    }
...
//            ->remove('comment,productIds,realRelease,realOnline,stageBegin,stageEnd,uid,files,labels')
            ...
            ->stripTags($this->config->projectplansh->editor->create['id'], $this->config->allowedTags)
            ->get();
        ...
        $projectplan = $this->loadModel('file')->processImgURL($projectplan, $this->config->projectplansh->editor->create['id'], $this->post->uid);
        $this->dao->insert(TABLE_PROJECTPLAN)
            ->data($projectplan)->autoCheck()
            ->batchCheck($this->config->projectplansh->create->requiredFields, 'notempty')
            ->exec();
...
//            ->remove('productIds,realRelease,realOnline,stageBegin,stageEnd,uid,files,labels')
            ...
            ->stripTags($this->config->projectplansh->editor->edit['id'], $this->config->allowedTags)
            ->get();
        ...
        $plan = $this->loadModel('file')->processImgURL($plan, $this->config->projectplansh->editor->edit['id'], $this->post->uid);
        $this->dao->update(TABLE_PROJECTPLAN)->data($plan)->autoCheck()
            ->batchCheck($this->config->projectplansh->edit->requiredFields, 'notempty')
            ->where('id')->eq($planID)
            ->exec();
        //处理outsideplan表相关字段
...
//        $commoncreatechange = common::createChanges($oldPlan, $plan);
        $commoncreatechange = common::createRuleChanges($oldPlan, $plan,[],$this->config->projectplansh->changeFieldsRule);
...
//            ->remove('projectChange,comment,productIds,realRelease,realOnline,stageBegin,stageEnd,uid,files,labels')
            ...
            ->stripTags($this->config->projectplansh->editor->execedit['id'], $this->config->allowedTags)
            ->get();
        $plan->platformowner = trim($plan->platformowner,',');
        $plan->basis = trim($plan->basis,',');
        $plan->owner = trim($plan->owner,',');
...
//a($plan);exit();
        ...
        if (empty($_POST['app'])) {
            dao::$errors['app'] = $this->lang->projectplansh->appEmpty;
            return false;
        }
        if ($this->checkPost() == false) {
            return false;
        ...
        $plan = $this->loadModel('file')->processImgURL($plan, $this->config->projectplansh->editor->execedit['id'], $this->post->uid);
        ...
        $this->dao->update(TABLE_PROJECTPLAN)->data($plan)->autoCheck()
            ->batchCheck($this->config->projectplansh->execedit->requiredFields, 'notempty')
            ->where('id')->eq($planID)
            ->exec();
...
//        common::createChanges($oldPlan, $plan);
        $changeRes =  common::createRuleChanges($oldPlan, $plan,[],$this->config->projectplansh->changeFieldsRule);
        return $changeRes;
    }
...
//            $data = ['repeatID'=>$repaetID,'repeatName'=>$tempArr[$repaetID],'targetID'=>$targetID,'targetName'=>$taskList[$targetID]->subTaskName];
            $message = vsprintf($this->lang->projectplansh->repateTaskNameErrot,[$tempArr[$repaetID],$repaetID,$taskList[$repaetID]->subProjectName,$tempArr[$repaetID]]);
...
//            ->remove('projectChange,comment,productIds,realRelease,realOnline,stageBegin,stageEnd,uid,files,labels')
        ...
        if (empty($_POST['planRemark'])) {
            dao::$errors['planRemark'] = $this->lang->projectplansh->planRemarkDescNotEmpty;
            return false;
        }
        if ($this->checkPost() == false) {
            return false;
...
//        $change = common::createNewChanges($oldPlan, $plan);
        $change = common::createRuleChanges($oldPlan, $plan,[],$this->config->projectplansh->changeFieldsRule);
        ...
        $addNodeStage = 1;
        $planPerson = $this->isBuildDeptPerson($planID);
        if($planPerson){
            $panchangeNode = 2;
        }else{
            $panchangeNode = 1;
        ...
        $archDept = $shProductAndarchList['archDeptUser'];
        ...
        if(!empty($change)){
            ...
            if($number == 0){
                ...
                if($isChangeReview){
                    ...
                    //是架构部
                    if($planPerson){
                        $this->newAddNodeInfoAboutBuild($planID,$changeVersion,array($archDept),$addNodeStage,$addNodeStage,'planchange');
                    }else{
                        $param = ['nodeCode'=>$this->lang->projectplansh->changeNodeCode[1]];//部门负责人审核
    ...
    public function submit($planID)
    {
        try {
            if (empty($_POST['depts'])) {
                dao::$errors[] = $this->lang->projectplansh->deptEmpty;
                return;
            }
            $this->dao->begin();
            $myDept = $this->loadModel('dept')->getByID($this->app->user->dept);
            ...
            {
                dao::$errors[] = $this->lang->projectplansh->managerEmpty;
                $this->dao->rollBack();
                return;
            }
            if (!isset($myDept->leader1) or empty($myDept->leader1)) {
                dao::$errors[] = $this->lang->projectplansh->leaderEmpty;
                $this->dao->rollBack();
                return;
            }
            ...
            if(!$planCreation){
                dao::$errors[] = $this->lang->projectplansh->planCreationErrorEmpty;
                $this->dao->rollBack();
                return;
            }
            ...
            foreach ($this->post->depts as $deptID) {
                ...
                if (empty($dept->manager1)) {
                    dao::$errors[] = $dept->name . $this->lang->projectplansh->managerEmpty;
                    $this->dao->rollBack();
                    return;
                }
            ...
            if (empty($productDept->manager1)) {
                dao::$errors[] = $productDept->name . $this->lang->projectplansh->managerEmpty;
                $this->dao->rollBack();
                return;
            }
            $shProductAndarchList =  $this->lang->projectplan->shProductAndarchList ; //上海产创和架构自定义配置
...
//            ->join('demand', ',')
            ...
            if($issyncjob != 0 ){
                if(!isset($plan->opinion) || !isset($plan->requirement)){
                    dao::$errors[] = $this->lang->projectplansh->isSyncJob;
                    $this->dao->rollBack();
                    return false;
                }
...
//            $plan->project = $projectID;
            ...
            if (!dao::isError()) {
                 ...
                 */
                //看看是不是 产创/架构的
                $isFilterDept = in_array($this->app->user->dept,$this->lang->projectplansh->submitFilterDept);
                   ...
                   // if($planCreation->workload >= 100){
                        ...
                        if($myDept->leader1 == $reviewerceo){
                            $leader1nodeCode['reviewerExtParams'] = [
                                'status'=>'ignore',
                                'comment'=>$this->lang->projectplansh->submitignoreleader,
                    ...
                    //if($planCreation->workload >= 100){
                        ...
                        if($this->app->user->dept == 1){
                            // 产创部 自动跳过
                            $this->review->addNode('projectplan', $planID, $plan->version, array($productDept->manager1), true,'ignore',$stage++,['nodeCode'=>'productdeptleader','reviewerExtParams'=>['status'=>'ignore','comment'=>$this->lang->projectplansh->submitignoreprojectbearnotice]]);
                            ...
                            $this->review->addNode('projectplan', $planID, $plan->version, array($archDept->manager1), true,'ignore',$stage++,['nodeCode'=>'archdeptleader','reviewerExtParams'=>['status'=>'ignore','comment'=>$this->lang->projectplansh->submitignoreprojectbearnotice]]);
                        }else{
    ...
    public function yearReview($planID)
    {
        ...
        if (!dao::isError()) {
            if($status == 'yearstart' && $rejectStatus > 0){
                ...
                $planPerson = $this->isBuildDeptPerson($planID);
                if(in_array($rejectStatus,$this->lang->projectplansh->leaderStage)){
                    //平台架构部跳过负责人
                    if(!empty($planPerson)){
                        $plan->reviewStage = 4;
                ...
                }elseif(in_array($rejectStatus,$this->lang->projectplansh->jumpStage)){
                    //2,3节点退回，重新申请选择是否跳过
     ...
     * @return string
     */
    public function isBuildDeptPerson($planID)
    {
        $plan = $this->getByID($planID);
        /*$shanghaiDeptList = $this->loadModel('dept')->getAllChildId(30); //获取所有上海部门
        $userList = $this->loadModel('user')->getUserListGroupDeptId(trim(implode(',',$shanghaiDeptList)));
        //$userList = $this->loadModel('user')->getUserListByDeptId(2,'account');
        //架构部发起审批跳过负责人
        $deptOfBuild = [];
        foreach ($userList as $key => $item) {
            foreach ($item as $ke => $ite) {
                $deptOfBuild[] = $ke;
            }
        }*/
        $deptOfBuild = [];
        ...
        $archDept = $shProductAndarchList['archDeptUser'];
        $planPerson = '';
        $deptInfo = $this->loadModel('dept')->getByID($plan->bearDept);
        $person = $deptInfo->planPerson;
        if(in_array($this->app->user->account,array($person,$archDept))){
            $planPerson = $person;
        }
        return $planPerson;
    }
    ...
    {
        $nodeCodeLang = $this->lang->projectplansh->nodeCode;
    ...
    {
        if (!$this->post->result) {
            dao::$errors['result'] = $this->lang->projectplansh->resultEmpty;
            return false;
        }
        if ($this->post->result == 'reject' && !$this->post->comment) {
            dao::$errors['comment'] = $this->lang->projectplansh->commentEmpty;
            return false;
        }
        ...
        if(!$repeatVerify){
            dao::$errors[] = $this->lang->projectplansh->approvalEmpty;
            return false;
        }
        $extra = new stdClass();
        //会签节点 分配资源
...
//        $curPendingNode = $this->review->getFirstPendingNode('projectplan', $planID, $plan->version);
        ...
        if(!$curPendingNode){
            dao::$errors[] = $this->lang->projectplansh->submitnodeCodeEmptyError;
            return false;
        }

        //如果是通过状态 并且有资源分配的节点 验证资源是否勾选
        if ($this->post->result== 'pass' and in_array($curPendingNode->nodeCode, $this->lang->projectplansh->reviewinvolvedNode) and !$extra->involved) {
            dao::$errors[] = $this->lang->projectplansh->involvedEnpty;
            return false;
        }
        //如果是不通过状态 时 资源分配表单隐藏，如果此时选择了资源，则清除所选的资源分配，防止隐藏的表单值入库
        if ($this->post->result== 'reject' and in_array($curPendingNode->nodeCode, $this->lang->projectplansh->reviewinvolvedNode) and $extra->involved) {
        ...
        try{
            if($curPendingNode->nodeCode == 'deptLeader'){
                if(!$this->post->owner){
                    dao::$errors[] = $this->lang->projectplansh->ownerEmpty ;
                    $this->dao->rollBack();
                    return false;
                }
                //更新
            ...
            if ($result == 'pass') {
                ...
                if(!$pendingNode){
                    if($curPendingNode->nodeCode == 'deptLeader'){
                        ...
                        $productdeptleaderNode = $this->loadModel('review')->getNodeByNodeCode('projectplan', $planID, $plan->version, $this->lang->projectplansh->submitnodeCode['productdeptleader']);
                        $archdeptleaderNode = $this->loadModel('review')->getNodeByNodeCode('projectplan', $planID, $plan->version, $this->lang->projectplansh->submitnodeCode['archdeptleader']);
...
//            $this->dao->rollBack();
    ...
    public function yearReviewing($planID)
    {
       ...
       // a($plan->reviewStage );die;
        ...
        if (!$this->post->leaderOther && !$this->post->leader && $plan->reviewStage == 4) {
            dao::$errors['leaderOther'] = $this->lang->projectplansh->leaderOtherEmpty;
            return false;
        }
            ...
            if ($plan->reviewStage == 2) {
                ...
                if ($postReviewer) {
                    ...
                    if ($numberReviewer == 1 && empty($postReviewer[0])) {
                        dao::$errors['reviewer'] = $this->lang->projectplansh->architectEmpty;
                        return false;
                    }
                } else {
                    //开发环境判断
            ...
            switch ($plan->reviewStage) {
                ...
                case 1:
                    ...
                    if($plan->yearVersion > 0 && $plan->rejectStatus > 0){
                        $shProductAndarchList =  $this->lang->projectplan->shProductAndarchList ; //上海产创和架构自定义配置
                        $archDeptmanager1 = $shProductAndarchList['archDeptUser'];//架构部
                        if($plan->rejectStatus >= 4){
                            $reviewer = array($archDeptmanager1);
                            $reviewStage = 4;
                        }else{
                            //申请人部门负责人退回
                            $reviewer = array($myDept->planPerson);
                ...
                case 3:
                    ...
                    if (!$next) {
                        ...
                        $archDeptmanager1 = $shProductAndarchList['archDeptUser'];//架构部
                        $param = ['nodeCode'=>$nodeCodeLang[$addNodeStage]];
                        $this->loadModel('review')->addNode('projectplanyear', $planID, $plan->yearVersion, array($archDeptmanager1), true, 'pending',$plan->reviewStage,$param);
                    }
                    break;
                case 4:
                    //架构部负责人审批
    ...
    public function changeReview($planID)
    {
        ...
        if($plan->changeStage == 4 and $this->post->leader){
            ...
            if(count($postLeader) == 1 and $postLeader[0] == 'isBoss'){
                dao::$errors['leader'] = $this->lang->projectplansh->leaderCheckTips;
                return false;
            }
        }
...
//                    $this->loadModel('review')->check('planchange', $planID, $version, 'pass', $this->post->comment, $stage, $extra);
                ...
                case 4:
                    $leader = $post->leader ?? [];
                    if(!empty($leader)){
                        $reviewer = array('maoting');//array('hetielin');
    ...
    {
        $nodeCodeLang = $this->lang->projectplansh->changeNodeCode;
        $reviewStage = $stage+1;
    ...
    public function initProject($planID, $changeCreationOnly = 0)
    {
        ...
        $creation = fixer::input('post')
            ...
            ->stripTags($this->config->projectplansh->editor->initproject['id'], $this->config->allowedTags)
            ->get();
        if(!isset($creation->linkPlan)){
            $creation->linkPlan = '';
        }

        $creation = $this->loadModel('file')->processImgURL($creation, $this->config->projectplansh->editor->initproject['id'], $this->post->uid);
        ...
        if ($mode == 'create') {
            $this->dao->insert(TABLE_PROJECTCREATION)->data($creation)->autoCheck()
                ->batchCheck($this->config->projectplansh->initproject->requiredFields, 'notempty')
    ...
    public function setListValue()
    {
        ...
        $typeList = $this->lang->projectplansh->typeList;
        foreach ($typeList as $id => $name) {
            if (!$id) continue;
            $types[$id] = "$name(#$id)";
        }
        $categoryList = $this->lang->opinion->categoryList;
        $basisList = $this->lang->projectplansh->basisList;
        $storyStatusList = $this->lang->projectplansh->storyStatusList;
        $structureList = $this->lang->projectplansh->structureList;
        $localizeList = $this->lang->projectplansh->localizeList;
        ...
        $dataEnterLakeList = $this->lang->projectplansh->dataEnterLakeList;
...
//        $this->post->set('lineList', array_values($lines));
        $this->post->set('platformownerList', join(',',$this->lang->projectplansh->platformownerList));
        ...
        $this->post->set('listStyle', $this->config->projectplansh->export->listFields);
...
//            $projectplanData->architrcturalTransform =  $data->architrcturalTransform[$key];
            ...
            if (isset($this->config->projectplansh->create->requiredFields)) {
                $requiredFields = explode(',', $this->config->projectplansh->create->requiredFields);
                foreach ($requiredFields as $requiredField) {
                    $requiredField = trim($requiredField);
                    if (empty($projectplanData->$requiredField)) dao::$errors[] = sprintf($this->lang->projectplansh->noRequire, $line, $this->lang->projectplansh->$requiredField);
                }
            }
...
//        $project->end            = $plan->creation->end;//$plan->end;//报工需求 计划结束时间改为项目起止时间
       ...
       //上海白名单 添加后台自定义配置人员到
        $setWhiteList = $this->lang->project->setShWhiteList;
        $setWhiteListValue = $setWhiteList['white'];
        //2022-04-12 tongyanqi 添加总经理和部门领导到项目白名单
        $whitelist_array = $this->getBosses($plan);
        $project->whitelist = trim(implode(',',$whitelist_array['managers']) .',' . implode(',',$whitelist_array['bosses']) . ','.$setWhiteListValue,',');
...
//        $this->addProjectToDemand($projectID, $_POST['demand'], $planID);
    ...
    public function editPlanOpinion(){
        ...
        if(!$projectplan->planID){
            dao::$errors[] = $this->lang->projectplansh->planIDNotEmpty;
            return false;
        }
        if(!$projectplan->opinionID){
            dao::$errors[] = $this->lang->projectplansh->opinionIDNotEmpty;
            return false;
        }
        $plan = $this->getSimpleByID($projectplan->planID);

        if(!$plan){
            dao::$errors[] = $this->lang->projectplansh->planNotExist;
            return false;
        }
        $intersect = [];
        if($plan->requirement){
        ...
        if($intersect){
            dao::$errors[] = $this->lang->projectplansh->planRequirementNotEmpty;
            return false;
        }
...
//        $requirementProjectList = $this->dao->select("*")->from(TABLE_REQUIREMENT)->where("opinion")->eq($projectplan->opinionID)->andWhere("FIND_IN_SET('{$plan->project}',`project`)")->fetchAll('id');
    ...
    public function editPlanRequirement(){
        ...
        if(!$projectplan->requirementID){
            dao::$errors[] = $this->lang->projectplansh->requirementIDNotEmpty;
            return false;
        }
        $plan = $this->getSimpleByID($projectplan->planID);
        ...
        if($intersect){
            dao::$errors[] = $this->lang->projectplansh->planDemandNotEmpty;
            return false;
        }
...
//        if ($action == 'yearreviewing') return ($object->status == 'yearreviewing' or $object->status == 'yearwait') and strpos(",$object->reviewers,", ",{$app->user->account},") !== false;
    ...
    public function oldSendmail($planID, $actionID = 0, $isReissueMail = false)
    {
        ...
        //$mailContent .= '<td colspan="3">' . zget($this->lang->projectplansh->sourceList, $plan->creation->source, '') . '</td>';
        //$mailContent .= '</tr>';
        ...
        //$mailContent .= '<td colspan="3">' . zget($this->lang->projectplansh->typeList, $plan->creation->type, '') . '</td>';
        //$mailContent .= '</tr>';
...
//        $subject = $this->getSubject($plan);
//        $subject = '立项通知-' . $plan->mark . '(' . $plan->name . ')' . '项目立项审批通过';
        $subject = htmlspecialchars_decode(vsprintf($this->lang->projectplansh->projectApprovalMailSubject,[$plan->mark,$plan->name]));
...
//        $mailContent .= '<p style="margin-left:20px;">项目基本情况：（<a href="' . $detailsURL . '" target="_blank">点击查看详情</a>）</p>';
        ...
        $mailContent = vsprintf($this->lang->projectplansh->projectApprovalMailContent,[$plan->mark,$plan->name,$detailsURL]);
    ...
    public function sendmail($planID, $actionID)
    {
        ...
        if (!in_array($plan->status, $this->lang->projectplansh->mailAllowSendStatus)) return false;
    ...
    public function sendActionmail($planID)
    {
        ...
        $toList = array_filter(array_keys($this->lang->projectplansh->changeNoticeUser));
        $ccList = "";
...
//        $mailTitle = vsprintf($mailConf->mailTitle, $mailConf->variables);
    ...
    {
        return $this->lang->projectplansh->projectCreation . '#' . $plan->id . ' ' . $plan->name . '-' . $this->lang->projectplansh->labelList[$plan->status];
    }
    ...
    public function getChangeMsgPerson($plan,$changeStatus)
    {
        ...
        if($changeStatus == 'pending'){
            $reviewers = $this->review->getReviewer('planchange', $plan->id, $plan->changeVersion, $plan->changeStage);
            $ccList = '';
        }
...
//            ->andWhere('changeStatus')->ne($this->lang->projectplansh->ChangestatusEnglishList['pending'])
            ->fetchAll();
    }
...
//            ->andWhere('changeStatus')->ne($this->lang->projectplansh->ChangestatusEnglishList['pending'])
    ...
    private function reformProductRelated()
    {
        ...
        foreach ($_POST['productIds'] as $productId) {
            ...
            if (baseValidater::checkDate($_POST['realRelease'][$i]) == false) {
                dao::$errors['realRelease'] = [sprintf($this->config->projectplansh->realReleaseEmpty, $i + 1)];
                return false;
            }
            if (baseValidater::checkDate($_POST['realOnline'][$i]) == false) {
                dao::$errors['realOnline'] = [sprintf($this->config->projectplansh->realOnlineEmpty, $i + 1)];
                return false;
            }
            if ($_POST['realOnline'][$i] < $_POST['realRelease'][$i]) {
                dao::$errors['realOnline'] = [sprintf($this->config->projectplansh->realOnlineError, $i + 1)];
                return false;
            }
    ...
    private function reformPlanStage()
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            if (empty($_POST['stageBegin'][$i])) {
                dao::$errors['planStage'] = [sprintf($this->config->projectplansh->stageBeginError, $i + 1)];
                return false;
            }
            if (empty($_POST['stageEnd'][$i])) {
                dao::$errors['planStage'] = [sprintf($this->config->projectplansh->stageEndError, $i + 1)];
                return false;
            }
    ...
    {
        $codePrefix = $this->lang->projectplansh->typeCodeList[$type] . '-' . $year . '-';
    ...
    {
        $plan = $this->getByID($planId);
        $codePrefix = $this->lang->projectplansh->typeCodeList[$plan->type] . '-' . $plan->year . '-';
        $last = $this->dao->select('planCode')->from(TABLE_PROJECTPLAN)->where('planCode')->like("$codePrefix%")->orderby('planCode_desc')->fetch();
        $number = 1;
        if(isset($last->planCode)){
            $codeArr = explode('-',$last->planCode);
    ...
    private function checkPost()
    {
        if (empty($_POST['basis'])) {
            dao::$errors['basis'] = $this->lang->projectplansh->basisEmpty;
            return false;
        }
        if (empty($_POST['category'])) {
            dao::$errors['category'] = $this->lang->projectplansh->categoryEmpty;
            return false;
        }
        if (empty($_POST['app'])) {
            dao::$errors['app'] = $this->lang->projectplansh->appEmpty;
        ...
        if (empty($_POST['bearDept'])) {
            dao::$errors['bearDept'] = $this->lang->projectplansh->bearDeptEmpty;
            return false;
        }
        //如果是 应用研发类改造和新建 则需求状态必填
        if($_POST['type'] == 1 || $_POST['type'] == 2){
            if(!$_POST['storyStatus']){
                dao::$errors['storyStatus'] = $this->lang->projectplansh->storyStatusEmpty;
                return false;
            }
            if(!isset($_POST['platformowner'])){
                dao::$errors['platformowner'] = $this->lang->projectplansh->platformownerEmpty;
                return false;
            }
