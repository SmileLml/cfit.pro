...
class baseDAO
{
    ...
    /**
     * 读写分离标记变量
     * The flag variable for read and write separation.
     *
     * @var bool
     * @access protected
     */
    protected $rwSeparation = false;

    /**
     * 从库读取数据标记变量
     * The flag variable for reading data from slave database.
     *
     * @var bool
     * @access protected
     */
    ...
    /**
     * 写入数据的数据库表数组
     * The database table list be writed.
     *
     * @var bool
     * @access protected
     */
    ...
    public function __construct()
    {
        ...
        /* Set auto-split read and write flag variable. */
        $this->rwSeparation = isset($config->rwSeparation) ? $config->rwSeparation : false;
    ...
    /**
     * 设置从服务器的启用状态。
     * Set enabled status of slave database.
     *
     * @param  bool   $enabled
     * @access public
     * @return void
     */
    public function slave($enabled = true)
    {
        $this->useSlave = $enabled;
        return $this;
    }
    ...
    public function count($distinctField = '')
    {
        ...
        $dbh = $this->getDbHandler($sql);
        self::$querys[] = (($this->config->debug and $dbh != $this->dbh) ? '[SLAVE]' : '[MASTER]') . $sql;
    ...
    public function processSQL($record = true)
    {
        ...
        /* Auto-collect tables with writing operation. */
        if($this->rwSeparation and in_array($this->method, array('insert', 'update', 'delete', 'replace'))) $this->changedTables[$this->table] = true;
        ...
        if($record) self::$querys[] = (($this->config->debug and $this->getDbHandler($sql) != $this->dbh) ? "[SLAVE]" : '[MASTER]') . $this->processKeywords($sql);
        return $sql;
    }
    ...
    public function query($sql = '')
    {
        ...
        try
        {
            ...
            /* If specified slave DB handler. */
            if($this->slaveDBH and $method == 'select') $dbh = $this->getDbHandler($sql);
     ...
     * @return array
     */
    public function fetchPairs($keyField = '', $valueField = '', $valueFieldMore = '')
    {
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            ...
            if($valueFieldMore){
                $pairs[$row[$keyField]][$valueField] = $row[$valueField];
                $pairs[$row[$keyField]][$valueFieldMore] = $row[$valueFieldMore];
            } else {
     ...
     * @return array
     */
    public static function getWarn($join = true)
    {
        $warns = dao::$warns;
        dao::$warns = array();     // 清除dao的错误信息(Must clear errors)

        if(!$join) return $warns;

        if(is_array($warns))
        {
            $message = '';
            foreach($warns as $item)
            {
                is_array($item) ? $message .= join('', $item) . '' : $message .= $item . '';
            }
            return $message;
        }
    }
     ...
     * @return oject
     */
    public function getDbHandler($sql = '')
    {
        if(self::$inTransaction) return $this->dbh;
        if(empty($this->slaveDBH)) return $this->dbh;

        /* Auto splitting read and write. */
        if(!$this->rwSeparation) return $this->dbh;

        /* Fetch joined tables. */
        preg_match_all("/join {1,}`{0,1}([a-z_]+)`{0,1} {1,}/", strtolower($sql), $matches);
        foreach(array_merge($matches[0], array($this->table)) as $table) if(array_key_exists($table, $this->changedTables)) return $this->dbh;

        /* 如果请求方法不在配置的只读方法中，则使用主数据库。 */
        $action = $this->app->rawModule . '.' .$this->app->rawMethod;
        if(!in_array(strtolower($action), $this->config->readonlyActions)) return $this->dbh;
        return $this->slaveDBH;
    }
}
