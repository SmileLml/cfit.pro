...
$(function()
{
    ...
    {
        var url = createLink('testcase', 'showImport', "applicationID=" + applicationID  + "&productID=" + productID + "&branch=" + branch + "&pageID=1&maxImport=" + $('#maxImport').val());
        location.href = url;
    });
});
</script>
    ...
    <table class='table table-form' id='showData'>
      <thead>
        <tr>
          ...
          <th class='required w-120px'><?php echo $lang->bug->product;?></th>
          <?php if($this->app->openApp != 'project'):?>
          <th class='required w-120px'><?php echo $lang->bug->project;?></th>
          <?php endif;?>
          <th class='required'><?php echo $lang->testcase->title?></th>
          <th class='w-160px'><?php echo $lang->testcase->module?></th>
          <th class='w-120px'><?php echo $lang->testcase->story?></th>
          <th class='w-80px'><?php echo $lang->testcase->pri?></th>
          <th class='required w-100px'><?php echo $lang->testcase->type?></th>
          <th class='w-120px'><?php echo $lang->testcase->stage?></th>
          <th class='w-100px'><?php echo $lang->testcase->categories;?></th>
          <th><?php echo $lang->testcase->precondition?></th>
          <th><?php echo $lang->testcase->intro?></th>
      ...
      <tbody>
        ...
        <tr valign='top' class='text-left'>
          ...
          if($this->app->openApp == 'project') echo html::hidden("project[$key]", $case->project);
          echo html::select("product[$key]", $products, $case->product, "class='form-control chosen'");
          ?>
          </td>
          ...
          <td style='overflow:visible'>
          <?php
          echo html::select("project[$key]", $projects, $case->project, "class='form-control chosen'");
          ?>
          </td>
          <?php endif;?>
          ...
          <td style='overflow:visible'>
            <?php echo html::select("module[$key]", $modules, isset($case->module) ? $case->module : ((!empty($case->id) and isset($cases[$case->id])) ? $cases[$case->id]->module : ''), "class='form-control  moduleChange'")?>
          </td>
          <td style='overflow:visible'>
          <?php $storyID = isset($case->story) ? $case->story : ((!empty($case->id) and isset($cases[$case->id])) ? $cases[$case->id]->story : '');?>
          <?php echo html::select("story[$key]", array($storyID => zget($stories, $storyID, '')), $storyID, "class='form-control  storyChange'")?></td>
          ...
          <td><?php echo html::select("type[$key]", $lang->testcase->typeList, isset($case->type) ? $case->type : '', "class='form-control '")?></td>
          ...
          <td style='overflow:visible'><?php echo html::select("categories[$key][]", $lang->testcase->categoryList, $case->categories, "multiple='multiple' class='form-control chosen'")?></td>
          ...
          <td><?php echo html::textarea("intro[$key]", isset($case->intro) ? htmlspecialchars($case->intro) : "", "class='form-control'")?></td>
          <?php if(!empty($appendFields)):?>
