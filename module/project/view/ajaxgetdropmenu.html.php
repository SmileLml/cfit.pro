...
if(!empty($closedProjects)) {
    foreach ($closedProjects as $project) {
        ...
        $link        = helper::createLink('project', 'index', "projectID=%s", '', '', $project->id);
        $projectName = zget($programs, $project->parent, '') ? zget($programs, $project->parent) . '/' . $project->name : $project->name;
        $closedProjectsHtml .= html::a(sprintf($link, $project->id), $projectName, '', "class='$selected' title='{$projectName}' data-key='" . zget($projectsPinYin, $projectName, '') . "'");
    }
}
...
<div class="table-row">
  <div class="table-col col-left">
    ...
    <div class="col-footer">
      ...
      <a class='pull-right toggle-right-col not-list-item'><?php echo $lang->project->closedProjects?><i class='icon icon-angle-right'></i></a>
    </div>
  </div>
