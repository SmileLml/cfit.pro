...
$(function()
{
    ...
    var refreshMenu = function()
    {
        ...
        if(val !== null && val !== "")
        {
            if(new RegExp(".*?script[^>]*?.*?(<\/.*?script.*?>)*", "ig").test(val))
            {
...
$(document).ready(function() {
    var resize = detectZoom();
    if (resize > 100) {
        $('#globalBarLogo').append('<div id="resiz" ><span style="color: red;white-space:nowrap;display: block;line-height: 40px;">浏览器缩放比例小于80%体验更佳</span></div>');
    }
})
