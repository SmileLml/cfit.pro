...
class user extends control
{
    ...
    public function login($referer = '', $from = '')
    {
        ...
        if(!empty($_POST) or (isset($_GET['account']) and isset($_GET['password'])))
        {
            ...
            else
            {
                ...
                //20240611 开启ldap，且user用户表不存在，但是ldap中存在，通过user-login登录的用户,提示用户不存在
                if(isset(dao::getError()['noUser'])){
                    $response['message'] = $this->lang->user->loginNoUser;
                }
                $this->send($response);
            }
        }
    ...
    public function ajaxGetContactList()
    {
        ...
        return print(html::select('contactListMenu', $contactList, '', "class='form-control' onchange=\"setMailto('mailto', this.value)\""));
    }
     ...
     * @return void
     */
    public function regularlyImportLDAP()
    {
        $this->loadModel('user');
        $users = $this->user->getLDAPUser('all');
        if($users == 'off')
        {
            echo 'The ldap function is not enabled.';
            return true;
        }

        $depts        = $this->loadModel('dept')->getOptionMenu();
        $defaultGroup = empty($this->app->config->ldap->group) ? '' : $this->app->config->ldap->group;
        $syncInterval = empty($this->app->config->ldap->syncInterval) ? 15 : $this->app->config->ldap->syncInterval;

        /* 判断一下同步间隔时间，如果不大于间隔时间，则本次不进行用户同步新增。*/
        $lastSyncTime = $this->dao->select('addTime')->from(TABLE_LDAPHISTORY)->orderBy('id_desc')->fetch('addTime');
        if(!empty($lastSyncTime))
        {
            $syncInterval = $syncInterval * 60;
            $nowTime      = time();
            $lastSyncTime = strtotime($lastSyncTime);
            $intervalDifference = $nowTime - $lastSyncTime;
            if($intervalDifference < $syncInterval)
            {
                echo 'The interval synchronization time has not been reached. Cancel synchronization.';
                return true;
            }
        }
        ...
        {
            /* 后续新增逻辑，如果不在同步的部门中，自动同步新增用户就不处理。*/
            $deptID = isset($depts[$user['dept']]) ? $user['dept']: 0;
            if(empty($deptID))
            {
                //$this->ldap->addSyncHistory($user['account'], $now, 'deptEmpty');
                continue;
            }
            ...
            {
                if($field == 'dept') continue;
                $_POST[$field][] = $value;
            }
        }

        $error = $this->user->importLDAP('all', 0, 'auto');
        if(!empty($error))
        {
            if(isset($error->repeat))
            {
                foreach($error->repeat as $account) $this->ldap->addSyncHistory($account, $now, 'repeat');
                echo sprintf($this->lang->user->error->repeat, join(',', $error->repeat)) . PHP_EOL;
            }

            if(isset($error->ill))
            {
                foreach($error->ill as $account) $this->ldap->addSyncHistory($account, $now, 'illaccount');
                echo sprintf($this->lang->user->error->illaccount, join(',', $error->ill)) . PHP_EOL;
            }
        }

        $this->ldap->sendMail($now);
        echo 'The user has been synchronized, and an email notification is sent.';
        return true;
    }
     ...
     * @return void
     */
    public function getLdapDeptUser()
    {
        $today = date('Y-m-d');
        $recordList = $this->dao->select('*')->from(TABLE_LDAPDEPT)->where('processDate')->eq($today)->fetch();;
        if(empty($recordList))
        {
            /* 清除用于记录每日要处理的数据。*/
            $this->dao->delete()->from(TABLE_LDAPDEPT)->exec();
            $this->dao->delete()->from(TABLE_LDAPUSER)->exec();

            /* 记录今日那些部门需要进行处理。*/
            $deptList = $this->dao->select('*')->from(TABLE_DEPT)->fetchAll();
            foreach($deptList as $dept)
            {
                if(!trim($dept->ldapName)) continue;

                $ldap = new stdClass();
                $ldap->deptID      = $dept->id;
                $ldap->deptName    = $dept->name;
                $ldap->ldapName    = $dept->ldapName;
                $ldap->processDate = $today;
                $this->dao->insert(TABLE_LDAPDEPT)->data($ldap)->exec();
            }
            echo $today . ' The department record to be processed was successful.';
        }
        else
        {
            $record = $this->dao->select('*')->from(TABLE_LDAPDEPT)->where('processDate')->eq($today)->andWhere('process')->eq(0)->fetch();
            if(empty($record))
            {
                echo $today . ' There is no more sector data to process.';
            }
            else
            {
                $result     = $this->user->addLdapDeptUser($record->ldapName, $record->deptID, $today);
                $resultDesc = $result ? ' Execution succeed.' : ' Failed to execute.';
                echo $today . ':' . $record->ldapName . $resultDesc;
                $this->dao->update(TABLE_LDAPDEPT)->set('process')->eq(1)->where('id')->eq($record->id)->exec();
            }
        }
    }
     ...
     * @return void
     */
    public function updateLdapDaily()
    {
        // 首次访问时，将今日要计算的用户都存起来。
        $today  = date('Y-m-d');
        $record = $this->dao->select('*')->from(TABLE_LDAPUPDATE)->where('processDate')->eq($today)->fetch();
        if(empty($record))
        {
            $this->dao->delete()->from(TABLE_LDAPUPDATE)->exec();
            $userList = $this->dao->select('id,account,dept,`role`,realname,ldap')->from(TABLE_USER)->where('deleted')->eq('0')->fetchAll();
            foreach($userList as $user)
            {
                if(!trim($user->ldap)) continue;
                $ldap = new stdClass();
                $ldap->account     = $user->account;
                $ldap->realname    = $user->realname;
                $ldap->ldap        = $user->ldap;
                $ldap->dept        = $user->dept;
                $ldap->role        = $user->role;
                $ldap->processDate = $today;
                $this->dao->insert(TABLE_LDAPUPDATE)->data($ldap)->exec();
            }
            echo 'The updateLdapDaily method adds records.';
        }
        else
        {
            $recordList = $this->dao->select('*')->from(TABLE_LDAPUPDATE)->where('process')->eq(0)->limit(50)->fetchAll();
            $this->user->updateLdapUser($recordList);
            echo 'The updateLdapDaily method is executed.' . ' Quantity: ' . count($recordList);
        }
    }
}
