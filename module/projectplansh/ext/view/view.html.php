...
<div id="mainContent" class="main-row">
    <div class="main-col col-8">
        <div class="cell">
            <div class="detail">
                <div class="detail-title"><?php echo $lang->projectplansh->content; ?></div>
                        ...
                        <table class="table ops">
                            ...
                            <?php if($list->status == 'reject' || $list->isreview == 2){
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><button href="<?php echo $this->createLink('projectplansh', 'ajaxshowdiffchange', "changeID=$list->id");?>" onclick="showajaxdiff(this)" class="btn" data-app="platform">查看</button></td>
                                        ...
                                        <?php $reviewedCount = count($node->reviewers); ?>
                                            ...
                                            foreach ($node->reviewers as $reviewkey=>$review){
                                                    if($reviewkey==0){
                                                        ?>
                                                        <tr>
                                                            <th rowspan="<?php echo $reviewedCount; ?>"><?php echo zget($this->lang->projectplansh->nodeCodeDesc, $node->nodeCode, ''); ?></th>
                ...
                <div class="detail">
                    ...
                    <div class="detail-content article-content">
                        <?php if (!empty($yearNodes)): ?>
                            <table class="table ops">
                                <tr>
                                    <th class="w-220px"><?php echo $lang->projectplan->node; ?></th>
                                    <td class="w-100px"><?php echo $lang->projectplan->reviewer; ?></td>
                                    <td class="w-120px"><?php echo $lang->projectplan->reviewResult; ?></td>
                                    <td class="w-160px"><?php echo $lang->projectplan->dealDate; ?></td>
                                    <td ><?php echo $lang->projectplan->reviewComment; ?></td>
                                ...
                                <tr>
                                    <th rowspan="<?php echo count($yearNodes[0]->reviewers); ?>"><?php echo zget($this->lang->projectplansh->nodeCodeDesc, $yearNodes[0]->nodeCode, ''); ?></th>
                                ...
                                <?php if (isset($allow) && $isShow == true): ?>
                                    <?php if (!empty($yearNodes[1])) { ?>
                                        ...
                                        <tr>
                                            <th rowspan="<?php echo $reviewedCount; ?>"><?php echo zget($this->lang->projectplansh->nodeCodeDesc, $yearNodes[1]->nodeCode, ''); ?></th>
                                    ...
                                    <?php if (!empty($yearNodes[2])) { ?>
                                        <tr>
                                            ...
                                            <th rowspan="<?php echo $reviewedCount; ?>"><?php echo zget($this->lang->projectplansh->nodeCodeDesc, $yearNodes[2]->nodeCode, ''); ?></th>
                                ...
                                <?php if (!empty($yearNodes[3])): ?>
                                    <tr>
                                        ...
                                        <th rowspan="<?php echo $reviewedCount; ?>"><?php echo zget($this->lang->projectplansh->nodeCodeDesc, $yearNodes[3]->nodeCode, ''); ?></th>
...
<!--                                        <th rowspan="--><?php //echo $reviewedCount; ?><!--">--><?php //echo zget($this->lang->projectplansh->nodeCodeDesc, $yearNodes[4]->nodeCode, ''); ?><!--</th>-->
...
<!--                                --><?php //endif; ?>
                                ...
                                <?php if (!empty($yearNodes[5])): ?>
                                    <tr>
                                        ...
                                        <th rowspan="<?php echo $reviewedCount; ?>"><?php echo zget($this->lang->projectplansh->nodeCodeDesc, $yearNodes[5]->nodeCode, ''); ?></th>
                            ...
                            <table class="table ops">
                                ...
                                foreach ($bookNodes as $bnode){
                                    ...
                                    foreach ($bnode->reviewers as $k=>$bnReviewer){
                                        ...
                                        ?>
                                        <tr>
                                            <th rowspan="<?php echo $reviewedCount; ?>"><?php echo zget($lang->projectplansh->submitnodeCodeDesc,$bnode->nodeCode); ?></th>
                                ...
                                <td colspan='3'>
                                    ...
                                    if (isset($plan->creation->linkPlan)) {
                                        ...
                                        foreach ($linkPlans as $linkPlan) {
                                            zget($plans, $linkPlan, '') ? html::a(common::printLink('projectplansh', 'planview', "appid=$linkPlan", zget($plans, $linkPlan), '', 'class="iframe"', '', true)) : '';
                                            $i++;
        ...
        <div class='main-actions'>
            <div class="btn-toolbar">
                ...
                <div class='divider'></div>
                <?php
                common::printIcon('projectplansh', 'initProject', "projectplanID=$plan->id&creationID=$creationID", $plan, 'button', 'file-text');

                // 判断是否审批年度计划
                if (in_array($plan->status, array('yearstart', 'yearwait', 'yearreviewing', 'yearreject'))) {
                    common::printIcon('projectplansh', 'yearReview', "projectplanID=$plan->id", $plan, 'button', 'start', '', 'iframe', true, '', $this->lang->projectplan->yearReview);
                    common::printIcon('projectplansh', 'yearReviewing', "projectplanID=$plan->id", $plan, 'button', 'glasses', '', 'iframe', true, '', $this->lang->projectplan->yearReviewing);
                } else if (in_array($plan->status, array('yearpass', 'start'))) {
                    if ($plan->changeStatus != 'pending') {
                        common::printIcon('projectplansh', 'submit', "projectplanID=$plan->id", $plan, 'button', 'start', '', 'iframe', true);
...
//                                        echo "<button type='button' class='disabled btn' title='".$this->lang->projectplan->submit."' style='pointer-events: unset;'><i class='icon-common-start disabled icon-start'></i></button>\n";
                        common::printIcon('projectplansh', 'planChange', "id=$plan->id", $plan, 'button', 'feedback', '', '', '', $lang->projectplan->planChange);
                    } else {
                        if ($plan->reviewers == $this->app->user->account) {
                            common::printIcon('projectplansh', 'changeReview', "projectplanID=$plan->id", $plan, 'button', 'glasses', '', 'iframe', true, '', $this->lang->projectplan->changeReview);
                        } else {
                    ...
                    common::printIcon('projectplansh', 'submit', "projectplanID=$plan->id", $plan, 'button', 'start', '', 'iframe', true);
                    common::printIcon('projectplansh', 'review', "projectplanID=$plan->id", $plan, 'button', 'glasses', '', 'iframe', true, '', $plan->reviewStage == 2 ? $this->lang->projectplan->involved : $this->lang->projectplan->review);
                }
                if ($plan->status == 'pass') {
                    common::printIcon('projectplansh', 'exec', "projectplanID=$plan->id", $plan, 'button', 'run', '', 'iframe', true);
                } else {
                    common::printIcon('projectplansh', 'edit', "projectplanID=$plan->id", $plan, 'button');
                }
                common::printIcon('projectplansh', 'execEdit', "id=$plan->id", $plan, 'button', 'change', '', '', '', '', $lang->projectplan->execEdit);
                common::printIcon('projectplansh', 'delete', "projectplanID=$plan->id", $plan, 'button', 'trash', 'hiddenwin');
                ?>
            </div>
        </div>
    </div>
                    ...
                    <table class='table table-data'>
                        ...
                        <tr>
                            ...
                            <td><?php echo zget($lang->projectplan->insideStatusList, $plan->insideStatus, ''); ?><span
                                    ...
                                    common::printIcon('projectplansh', 'editStatus', "id=$plan->id", $plan, 'list', 'edit', '', 'iframe', true);
                                    ?>
                    </span></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td><?php echo zget($lang->projectplan->isDelayPreYearList, $plan->isDelayPreYear, ''); ?><span
                                        class='c-actions text-left'><?php
                                common::printIcon('projectplansh', 'editDelayYear', "id=$plan->id", $plan, 'list', 'edit', '', 'iframe', true);
                                    ?></span>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                ...
                                foreach($slaveRelationArr as $slave){
                                    ...
                                    if($tempname){
                                        $link = $this->createLink('projectplansh', 'view',"planID=$slave");
                            ...
                            <td colspan="3">
                                ...
                                if($slaveRelationInfo) {
                                    ...
                                    foreach ($slaveRelationInfo as $mainRelation) {
                                        ...
                                        if ($tempname) {
                                            $link = $this->createLink('projectplansh', 'view',"planID=$mainRelation->mainPlanID");
                                            $htmla = html::a($link, $tempname, '');
                                            $tempslaveProjectPlanStr .= $htmla. '<br />';
                                        }
                            ...
                            <?php if ($opinionIndex <= 5): ?>
                                <tr>
                                    <td class='text-ellipsis'>
                                        ...
                                        <!--<span
                                                class='c-actions text-left'><?php
/*                                            common::printIcon('projectplansh', 'editPlanOpinion', "planID=$plan->id&opinionID=$opinion->id", $plan, 'list', 'trash', '', 'iframe', true);
                                            */?>
                                        </span>-->
                                    </td>
                                </tr>
                            ...
                            <?php if ($requirementIndex <= 5): ?>
                                <tr>
                                    <td class='text-ellipsis'>
                                        ...
                                        <!--<span
                                                class='c-actions text-left'><?php
/*                                            common::printIcon('projectplansh', 'editPlanRequirement', "planID=$plan->id&requirementID=$requirement->id", $plan, 'list', 'trash', '', 'iframe', true);
                                            */?>
                                        </span>-->
                                    </td>
                                </tr>
...
<script>
    ...
    function showAllChangeInfo(){
        let link = '<?php    echo helper::createLink("projectplansh","ajaxGetAllChangeInfo","planID=".$plan->id)."?onlybody=yes";  ?>';
