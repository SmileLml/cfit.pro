          ...
          <tr>
            ...
            <td style='overflow:visible'>
              <div class="input-control has-icon-right">
                <?php echo html::input("name[$i]", '', "maxlength='80' class='form-control title-import'");?>
            ...
            <td <?php echo zget($visibleFields, 'estStarted', "class='hidden'")?>><?php echo html::input("estStarted[$i]", '', "onchange='changeComputerBegin(this)' class='form-control text-center form-date begin-td'");?></td>
            <td <?php echo zget($visibleFields, 'deadline', "class='hidden'")?>><?php echo html::input("deadline[$i]", '', "onchange='changeComputerEnd(this)' class='form-control text-center form-date end-td'");?></td>
        ...
        <tfoot id='submitFoot'>
          <tr>
            <td colspan='<?php echo $colspan?>' class='text-center form-actions'>
              ...
              <?php echo html::a("javascript:history.go(-1);location.replace(document.referrer);",'返回','','class ="btn btn-back btn-wide"')?>
            </td>
          </tr>
        </tfoot>
      </table>
...
<table class='template' id='trTemp'>
  <tbody>
    <tr>
      ...
      <td <?php echo zget($visibleFields, 'estStarted', "class='hidden'")?>><?php echo html::input("estStarted[%s]", '', "onchange='changeComputerBegin(this)' class='form-control text-center form-date begin-td'");?></td>
      <td <?php echo zget($visibleFields, 'deadline', "class='hidden'")?>><?php echo html::input("deadline[%s]", '', "onchange='changeComputerEnd(this)' class='form-control text-center form-date end-td'");?></td>
...
<?php js::set('taskDurationError', $lang->task->taskDurationError);?>
<script>
function changeComputerBegin(obj)
{
    ...
    var diffValue = (dateEnd - dateStart) / (1000 * 60 * 60 * 24);

    if(diffValue > 7)
    {
        if($("#taskDurationError").length > 0)
        {
            return false;
        }

        $('#submitFoot').append('<tr><td class="text-center text-red" colspan="9" id="taskDurationError">' + taskDurationError + '</td></tr>');
    }
    else
    {
        $('#taskDurationError').remove();
    }
}
