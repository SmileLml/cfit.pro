...
class blockModel extends model
{
    ...
    public function save($id, $source, $type, $module = 'my')
    {
        ...
        $data = fixer::input('post')
            ...
            ->setIF($id, 'id', $id)
            ->add('order', $source =='workreport' ? '0' : ($block ? $block->order : ($this->getLastKey($module) + 1)))
            ->add('module', $module)
            ->add('hidden', 0)
            ->setDefault('grid', '4')
    ...
    public function getWelcomeBlockData()
    {
        ...
        /**
         * 仪表盘我的bug总数=实验室缺陷数+清总缺陷数据
         */
        //实验室缺陷数
        $bugs = (int)$this->dao->select('count(*) AS count')->from(TABLE_BUG)
            ->where('assignedTo')->eq($this->app->user->account)
            ->andWhere('deleted')->eq(0)
            ->fetch('count');
        //清总缺陷数据
        $defectCount = (int)$this->dao->select('count(*) AS count')->from(TABLE_DEFECT)
            ->where('dealUser')->eq($this->app->user->account)
            ->fetch('count');
        $data['bugs']  = $bugs + $defectCount;
