...
js::set('belongModel', $this->lang->demandcollection->belongModel);
js::set('kanban', $kanban);
js::set('isCanRecordEstimate', $isCanRecordEstimate);
...
$canRecordEstimate         = commonModel::hasPriv('kanban', 'recordEstimate');
...
$canSetCardType = commonModel::hasPriv('kanban', 'setCardType');
$user = $this->app->user->account;
if ($canSetCardType && ($kanban->owner == $user || $kanban->createdBy == $user || $user == 'admin')){
    $canSetCardType = 1;
}else{
    $canSetCardType = 0;
}
?>
<!--<a href="" class="iframe" id="agency" data-toggle="modale" data-width="width:80%" style="display:none"></a>-->
<div id='kanbanBox' class='hidden'>
    <div id="mainMenu" class="clearfix" style="padding:10px;">
        <div class="btn-toolbar pull-left">
            <a href="<?php echo $this->createLink('kanban', 'view', "kanbanID=$kanbanID&browseType=all")?>" class="<?php if($browseType == 'all') echo 'btn-active-text';?> btn btn-link"><span><?php echo $lang->kanbancard->allcard?></span></a>
            <a href="<?php echo $this->createLink('kanban', 'view', "kanbanID=$kanbanID&browseType=relatedToMe")?>" class="<?php if($browseType == 'relatedtome') echo 'btn-active-text';?> btn btn-link"><span><?php echo $lang->kanbancard->relatedToMe?></span></a>
        </div>
        <div class="btn-toolbar pull-left">
            <a class="btn btn-link querybox-toggle" id='bysearchTab'>
                <i class="icon icon-search muted"></i> <?php echo $lang->searchAB; ?>
            </a>
        </div>
    </div>
    <div style="max-width:99%" class="cell<?php if ($browseType == 'bysearch') echo ' show'; ?>" id="queryBox" data-module='kanban'></div>
        ...
        <div id='regionTabs'>
            ...
            <div id='region-tab-actions' class="<?php echo $regionID !== 'all' ? 'active' : '';?>">
                <div class='region-actions'>
                    <?php if(($canEditRegion or $canSetCardType or $canCreateLane or $canDeleteRegion or $canCreateRegion or ($kanban->archived and ($canViewArchivedCard or $canViewArchivedColumn))) and !(isset($this->config->CRKanban) and $this->config->CRKanban == '0' and $kanban->status == 'closed')):?>
                        ...
                        <ul class="dropdown-menu pull-right">
                            ...
                            <?php if($canSetCardType) echo '<li>' . html::a(inlink('setCardType', "kanbanID={$kanban->id}", '', 1), '<i class="icon icon-pencil"></i>' . $this->lang->kanban->setCardType, '', 'class="iframe" data-toggle="modal" data-width="600px"') . '</li>';?>
