...
<div id='mainContent'>
  <form class='main-table' method='post' target='hiddenwin' id='importFromLib' data-ride='table'>
    <table class='table has-sort-head table-fixed'>
      <thead>
        <?php $vars = "applicationID=$applicationID&productID=$productID&projectID=$projectID&branch=$branch&libID=$libID&orderBy=%s&browseType=$browseType&queryID=$queryID&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}";?>
        <tr>
          <th class='c-id'>
          ...
          <th class='w-120px'><?php common::printOrderLink('type',  $orderBy, $vars, $lang->testcase->type)?></th>
        </tr>
      </thead>
      <tbody>
        <?php $i = 0;?>
        ...
        <tr>
          ...
          <td class='text-left' data-module='<?php echo $case->module?>' style='overflow:visible'>
            ...
            <?php
            $selected = $moduleID == 0 ? 'ditto' : $moduleID;
            echo html::select("module[{$case->id}]", $modules, $selected, "class='form-control chosen'");
            ?>
          </td>
