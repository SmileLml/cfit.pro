...
class baseControl
{
    ...
    public function __construct($moduleName = '', $methodName = '', $appName = '')
    {
         ...
         */
        global $app, $config, $lang, $dbh, $common, $slaveDBH;
        $this->app      = $app;
        $this->config   = $config;
        $this->lang     = $lang;
        $this->dbh      = $dbh;
        ...
        $this->config->debug = $_GET['dbug'] ?? $this->config->debug; //手动开debug
        /**
         * 设置当前模块，读取该模块的model类。
    ...
    public function parseJSON($moduleName, $methodName)
    {
        ...
        $output['data']   = json_encode($this->view,JSON_UNESCAPED_UNICODE); //为了解决页面汉字编码，增加JSON_UNESCAPED_UNICODE
        $output['md5']    = md5(json_encode($this->view,JSON_UNESCAPED_UNICODE));//同上

        $this->output = json_encode($output,JSON_UNESCAPED_UNICODE);//同上
    }
    ...
    public function send($data, $type = 'json')
    {
        ...
        if(isset($data['result']) and $data['result'] == 'fail')
        {
            if(!empty($data['message']))
            {
                ...
                {
                    $message->$item = is_string($errors) ? $errors : implode(',', $errors);
                }
     ...
     * ajax 提交返回消息并跳转页面
     */
    public function sendBack($response, $target = 'parent')
    {
        echo js::alert($response['message']);
        die(js::locate($response['locate'], $target));
    }
    /**
     * 创建一个模块方法的链接。
