...
class baseHelper
{
    ...
    static public function diffDate2($date1, $date2, $weekend = true)
    {
    ...
    static public function diffDate3($end, $begin)
    {
        $end   = substr($end, 0, 10);
        $begin = substr($begin, 0, 10);
        if($end == '0000-00-00' or $begin == '0000-00-00' or $begin == '3000-00-00')
        {
            return 0;
        }
        else
        {
            return round((strtotime($end) - strtotime($begin)) / 86400, 0) + 1;
        }
    }
    ...
    {
        return  date(DT_DATE1, (strtotime(date(DT_DATE1)) - 86400));
    }
     ...
     * @return false|string
     */
    static public function currentYear(){
        return date('Y');
    }
     ...
     * @return false|string
     */
    static public function currentYearTime(){
        return date('Y-00-00 00:00:00');
    }
     ...
     * @return false|string
     */
    static public function currentMonthFirstDay(){
     ...
     * @return false|string
     */
    static public function lastMonthFirstDay(){
        return date('Y-m-01 00:00:00', (strtotime(date('Y-m-01 00:00:00')) - 86400));
    }
     ...
     * @return false|string
     */
    static function lastMonthEndDay(){
        return date('Y-m-d 00:00:00', (strtotime(date('Y-m-01 00:00:00')) - 86400));
    }
     ...
     * @return false|string
     */
    static function getWorkDate($workDays, $currentDay = ''){
        if(!$currentDay){
            $currentDay = date(DT_DATE1);
        }
        $time = strtotime($currentDay);
        $num = 0;
        while ($num < $workDays){
            $time = $time + 86400;
            $day = date(DT_DATE1, $time);
            $isWorkDay = helper::getIsWorkDay($day);
            if($isWorkDay){
                $num++;
            }
        }
     ...
     * @return false|void
     */
    static function getIsWorkDay($day){
        $isWorkDay = false;
        if(!$day){
            return $isWorkDay;
        }
    ...
    /**
     * 根据开始时间 时长 获取实际截止工作日 日期
     * @param $beginDay
     * @param $severalDays
     */
    static function getTrueWorkDay($beginDay,$severalDays = '',$flag = false){

        //获取开始时间 年份
        $year = date('Y',strtotime($beginDay));
        $dao = new baseDAO();
        if($flag){
            //查询后台节假日配置  假期
            $holiday = $dao->select('type,name,year,begin,end')->from(TABLE_HOLIDAY)->where('year')->eq($year)->andWhere('type')->eq('holiday')->fetchAll();

            $holidays = array(); //假期
            foreach ($holiday as $item){
                $holidays[] = helper::getDateRange($item->begin,$item->end);
            }
            $holidayall = array();
            foreach ($holidays as $al) {
                foreach ($al as $date) {
                    $holidayall[] = $date;
                }
            }
            //查询后台节假日配置  工作日
            $work = $dao->select('type,name,year,begin,end')->from(TABLE_HOLIDAY)->where('year')->eq($year)->andWhere('type')->eq('working')->fetchAll();

            $works = array(); //工作日
            foreach ($work as $it){
                $works[] = helper::getDateRange($it->begin,$it->end);
            }
            $workall = array();
            ...
            if($severalDays){
                $day = 1;
                while($day <= $severalDays){
                    $nextDate = date(DT_DATE1,strtotime( "+1 day",strtotime($beginDay)));
                    $number = (int)date('N',strtotime($nextDate));
                    if(!in_array($nextDate,$holidayall)){
                        if(($number != '6' && $number != '7') || in_array($nextDate,$workall)){
                            $day++;
                        }
                    }
                    $beginDay = $nextDate;
            ...
            $severalDays = $severalDays > 1 ? $severalDays - 1 : $severalDays;
            $beginDay = date(DT_DATE1,strtotime($beginDay) + 24*3600*$severalDays);
        }
        return $beginDay;
    }
    static function getWorkDay($beginDay,$severalDays = ''){
        //获取开始时间 年份
        $year = date('Y',strtotime($beginDay));
        $dao = new baseDAO();
        //查询后台节假日配置  假期
        ...
        if($severalDays){
            $day = 1;
            $symbol = $severalDays >= 0 ? '+' : '-';
            $severalDays = abs($severalDays);
            while($day <= $severalDays){
                $nextDate = date(DT_DATE1,strtotime( "{$symbol}1 day",strtotime($beginDay)));
                $number = (int)date('N',strtotime($nextDate));
                if(!in_array($nextDate,$holidayall)){
                    if(($number != '6' && $number != '7') || in_array($nextDate,$workall)){
                        $day++;
     ...
     * @throws Exception
     */
    static function isWorkDay($beginDay){
        //获取开始时间 年份
        $year = date('Y',strtotime($beginDay));
        $dao = new baseDAO();
        //查询后台节假日配置  假期
        ...
        $number = (int)date('N',strtotime($beginDay));
        if(!in_array($beginDay,$holidayall)){
            if(($number != '6' && $number != '7') || in_array($beginDay,$workall)){
                return true;
            }
        }
     ...
     * @throws Exception
     */
    static public function getDateRange($startDtae,$endDate,$format = 'Y-m-d'){
        $begin = new DateTime($startDtae);
     ...
     * @return array
     */
    static public function getReviewRequiredNodes($requiredNodesData){
        $data = [];
     ...
     * @return int
     */
    public static function getArrayLeafDataCount($data, &$total = 0){
        if(is_array($data)){
            foreach ($data as $val){
                self::getArrayLeafDataCount($val, $total);
            }
        }else{
            $total = $total +1;
        }
 ...
 * @return string
 */
function zmget($var, $keys, $defaultStr = '', $divisionCode = ',')
{
    $keys = trim($keys, ',');
    $keyArr = array_unique(explode(',', $keys));
    $res = [];
    if (!is_array($var) and !is_object($var)) return "";
    $type = is_array($var) ? 'array' : 'object';
    foreach ($keyArr as $key) {
        if(empty($key)) continue;
        if($type == 'array' && !empty($var[$key])){
            $res [] = $var[$key];
        } elseif($type == 'object' && !empty($var->$key)){
            $res [] = $var->$key;
        }
    }
    if($res){
        $info = implode($divisionCode, $res);
    }else{
        $info = $defaultStr;
    }
...
/**
 * get array values get key
 *
 * @param $arrayData
 * @param $keys
 * @return array
 */
function getArrayValuesByKeys($arrayData, $keys){
    $data = [];
    if(!($arrayData && is_array($arrayData) && $keys && ($keys))){
        return $data;
    }
    foreach ($keys as $keyVal){
        if(isset($arrayData[$keyVal])){
            $data[] = $arrayData[$keyVal];
        }
    }
    return $data;
}
 ...
 * @param bool $isEllipsis
 * @param string $ellipsisSign
 * @return string $subValuesStr
 */
function getArraySubValuesStr($arrayData, $subCount = 2, $separator = ',', $isEllipsis = true, $ellipsisSign = '...'){
    $subValuesStr = '';
    if(!$arrayData){
        return $subValuesStr;
    }
    ...
    if($count <= $subCount){
        $subValuesStr = implode($separator, $arrayData);
    }else{
        $subArray = array_slice($arrayData, 0, $subCount);
        $subValuesStr = implode($separator, $subArray);
        if($isEllipsis){
            $subValuesStr .= $ellipsisSign;
        }
    }
    return $subValuesStr;
}
