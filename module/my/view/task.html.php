    ...
    <table class="table has-sort-head table-fixed" id='taskTable'>
      ...
      <thead>
        <tr>
          ...
          <th class='c-mark w-120px'>              <?php common::printOrderLink('mark',       $orderBy, $vars, $lang->project->code);?></th>
          ...
          <th class='c-assignedTo w-90px'><?php common::printOrderLink('assignedTo', $orderBy, $vars, $lang->task->assignedTo);?></th>
          <?php endif;?>
          <?php if($type != 'finishedBy'): ?>
          <th class='c-user w-90px'>      <?php common::printOrderLink('finishedBy', $orderBy, $vars, $lang->task->finishedBy);?></th>
          <?php endif;?>
          <th class='c-hours w-90px'>      <?php common::printOrderLink('estimate',   $orderBy, $vars, $lang->task->estimateAB);?></th>
          <th class='c-hours w-90px'>      <?php common::printOrderLink('consumed',   $orderBy, $vars, $lang->task->consumedAB);?></th>
          <th class='c-hours w-90px'>      <?php common::printOrderLink('left',       $orderBy, $vars, $lang->task->leftAB);?></th>
          <th class='c-status w-90px'>     <?php common::printOrderLink('status',     $orderBy, $vars, $lang->statusAB);?></th>
...
<!--  --><?php /*echo html::a($this->createLink('task', 'view', "taskID=$task->id", '', '', $task->project), $task->name, null, "style='color: $task->color' data-group='project'");*/?>
            <?php echo html::a('javascript:void(0)', $task->name, null, 'data-app="project" style="color: $task->color" onclick="seturl('.$task->project.','.$task->id.')"')?>
              <?php if(!empty($task->children)) echo '<a class="task-toggle" data-id="' . $task->id . '"><i class="icon icon-angle-double-right"></i></a>';?>
          </td>
          <td class="hidden"><?php echo html::a('','','','data-app="project"  id="mytaskurl"')?></td>
          <td class='c-mark'><?php echo $task->mark;?></td>
          <td class='c-project' title="<?php echo $task->executionName;?>"><?php echo html::a($this->createLink('execution', 'task', "executionID=$task->execution", '', '', $task->project), $task->executionName, '', "data-group='project'");?></td>
          <?php if($type != 'openedBy'): ?>
          ...
          <td class='c-actions'>
            ...
            if($canBeChanged)
            {
                ...
                else
                {
                    ...
                    common::printIcon('task', 'finish', "taskID=$task->id&flag=2", $task, 'list', '', '', 'iframe', true, '', '', $task->project);
                    common::printIcon('task', 'editTask', "taskID=$task->id", $task, 'list', 'edit', '', 'iframe', true, "data-width='95%'", '', $task->project);
        ...
        <?php if(!empty($task->children)):?>
          ...
          <tr class='table-children<?php echo $class;?> parent-<?php echo $task->id;?>' data-id='<?php echo $child->id?>' data-status='<?php echo $child->status?>' data-estimate='<?php echo $child->estimate?>' data-consumed='<?php echo $child->consumed?>' data-left='<?php echo $child->left?>'>
            ...
            <td class='c-mark'><?php echo $child->mark;?></td>
            <td class='c-project' title="<?php echo $child->projectName;?>"><?php echo html::a($this->createLink('execution', 'task', "executionID=$child->project", '', '', $child->project), $child->executionName, '', "data-group='project'");?></td>
            <?php if($type != 'openedBy'): ?>
            ...
            <td class='c-actions'>
              ...
              if($canBeChanged)
              {
                  ...
                  else
                  {
                      ...
                      common::printIcon('task', 'finish', "taskID=$child->id&flag=2", $child, 'list', '', '', 'iframe', true, '', '', $child->project);
                     common::printIcon('task', 'editTask', "taskID=$task->id", $task, 'list', 'edit', '', 'iframe', true, "data-width='95%'", '', $task->project);
...
function seturl(project,id){
    ...
    $.post(createLink('my', 'ajaxGetProjectId', "project="+ project,''), function(data)
    {

    });
    $.ajaxSettings.async = true;
    var taskurl = createLink('task', 'view', 'id=' + id+'-workWaitList');
    $('#mytaskurl').attr('href',taskurl);
    $('#mytaskurl')[0].click();
}
</script>
