...
class baseHTML
{
    ...
    static public function a($href = '', $title = '', $misc = '', $newline = true)
    {
        ...
        $pathArr = explode('-',str_replace(".html","",$_SERVER['PATH_INFO']));
         if (substr_count($title,'返回') && in_array('workWaitList',$pathArr)){
            return "<a href='".$app->session->workWaitList."' $misc>$title</a>$newline";
        }
     ...
     * @return string
     */
    public static function closeModalButton($label = '')
    {
        return "<button type='button'  class ='btn btn-primary' data-dismiss='modal' aria-hidden='true'>$label</button>";
    }
...
class baseJS
{
     ...
     * @return string
     */
    static public function alert($message = '', $full = true,$link='')
    {
        global $app;
        ...
        if ($link != ''){
            return self::start($full) . "alert('" . $message . "');location.href='".$link."'" . self::end() . self::resetForm();
        }else{
