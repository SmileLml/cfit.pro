...
<div id="mainContent" class="main-content fade">
        <div class="main-header">
            <h2><?php echo $lang->reviewproblem->batchCreate;?></h2>
        </div>
        <?php
        $visibleFields  = array();
        ...
        {
            if(strpos(",{$config->reviewproblem->availableBatchCreateFields},", ",{$field},") === false) continue;
            if($field) $visibleFields[$field] = '';
        }
        foreach(explode(',', $config->reviewproblem->create->requiredFields) as $field)
        {
            if($field)
            {
                $requiredFields[$field] = '';
                if(strpos(",{$config->reviewproblem->availableBatchCreateFields},", ",{$field},") !== false) $visibleFields[$field] = '';
            }
        }
        ...
        <div class="table-responsive">
            <table class="table table-form">
                ...
                <tr>
                    ...
                    <th class='w-300px required text-center' <?php echo zget($requiredFields, 'review', '', ' required')?>'><?php echo $lang->reviewproblem->review;?></th>
                    <th class='w-200px required text-center' <?php echo zget($requiredFields, 'type', '', ' required')?>'><?php echo $lang->reviewproblem->type;?></th>
                    <th class='w-200px required text-center' <?php echo zget($requiredFields, 'raiseBy', '', '')?>'><?php echo $lang->reviewproblem->raiseBy;?></th>
                    <th class='w-200px required text-center' <?php echo zget($requiredFields, 'raiseDate', '', '')?>'><?php echo $lang->reviewproblem->raiseDate;?></th>
                    <th class='w-400px required text-center' <?php echo zget($requiredFields, 'title', '', ' required')?>'><?php echo $lang->reviewproblem->title;?></th>
                    <th class='w-500px required text-center' <?php echo zget($requiredFields, 'desc', '', ' required')?>'><?php echo $lang->reviewproblem->desc;?></th>
                </tr>
                </thead>
                <tbody>

                <?php for($i = 0; $i < $config->reviewproblem->batchCreate; $i++):?>
                    <tr>
                            <td><?php echo $i+1;?></td>
                        <?php if($i > 0):?>
                        ...
                        <?php endif;?>
                            ...
                            <td><?php echo html::input("title[$i]", '', "class='form-control' placeholder='{$lang->reviewproblem->titleTemplate}'");?></td>
...
<script>
    function reviewCheck($i)
    {
        ...
        var link = createLink('reviewproblem', 'ajaxGetType','reviewID=' + reviewID);
