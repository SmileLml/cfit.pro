...
<?php $browseLink = $app->session->bugList ? $app->session->bugList : inlink('browse', "applicationID=$bug->applicationID&productID=$bug->product");?>
<div id="mainMenu" class="clearfix">
  ...
  <div class="btn-toolbar pull-right">
    ...
    <?php common::printLink('bug', 'create', "applicationID={$bug->applicationID}&productID=$bug->product&branch={$bug->branch}&extra=projectID={$bug->project}", "<i class='icon icon-plus'></i>" . $lang->bug->create, '', "class='btn btn-primary' data-app='$openApp'"); ?>
  </div>
  <?php endif;?>
</div>
<div id="mainContent" class="main-row">
  <div class="main-col col-8">
    <div class="cell">
      ...
      <div class="detail">
        <div class="detail-title"><?php echo $lang->bug->analysis;?></div>
        <div class="detail-content article-content"><?php echo $bug->analysis;?></div>
      </div>
    ...
    $copyParams    = "applicationID={$bug->applicationID}&productID=$productID&branch=$bug->branch&$extraParams";
    $convertParams = "applicationID={$bug->applicationID}&productID=$productID&branch=$bug->branch&moduleID=0&from=bug&bugID=$bug->id";
    ?>
    <div class='main-actions'>
      <div class="btn-toolbar">
        ...
        common::printIcon('bug', 'defectbug',  $params, $bug, 'button', 'exchange', '', 'iframe', true);
        ...
        <div class='tab-content'>
          <div class='tab-pane active' id='legendBasicInfo'>
            <table class="table table-data">
              <tbody>
                <tr valign='middle'>
                  <th class='thWidth'><?php echo $lang->bug->status;?></th>
                  <td><span class='status-bug status-<?php echo $bug->status?>'><?php echo $this->processStatus('bug', $bug);?></span></td>
                </tr>
                <tr valign='middle'>
                  <th class='thWidth'><?php echo $lang->bug->project;?></th>
                  <td><?php if(!empty($projectPlan)) common::printLinks('projectplan', 'view', "id={$projectPlan->id}", $projectPlan->name, '', 'data-app="platform"');?></td>
                </tr>
                <tr valign='middle'>
                  <th class='thWidth'><?php echo $lang->bug->applicationID;?></th>
                  <td>
                  <?php $applicationName = zget($applicationList, $bug->applicationID, '');
                  common::printLinks('application', 'view', "applicationID={$bug->applicationID}", $applicationName, '', 'data-app="application"');
                  ?>
                  </td>
                </tr>
                <tr valign='middle'>
                    <th class='thWidth'><?php echo $lang->bug->linkDefect;?></th>
                    <td><?php if(!empty($defectID)) common::printLinks('defect', 'view', "defectId=$defectID", $defectCode, '', 'data-app="project"');?></td>
                </tr>
                ...
                <tr>
                  ...
                  <td><?php if(!common::printLinks('bug', 'browse', "productID=$bug->product&branch=$bug->branch", $branchName)) echo $branchName;?></td>
                </tr>
                <?php endif;?>
                <tr>
                  ...
                  else
                  {
                      foreach($modulePath as $key => $module)
                      {
                          ...
                          if(!common::printLink('bug', 'browse', "applicationID=$bug->applicationID&productID=$bug->product&branch=$module->branch&browseType=byModule&param=$module->id", $module->name)) echo $module->name;
                ...
                <tr>
                  ...
                  <td><?php echo zget($typeTileList, $bug->childType, '');?></td>
                </tr>
                <tr>
                  <?php
                  ...
                  <td>
                    ...
                    if(!empty($file) and common::hasPriv('file', 'download'))
                    {
            ...
            <table class='table table-data'>
              <tbody>
                <tr>
                  ...
                  <td><?php if($bug->execution) echo html::a($this->createLink('execution', 'view', "executionid=$bug->execution", '', true), $bug->executionName, '', 'class="iframe"');?></td>
                </tr>
                <tr class='nofixed'>
            ...
            <table class="table table-data">
              <tbody>
                ...
                <tr valign='middle'>
                  ...
                  <td><?php common::printLinks('product', 'view', "productID={$bug->product}", $product->name, '', 'data-app="product"');?></td>
                </tr>
                <tr valign='middle'>
                  <th><?php echo $lang->bug->productplan;?></th>
                  <td>
                  <?php
                  $planInfo = $this->loadModel('productplan')->getPairs($bug->product);
                  $plans = explode(',', $bug->linkPlan);
                  $plans = array_filter($plans);
                  $plansInfo = array();
                  foreach($plans as $key=> $plan)
                  {
                    $plansInfo[] = zget($planInfo, $plan);
                  }
                  echo implode(',', $plansInfo);
                  ?>
                  </td>
                </tr>
                <tr>
          ...
          <div class='tab-pane' id='legendMisc'>
            <table class="table table-data">
              <tbody>
                ...
                <tr class='text-top'>
                  <th class='w-80px'><?php echo $lang->bug->linkTesttask;?></th>
                  <td>
                    <?php
                    if(isset($bug->linkTesttask))
                    {
                        foreach($bug->linkTesttaskOddNumbers as $linkTesttaskID => $linkTesttaskOddNumber)
                        {
                            echo html::a($this->createLink('testtask', 'view', "testtaskID=$linkTesttaskID", '', false), "#$linkTesttaskID $linkTesttaskOddNumber", '', "class='' ") . '<br />';
                        }
                    }
                    ?>
                  </td>
                ...
                <tr>
                  ...
                  {
                      echo html::a($this->createLink('testcase', 'view', "caseID=$caseID", '', true), $case, '', "class='iframe' data-width='80%'") . '<br>';
                  }
                  ?>
                  </td>
                </tr>
