...
class caselibModel extends model
{
            ...
            else
            {
                ...
                if(!dao::isError())
                {
                    ...
                    foreach($data->desc[$key] as $id => $desc)
                    {
                        ...
                        $stepData->expect  = htmlspecialchars(trim($this->post->expect[$key][$id]));
                        $this->dao->insert(TABLE_CASESTEP)->data($stepData)->autoCheck()->exec();
                        if($stepData->type == 'group') $parentStepID = $this->dao->lastInsertID();
                        if($stepData->type == 'step')  $parentStepID = 0;
                    }
     ...
     * @return void
     */
    public function printCell($col, $case, $users, $modulePairs = [], $browseType = '', $mode = 'datatable')
    {
        /* Check the product is closed. */
        $canBeChanged = common::canBeChanged('case', $case);

        $canBatchEdit         = common::hasPriv('caselib', 'batchEditCase');
        $canBatchDelete       = common::hasPriv('caselib', 'batchDelete');
        
        $canBatchChangeModule = common::hasPriv('caselib', 'batchChangeModule');
        $canBatchAction       = ($canBatchEdit or $canBatchDelete or $canBatchChangeModule);
        ...
        if($col->show)
        {
            ...
            if(strpos(',product,stage,precondition,keywords,story,', ",{$id},") !== false)
            {
                $class .= ' text-ellipsis';
            }
            ...
            {
                $this->loadModel('flow')->printFlowCell('testcase', $case, $id);
            }
            switch($id)
            {
                case 'id':
                    if($canBatchAction)
                    {
                        ...
                        echo html::checkbox('caseIDList', array($case->id => '')) . html::a(helper::createLink('testcase', 'view', "caseID=$case->id&version=$case->version"), sprintf('%03d', $case->id));
                    }
                    else
                    {
                        printf('%03d', $case->id);
                ...
                case 'pri':
                    ...
                    break;
                case 'title':
                     if($modulePairs and $case->module) echo "<span title='{$this->lang->testcase->module}' class='label label-info label-badge'>{$modulePairs[$case->module]}</span> ";
                     $viewLink = helper::createLink('testcase', 'view', "caseID=$case->id&version=$case->version");
                     echo html::a($viewLink, $case->title, null, "style='color: $case->color'");
                    break;
                case 'type':
                ...
                case 'stage':
                    $stages = '';
                    foreach(explode(',', trim($case->stage, ',')) as $stage)
                    {
                        $stages .= $this->lang->testcase->stageList[$stage] . ',';
                    }
                    $stages = trim($stages, ',');
                ...
                case 'status':
                    ...
                    break;
                case 'categories':
                    echo $this->loadModel('testcase')->getCategoriesValueByKeys($case->categories);
                    break;
                case 'openedBy':
                    echo zget($users, $case->openedBy);
                    break;
                case 'actions':
                    if($this->config->testcase->needReview or !empty($this->config->testcase->forceReview))
                    {
                        common::printIcon('testcase', 'review', "caseID=$case->id", $case, 'list', 'glasses', '', 'iframe');
                    }
                    common::printIcon('testcase', 'edit', "caseID=$case->id", $case, 'list');
                    if(common::hasPriv('testcase', 'delete'))
                    {
                        $deleteURL = helper::createLink('testcase', 'delete', "caseID=$case->id&confirm=yes");
                        echo html::a("javascript:ajaxDelete(\"$deleteURL\", \"caseList\", confirmDelete)", '<i class="icon icon-trash"></i>', '', "title='{$this->lang->testcase->delete}' class='btn'");
                    }
                    common::printIcon('caselib', 'createCase', "libID=$case->lib&moduleID=$case->module&param=$case->id", $case, 'list', 'copy');
                    break;
            }
            echo '</td>';
        }
