...
class release extends control
{
    ...
    public function repush($releaseID, $type)
    {
        if(empty($releaseID) || empty($type)){
            $response['result']  = 'fail';
            $response['message'] = '推送目的类型错误';
            die($response['message']);
        }
        $release =  $this->dao->select('*')->from(TABLE_RELEASE)->where('id')->eq((int)$releaseID)->fetch();
        if($type == 'Qz'  && $release->pushStatusQz == 0){
            $response['result']  = 'fail';
            $response['message'] = '该介质未通过关联或审核，不能推送';
            die($response['message']);
        }
        //重新编辑时其他条件已经重置
        $this->dao->update(TABLE_RELEASE)->set('pushStatus'.$type)->eq(1)->set('pushFails'.$type)->eq(0)->where('id')->eq($releaseID)->exec();
        $actionID = $this->loadModel('action')->create('release', $releaseID, 'rePush'.$type, '', 1);
        $response['result']  = 'success';
        $response['message'] = '已发起重新推送';
        die($response['message']);
    }
}
