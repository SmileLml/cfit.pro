...
class kanban extends control
{
    ...
    public function deleteSpace($spaceID, $confirm = 'no')
    {
        ...
        else
        {
            ...
            foreach ($kanbans as $kanban){
                //删除该区域下所有卡片和工时
                $this->dao->update(TABLE_KANBAN)->set('deleted')->eq(1)->where('id')->eq($kanban->id)->exec();
                $kanbanCells = $this->dao->select('*')->from(TABLE_KANBANCELL)
                    ->where('`kanban`')->eq($kanban->id)
                    ->fetchAll('id');
                foreach ($kanbanCells as $kanbanCell){
                    $this->kanban->deleteCellCardAndConsumed($kanbanCell->id);
                }
            }
     ...
     * @return void
     */
    public function view($kanbanID,$browseType = 'all', $param = 0)
    {
        ...
        $regions = $this->kanban->getKanbanData($kanbanID,'',$browseType,$queryID);
        if(!isset($regions[$regionID])) $this->session->set('regionID', 'all', 'kanban');

        $actionURL = $this->createLink('kanban', 'view', "kanbanID=$kanbanID&browseType=bysearch&param=myQueryID");
        $type = $this->kanban->getTypeBykanban($kanbanID,'getKeyValue');
        $this->config->kanban->search['params']['type']['values'] = $type;
        $this->config->kanban->search['params']['assignedTo']['values'] = $users;
        $this->loadModel('kanban')->buildSearchForm($queryID, $actionURL);

        $this->view->isCanRecordEstimate = $this->kanban->isCanRecordEstimate($kanbanID);
        $this->view->users    = $users;
    ...
    public function deleteRegion($regionID, $confirm = 'no')
    {
        ...
        {
            $kanbanColumns = $this->dao->select('*')->from(TABLE_KANBANCOLUMN)
                ->where('`region`')->eq($regionID)
                ->fetchAll('id');
            foreach ($kanbanColumns as $kanbanColumn){
                $this->kanban->deleteColumnCardAndConsumed($kanbanColumn->id);
            }
    ...
    public function deleteColumn($columnID, $confirm = 'no')
    {
        ...
        {
            if($column->parent > 0){
                $this->kanban->processCards($column);
            }else{
                //若不为子列，则删除该列下所有卡片和卡片中的工时
                $this->kanban->deleteColumnCardAndConsumed($columnID);
            }
    ...
    public function createCard($kanbanID = 0, $regionID = 0, $groupID = 0, $columnID = 0)
    {
        if($_POST)
        {
            ...
            $kanbanGroup = $this->kanban->getKanbanData($kanbanID, $regionID);
            return $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'locate'=>'parent','closeModal' => 1, 'callback' => array('target' => 'parent', 'name' => 'updateRegion', 'params' => array($regionID, $kanbanGroup))));
        }

        $kanban      = $this->kanban->getById($kanbanID);
        $type = $this->kanban->getTypeBykanban($kanbanID,'getKeyValue');
        $kanbanUsers = $kanbanID == 0 ? ',' : trim($kanban->owner) . ',' . trim($kanban->team);
        $usersInfo       = $this->loadModel('user')->getListByAccounts($kanbanUsers);
        $users = array('' => '') +array_column($usersInfo,'realname','account');
        $this->view->users     = $users;
        $this->view->type      = $type;
    ...
    public function editCard($cardID)
    {
        ...
        if(!empty($_POST))
        {
            ...
            //更新卡片其他信息
            $this->kanban->computeCard($cardID);
            ...
            return $this->send(array('result' => 'success', 'message' => $this->lang->saveSuccess, 'closeModal' => 1, 'callback' => array('target' => 'parent', 'name' => 'updateRegion', 'params' => array($card->region, $kanbanGroup)),  'locate' => 'parent'));
        }
        $card        = $this->kanban->getCardByID($cardID);
        $type = $this->kanban->getTypeBykanban($card->kanban,'getKeyValue');

        $kanban      = $this->kanban->getById($card->kanban);
        $kanbanUsers = $card->kanban == 0 ? ',' : trim($kanban->owner) . ',' . trim($kanban->team);
        $kanbanUsersInfo       = $this->loadModel('user')->getListByAccounts($kanbanUsers);
        $kanbanUsers = array('' => '') +array_column($kanbanUsersInfo,'realname','account');
     ...
     * @return void
     */
    public function finishCard($cardID, $kanbanID ,$browseType = 'all', $param = 0)
    {
        $browseType = strtolower($browseType);
        $queryID = ($browseType == 'bysearch')  ? (int)$param : 0;
        $this->loadModel('action');
        ...
        $kanbanGroup = $this->kanban->getKanbanData($kanbanID,'',$browseType,$queryID);
        return print(json_encode($kanbanGroup));
    }
    ...
    public function viewCard($cardID)
    {
        ...
        $kanban = $this->kanban->getByID($card->kanban);
        $space  = $this->kanban->getSpaceById($kanban->space);
        $work   = $this->kanban->geteffortBycard($cardID,'kanban');
        $type = $this->kanban->getTypeBykanban($card->kanban,'getKeyValue');
     ...
     * @return void
     */
    public function moveCard($cardID, $fromColID, $toColID, $fromLaneID, $toLaneID, $kanbanID = 0,$browseType = 'all', $param = 0)
    {
        $browseType = strtolower($browseType);
        $queryID = ($browseType == 'bysearch')  ? (int)$param : 0;
        ...
        $this->loadModel('action')->create('kanbanCard', $cardID, 'kanbanmoved');
    ...
    public function importExecution($kanbanID = 0, $regionID = 0, $groupID = 0, $columnID = 0, $selectedProjectID = 0, $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        $this->app->loadClass('pager', $static = true);
        $pager = new pager($recTotal, $recPerPage, $pageID);
        $projects = $this->project->getPairsByProgram('', 'all', false, '', '', '', 'multiple');
        if (empty($projects)){
            $projects =  array($this->lang->kanban->allProjects);
            $executions2Imported = array();
        }else{
            $defaultProjectID = array_keys($projects)[0];
            $executions2Imported = $this->kanban->getFirstStats($selectedProjectID, 'all', 0, 0, 0, 'id_asc', $pager, $defaultProjectID);

        }
//        $pager->recTotal = count($executions2Imported);
//        $executions2Imported = array_splice($executions2Imported,($pageID-1)*$recPerPage ,$recPerPage);
...
//        $this->view->executions2Imported = $this->execution->getStatData($selectedProjectID, 'undone', 0, 0, false, '', 'id_asc', $pager);
        $this->view->pager               = $pager;
        $this->view->executions2Imported  = $executions2Imported;
        $this->view->users               = $this->loadModel('user')->getPairs('noletter|nodeleted');
        $this->view->kanbanID            = $kanbanID;
        $this->view->regionID            = $regionID;
        $this->view->groupID             = $groupID;
     ...
     * @return string
     */
    public function setCardColor($cardID, $color, $kanbanID,$browseType = 'all', $param = 0)
    {
        $browseType = strtolower($browseType);
        $queryID = ($browseType == 'bysearch')  ? (int)$param : 0;
     ...
     * @return void
     */
    public function archiveCard($cardID, $confirm = 'no',$browseType = 'all', $param = 0)
    {
        $browseType = strtolower($browseType);
        $queryID = ($browseType == 'bysearch')  ? (int)$param : 0;
        if($confirm == 'no')
        {
            return print(js::confirm($this->lang->kanbancard->confirmArchive, $this->createLink('kanban', 'archiveCard', "cardID=$cardID&confirm=yes&browseType=$browseType&param=$param")));
        }
        else
        {
            ...
            if(isonlybody()) return print(js::reload('parent.parent'));
            $card        = $this->kanban->getCardByID($cardID);
            $kanbanGroup = $this->kanban->getKanbanData($card->kanban, $card->region,$browseType,$queryID);
     ...
     * @return void
     */
    public function viewArchivedCard($regionID,$browseType='',$param = 0)
    {
        $browseType = strtolower($browseType);
        $queryID = ($browseType == 'bysearch')  ? (int)$param : 0;
        $this->app->loadLang('demandcollection');
        ...
        $actionURL = $this->createLink('kanban', 'viewArchivedCard', "regionID=$regionID&browseType=bysearch&param=myQueryID");
        unset($this->config->kanban->search['params']['type']);
        $this->loadModel('kanban')->buildSearchForm($queryID, $actionURL);
        $this->view->kanban      = $this->kanban->getByID($region->kanban);
	 ...
	 * @return void
	 */
    public function deleteCard($cardID, $confirm = 'no',$browseType = 'all', $param = 0)
    {
        $browseType = strtolower($browseType);
        $queryID = ($browseType == 'bysearch')  ? (int)$param : 0;
        $card = $this->kanban->getCardByID($cardID);
        if($confirm == 'no')
        {
            ...
            return print(js::confirm($confirmTip, $this->createLink('kanban', 'deleteCard', "cardID=$cardID&confirm=yes&browseType=$browseType&param=$param")));
        }
        else
        {
            if($card->fromType == '') {
                $this->kanban->delete(TABLE_KANBANCARD, $cardID);
                $this->kanban->deleteCardConsumed($cardID);
            }
    ...
    public function laneMove($executionID, $currentType, $targetType)
    {
        ...
        if(!dao::isError())
        {
            ...
            $this->loadModel('action')->create('kanbanlane', $laneID, 'kanbanmoved');
        }
     ...
     * Desc: 记录（看板模块新建）卡片的工时
     * 基于task模块的记录工时改造
     * liuyuhan
     */
    public function recordEstimate($cardID, $random = '')
    {
        ...
        if(!empty($_POST))
        {
            ...
            $this->kanban->batchCreateEffort($cardID, $efforts);
            if(dao::isError()) die(js::error(dao::getError()));
//            if($this->app->viewType == 'mhtml')
//            {
//                die(js::locate($this->createLink('kanban', 'view', "cardID=$cardID"), 'parent'));
//            }
        ...
        $date    = date(DT_DATE1);
        $efforts = $this->effort->getByObject('kanban', $cardID);
        ...
        $this->view->kanban    =  $kanban;
        $this->view->isCanRecordEstimate = $this->kanban->isCanRecordEstimate($kanban->id);
        $this->view->users      = $this->loadModel('user')->getPairs('noletter');
        $this->view->date       = $date;
        $this->view->card       = $card;
        $this->view->efforts    = $efforts;
        $this->view->objectType = 'kanban';
    ...
    public function editEstimate($effortID)
    {
        ...
        $cardID = $effort->objectID;
        $card = $this->dao->findByID($cardID)->from(TABLE_KANBANCARD)->andWhere('deleted')->eq(0)->fetch();

        if(!empty($_POST))
        {
            $this->kanban->checkConsumed();
            if(dao::isError()) die(js::error(dao::getError()));
            ...
            {
                $realData = $this->loadModel('effort')->getRealConsumedAndLeftByTaskID($cardID, $effort->account,'kanban');
                if(isset($kanban->team[$effort->account]))
                {
    ...
    public function deleteEstimate($effortID, $confirm = 'no')
    {
        ...
        if($confirm == 'no')
        {
            die(js::confirm($this->lang->effort->confirmDelete, $this->createLink('kanban', 'deleteEstimate', "effortID=$effortID&confirm=yes")));
        }
        else
        {
...
//            $task = $this->task->getByID($taskID);
            ...
            {
                $realData = $this->loadModel('effort')->getRealConsumedAndLeftByTaskID($cardID, $effort->account,'kanban');
                if(isset($card->team[$effort->account]))
                {
                    $teamUserLeft     = $realData->left;
                    $teamUserConsumed = $realData->consumed;
                    $this->dao->update(TABLE_TEAM)->set('left')->eq($teamUserLeft)->set('consumed')->eq($teamUserConsumed)
     ...
     * desc:获取泳道列表
     */
    public function ajaxGetLane(){
        $kanbanId = $this->post->kanbanId;
        $group = $this->dao->select('*')->from(TABLE_KANBANGROUP)->where('kanban')->eq($kanbanId)->orderBy('order')->fetchall();
        $groupID = array_column($group,'id');
        $lane = $this->kanban->getLanePairsByGroups($groupID);
        echo html::select('laneId', $lane, '', "class='form-control chosen' data-drop_direction='down'");
    }

    /**
     * @param $kanbanID
     */
    public function setCardType($kanbanID){
        $type = $this->kanban->getTypeBykanban($kanbanID);
        if ($_POST){
            $this->kanban->createKanbanType($kanbanID);
            if(dao::isError()) return $this->send(array('result' => 'fail', 'message' => dao::getError()));
    ...
    public function ajaxeditCardBydemand(){
        $fromID = $_POST['id'];
        $fromType = $_POST['fromType'];
        $info = $this->dao->findByID($fromID)->from(TABLE_DEMANDCOLLECTION)->fetch();

        $now         = helper::now();
        $style = "font-size: 14px;font-weight: 700;line-height: 20px;";

        $oldCards = $this->kanban->checkCard($fromID,$fromType,'',$fields="*");
        foreach ($oldCards as $oldCard) {
            ...
            $this->dao->update(TABLE_KANBANCARD)->data($newcard)->where('id')->eq($oldCard->id)->exec();
            $actionID = $this->loadModel('action')->create('kanbancard', $oldCard->id, 'edited');
            $changes = common::createChanges($oldCard, $newcard);
            $this->action->logHistory($actionID, $changes);
        }
        if(dao::isError()) return $this->send(array('result' => 'fail', 'message' => dao::getError()));
        $actionID = $this->loadModel('action')->create('demandcollection', $fromID, 'batchkanban', '');
        return $this->send(array('result' => 'success', 'message' => '更新成功', 'closeModal' => 1));
