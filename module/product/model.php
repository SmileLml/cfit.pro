...
class productModel extends model
{
     ...
     * @return mixed
     */
    public function getByIdPairs($productIDList)
    {
    ...
    {
        $products = $this->dao->select('id,concat(code,"_",name)')
            ->from(TABLE_PRODUCT)
            ->where('deleted')->eq(0)
     ...
     * 通过系统选产品
     */
    public function getCodeNamePairsByApp($app)
    {
        if(empty($app) || $app == 'null') return [];
        $appArray = array_filter(explode(',', $app));
        $products = $this->dao->select('id,concat(code,"_",name)')
            ->from(TABLE_PRODUCT)
            ->where('deleted')->eq(0)
            ->andWhere('status')->ne('closed')
            ->andWhere('app')->in($appArray)
            ->orderBy('id_desc')
            ->fetchPairs();
        return $products;
    }
    /**
     * Desc: 下拉框内容需要显示：${产品编号}（产品名称）
     * Date: 2022/9/5
     * Time: 15:59
     *
     * @param string $mode
     ...
     *
     */
    public function getProductInfo($mode = '', $programID = 0, $orderBy = '')
    {
        $orderBy = !empty($this->config->product->orderBy) ? $this->config->product->orderBy : 'isClosed';
        $products = $this->dao->select('*,  IF(INSTR(" closed", status) < 2, 0, 1) AS isClosed')
            ->from(TABLE_PRODUCT)
            ...
            ->beginIF(!$this->app->user->admin)->andWhere('id')->in($this->app->user->view->products)->fi()
            ->orderBy($orderBy)
            ->fetchPairs('id', 'name', 'code');
        foreach ($products as $key => $value) {
            $products[$key] = $value['code'] . "(" . $value['name'] . ")";
        }
        return $products;
    }
     ...
     * 获取拼接编号产品
     */
    public function getProductWithCodeName($mode = '', $programID = 0, $orderBy = '',$app = 0,  $productIds = [])
    {
        $orderBy = !empty($this->config->product->orderBy) ? $this->config->product->orderBy : 'isClosed';
        $products = $this->dao->select('id, concat(concat(code,"_"),name) AS name,  IF(INSTR(" closed", status) < 2, 0, 1) AS isClosed')
            ->from(TABLE_PRODUCT)
            ->where('deleted')->eq(0)
            ->beginIF($programID)->andWhere('program')->eq($programID)->fi()
            ->beginIF($app)->andWhere('app')->in($app)->fi()
            ->beginIF(strpos($mode, 'noclosed') !== false)->andWhere('status')->ne('closed')->fi()
            ->beginIF(!$this->app->user->admin)->andWhere('id')->in($this->app->user->view->products)->fi()
            ->beginIF($productIds)->andWhere('id')->in($productIds)->fi()
            ->orderBy($orderBy)
            ->fetchPairs('id', 'name');
        return $products;
    }
    ...
    public function getSwitcher($productID = 0, $extra = '', $branch = 0)
    {
        ...
        if ($productID) {
            ...
            $currentProductName = isset($currentProduct->name) ? $currentProduct->name : '';
            $this->session->set('currentProductType', isset($currentProduct->type) ? $currentProduct->type : '');
        }
    ...
    public function create()
    {
        ...
        if (!dao::isError()) {
            ...
            $productCode = $this->reformProductCode($productID);
            $this->dao->update(TABLE_PRODUCT)->set('code')->eq($productCode)->where('id')->eq($productID)->exec();
            $this->file->updateObjectID($this->post->uid, $productID, 'product');
            $this->dao->update(TABLE_PRODUCT)->set('`order`')->eq($productID * 5)->where('id')->eq($productID)->exec();
            if (isset($product->whitelist)) {
 ...
 * 涉及产品编号处理内容
 */
    public function reformProductCode($productID)
    {
        $i = 0;
        $products = [];
        if (count($_POST['enableTime']) != count(array_unique($_POST['enableTime']))) {

            $unique_arr = array_unique($_POST['enableTime']);
            $repeat_arr = array_diff_assoc($_POST['enableTime'], $unique_arr);
            dao::$errors['enableTime'] = $this->config->product->unique . '{' . $repeat_arr[1] . '}';

        }

        foreach ($_POST['codes'] as $code) {
            if (empty($_POST['codes'][$i])) {
                //dao::$errors['enableTime'] = $this->config->product->code;
                dao::$errors['enableTime'] = [sprintf($this->config->product->code, $i + 1)];
                return;
            }
            if (empty($_POST['enableTime'][$i])) {
                dao::$errors['enableTime'] = [sprintf($this->config->product->enableTime, $i + 1)];
                return;
            }
            if (empty($_POST['comment'][$i])) {
                dao::$errors['comment'] = [sprintf($this->config->product->desc, $i + 1)];
                return;
            }

            $codeInfo = new stdclass();
            $codeInfo->product = $productID;
            $codeInfo->code = $code;
            $codeInfo->enableTime = $_POST['enableTime'][$i];
            $codeInfo->desc = $_POST['comment'][$i];
            $codeInfo->createTime = helper::now();

            $this->dao->insert(TABLE_PRODUCTCODEINFO)->data($codeInfo)->exec();
            $i++;
        }
        $j = 0;
        foreach ($_POST['enableTime'] as $enableTime) {
            if ($enableTime == max($_POST['enableTime'])) {
                $productCode = $_POST['codes'][$j];
            }
            $j++;
        }
    ...
    public function update($productID)
    {
        ...
        $value =  $this->getCodeInfo($productID);
        $jsonKeyToChinese = $this->jsonKeyToChinese(json_encode($value,JSON_UNESCAPED_UNICODE));

        $oldProduct->productCodeInfo = $jsonKeyToChinese;
        ...
        $product->skipBuild = isset($product->skipBuild) ? $product->skipBuild : '';
        ...
        $this->dao->update(TABLE_PRODUCT)->data($product)->autoCheck()
            ...
            ->exec();
        $productCode = $this->reformProductCodeUpdate($productID);
        $this->dao->update(TABLE_PRODUCT)->set('code')->eq($productCode)->where('id')->eq($productID)->exec();
        $this->tryError(1); //检查报错 1= 需要rollback
        $this->dao->commit();
        if (!dao::isError()) {
            ...
            $this->loadModel('personnel')->updateWhitelist($whitelist, 'product', $productID);
            if ($product->acl != 'open') $this->loadModel('user')->updateUserView($productID, 'product');
            $value2 =  $this->getCodeInfo($productID);
            $jsonKeyToChinese2 = $this->jsonKeyToChinese(json_encode($value2,JSON_UNESCAPED_UNICODE));
            $product->productCodeInfo =$jsonKeyToChinese2;
 ...
 * 涉及产品编号处理内容
 */
    public function reformProductCodeUpdate($productID)
    {

        $this->dao->delete()->from(TABLE_PRODUCTCODEINFO)->where('product')->eq($productID)->exec();
        $i = 0;
        $products = [];
        ...
        if (count($_POST['enableTime']) != count(array_unique($_POST['enableTime']))) {
            ...
            $unique_arr = array_unique($_POST['enableTime']);
            $repeat_arr = array_diff_assoc($_POST['enableTime'], $unique_arr);
            foreach ($repeat_arr as $value){
                dao::$errors['enableTime'] = $this->config->product->unique . '{' . $value . '}';
            }
        }
        foreach ($_POST['codes'] as $code) {
            if (empty($_POST['codes'][$i])) {
    ...
    public function jsonKeyToChinese($productCodeInfos){
       $value1 =  str_replace('product','产品ID',$productCodeInfos);
       $value2 =  str_replace('code','产品编号',$value1);
       $value3 =  str_replace('enableTime','产品编号启用时间',$value2);
       $value4 =  str_replace('desc','备注',$value3);
       return $value4;
    }
     ...
     * @return array
     */
    public function getProjectPairsByProductIdList($productIdList = array())
    {
        return $this->dao->select('t2.id,t2.name')->from(TABLE_PROJECTPRODUCT)->alias('t1')
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.project = t2.id')
     ...
     * getPairs 取多一个字段
     */
    public function getPairsByKey($key = 'code', $mode = '', $programID = 0, $orderBy = '')
    {
        if (defined('TUTORIAL')) return $this->loadModel('tutorial')->getProductPairs();
    ...
    {
        $langData = $this->dao->select('`section`, `key`, `value`, `system`')->from(TABLE_LANG)->where('module')->eq('productplan')->fetchAll();
        $selects = [];
        foreach ($langData as $item) {
            $selects[$item->section][$item->key] = $item->value;
        }
        unset($selects['osTypeList']['']);
        $selects['osTypeList'] = isset($selects['osTypeList']) ? [''] + $selects['osTypeList'] : [''];
        unset($selects['archTypeList']['']);
        $selects['archTypeList'] = isset($selects['archTypeList']) ? [''] + $selects['archTypeList'] : [''];
        return $selects;
    }
    ...
    {
        if (empty($ids)) return [];
        return $this->dao->select('id,name')->from(TABLE_PRODUCT)->where('id')->in($ids)->fetchPairs('id', 'name');
    }
     ...
     * @param $productID
     */
    public function getCodeInfo($productID)
    {
        if (empty($productID)) return false;
        $codeInfo = $this->dao->select('product,code,enableTime,`desc`')->from(TABLE_PRODUCTCODEINFO)->where('product')->eq($productID)
            ->andWhere('deleted')->eq(0)
            ->orderBy('enableTime desc')
            ->fetchAll();
    ...
    public function changeApplication($productID, $changeType = '', $oldAppID = 0)
    {
        $product = $this->dao->select('id,app')->from(TABLE_PRODUCT)->where('id')->eq($productID)->fetch();

        // 判断所属应用系统下是否只剩下一个产品。
        $relatedProducts = $this->dao->select('id,app')->from(TABLE_PRODUCT)->where('app')->eq($product->app)->andWhere('deleted')->eq('0')->fetchAll();

        $this->dao->begin();
        if($changeType == 'delete')
        {
            $this->dao->update(TABLE_BUG)->set('deleted')->eq(1)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_CASE)->set('deleted')->eq(1)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_TESTSUITE)->set('deleted')->eq(1)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_TESTTASK)->set('deleted')->eq(1)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_TESTREPORT)->set('deleted')->eq(1)->where('product')->eq($productID)->exec();
        }
        elseif($changeType == 'update')
        {
            $this->dao->update(TABLE_BUG)->set('applicationID')->eq($product->app)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_CASE)->set('applicationID')->eq($product->app)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_TESTSUITE)->set('applicationID')->eq($product->app)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_TESTTASK)->set('applicationID')->eq($product->app)->where('product')->eq($productID)->exec();
            $this->dao->update(TABLE_TESTREPORT)->set('applicationID')->eq($product->app)->where('product')->eq($productID)->exec();
        }

        if(count($relatedProducts) == 1)
        {
            $newProductID = $relatedProducts[0]->id;
            $this->dao->update(TABLE_BUG)->set('product')->eq($newProductID)->where('applicationID')->eq($product->app)->exec();
            $this->dao->update(TABLE_CASE)->set('product')->eq($newProductID)->where('applicationID')->eq($product->app)->exec();
            $this->dao->update(TABLE_TESTSUITE)->set('product')->eq($newProductID)->where('applicationID')->eq($product->app)->exec();
            $this->dao->update(TABLE_TESTTASK)->set('product')->eq($newProductID)->where('applicationID')->eq($product->app)->exec();
            $this->dao->update(TABLE_TESTREPORT)->set('product')->eq($newProductID)->where('applicationID')->eq($product->app)->exec();
        }

        // 对产品原旧系统的处理。
        // 判断所属应用系统下是否只剩下一个产品，假如旧系统只有一个产品了，那都迁移到这个产品下。
        $relatedProducts = $this->dao->select('id,app')->from(TABLE_PRODUCT)->where('app')->eq($oldAppID)->andWhere('deleted')->eq('0')->fetchAll();
        if(count($relatedProducts) == 1 and $product->app != $oldAppID)
        {
            $newProductID = $relatedProducts[0]->id;
            $this->dao->update(TABLE_BUG)->set('product')->eq($newProductID)->where('applicationID')->eq($oldAppID)->exec();
            $this->dao->update(TABLE_CASE)->set('product')->eq($newProductID)->where('applicationID')->eq($oldAppID)->exec();
            $this->dao->update(TABLE_TESTSUITE)->set('product')->eq($newProductID)->where('applicationID')->eq($oldAppID)->exec();
            $this->dao->update(TABLE_TESTTASK)->set('product')->eq($newProductID)->where('applicationID')->eq($oldAppID)->exec();
            $this->dao->update(TABLE_TESTREPORT)->set('product')->eq($newProductID)->where('applicationID')->eq($oldAppID)->exec();
        }
     ...
     * @return array
     */
    public function getProductCodeListByCondition($condition, $select = '*'){
        $data = [];
        if(!($condition)){
            return $data;
        }
        $sql  = "select ".$select." from " . TABLE_PRODUCTCODEINFO .
     ...
     * @return bool
     */
    public function getProductBasicInfo($productId, $select = '*'){
        if(!$productId){
            return false;
        }
        $ret = $this->dao->select($select)
