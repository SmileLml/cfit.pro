...
class projectdocModel extends model
{
    ...
    public function setMenu($libs, $libID = '')
    {
        ...
        if($libID)
        {
            ...
            if(empty($doclib))
            {
                echo(js::alert($this->lang->projectdoc->error->noFound));
                die(js::locate('back'));
            }

            if(!$this->checkPriv($doclib))
            {
                echo(js::alert($this->lang->projectdoc->error->accessDenied));
                die(js::locate('back'));
            }
        }
     ...
     * @return mixed
     */
    public function getLibList($projectID = 0, $orderBy, $pager)
    {
        $libs = $this->dao->select('*')->from(TABLE_PROJECTDOCUMENT)
            ->where('deleted')->eq('0')
            ->beginIF(!empty($projectID))->andWhere('projectID')->eq($projectID)->fi()
            ->orderBy($orderBy)
    ...
    {
        $doclib = $this->dao->select('*')->from(TABLE_PROJECTDOCUMENT)->where('id')->eq($libID)->fetch();
     ...
     * @return array
     */
    public function getLibPairs($projectID = 0)
    {
        $libs = $this->dao->select('*')->from(TABLE_PROJECTDOCUMENT)
            ->where('deleted')->eq(0)
            ->beginIF(!empty($projectID))->andWhere('projectID')->eq($projectID)->fi()
     ...
     * @return mixed
     */
    public function getLibPathPairs($projectID = 0)
    {
        $pairs = $this->dao->select('id,path')->from(TABLE_PROJECTDOCUMENT)
            ->where('deleted')->eq(0)
            ->beginIF(!empty($projectID))->andWhere('projectID')->eq($projectID)->fi()
            ->fetchPairs();
    ...
    public function create($projectID)
    {
        ...
        $this->dao->insert(TABLE_PROJECTDOCUMENT)->data($data)
            ->batchCheck($this->config->projectdoc->create->requiredFields, 'notempty')
            ->check($this->config->projectdoc->svn->requiredFields, 'notempty')
            ->autoCheck()
            ->exec();
    ...
    public function update($id)
    {
        ...
        if($data->encrypt == 'base64') $data->password = base64_encode($data->password);
        $this->dao->update(TABLE_PROJECTDOCUMENT)->data($data)
            ->batchCheck($this->config->projectdoc->edit->requiredFields, 'notempty')
            ->check($this->config->projectdoc->svn->requiredFields, 'notempty')
            ->autoCheck()
            ->where('id')->eq($id)->exec();
    ...
    public function checkConnection()
    {
        ...
        if($versionResult)
        {
            $message = sprintf($this->lang->projectdoc->error->output, $versionCommand, $versionResult, join("<br />", $versionOutput));
            dao::$errors['client'] = $this->lang->projectdoc->error->cmd . "<br />" . nl2br($message);
            return false;
        }
        ...
        if(stripos($path, 'https://') === 1 or stripos($path, 'svn://') === 1)
        {
            ...
            {
                dao::$errors['client'] = $this->lang->projectdoc->error->version;
                return false;
            }
        ...
        if($result)
        {
            $message = sprintf($this->lang->projectdoc->error->output, $command, $result, join("<br />", $output));
            if(stripos($message, 'Expected FS format between') !== false and strpos($message, 'found format') !== false)
            {
                dao::$errors['client'] = $this->lang->projectdoc->error->clientVersion;
                return false;
            }
            if(preg_match('/[^\:\/\\A-Za-z0-9_\-\'\"\.]/', $path))
            {
                dao::$errors['encoding'] = $this->lang->projectdoc->error->encoding . "<br />" . nl2br($message);
                return false;
            }

            dao::$errors['submit'] = $this->lang->projectdoc->error->connect . "<br>" . nl2br($message);
            return false;
        }
        return true;
    }
    ...
    public function checkClient()
    {
        ...
        {
            dao::$errors['client'] = $this->lang->projectdoc->error->clientPath;
            return false;
        }
        ...
        dao::$errors['client'] = sprintf($this->lang->projectdoc->error->safe, $clientVersionFile, $cmd);
    ...
    {
        if($this->config->requestType == 'GET') return helper::createLink('projectdoc', $method, $params, $viewType, $onlybody);
        ...
        $link   = helper::createLink('projectdoc', $method, $params, $viewType, $onlybody);
    ...
    public function markSynced($libID)
    {
        ...
        $this->dao->update(TABLE_PROJECTDOCUMENT)->set('synced')->eq(1)->where('id')->eq($libID)->exec();
    }
    ...
    public function updateCommit($doclib)
    {
        ...
        if(!empty($logs))
        {
            ...
            $this->dao->update(TABLE_PROJECTDOCUMENT)->set('lastSync')->eq(helper::now())->where('id')->eq($doclib->id)->exec();
        }
    }
    ...
    {
        return $this->dao->update(TABLE_PROJECTDOCUMENT)->set('commits')->eq($count)->where('id')->eq($libID)->exec();
    }
    ...
    {
        if(strpos($this->config->projectdoc->binary, "|$suffix|") !== false) return true;
