...
class mailModel extends model
{
     ...
     * @return void
     */
    public function send($toList, $subject, $body = '', $ccList = '', $includeMe = false, $extendInfo = null)
    {
        ...
        $objectType     = $this->session->mailObjectType != '' ? $this->session->mailObjectType : '';
        $actionType     = $this->session->mailActionType != '' ? $this->session->mailActionType : '';
        $objectId       = $this->session->mailObjectId != '' ? $this->session->mailObjectId : '';


        if(isset($extendInfo->objectType) && ($extendInfo->objectType)){
            $objectType = $extendInfo->objectType;
        }
        if(isset($extendInfo->objectId) && ($extendInfo->objectId)){
            $objectId = $extendInfo->objectId;
        }

        if(isset($extendInfo->actionType) && ($extendInfo->actionType)){
            $actionType = $extendInfo->actionType;
        }

        $this->session->mailObjectType = '';
        $this->session->mailActionType = '';
        $this->session->mailObjectId   = '';
        ...
        catch (phpmailerException $e) 
        {
            ...
            $mailLog['error'] = $e->errorMessage(). '<br />' . $mailError;
        ...
        //增加 mail_log记录
        $this->dao->insert(TABLE_MAIL_LOG)->data($mailLog)->exec();
