...
<div id='createHook'>
  <?php extCommonModel::printLink('workflowhook', 'create', "action=$action->id", "<i class='icon-plus'> </i>" . $lang->workflowhook->create, "class='btn btn-primary loadInModal'");?>
</div>
  ...
  <table class='table table-fixed' id='hookTable'>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td title="<?php echo $title;?>">
          ...
          elseif(is_array($hook->conditions))
          {
              foreach($hook->conditions as $k => $condition)
              {
                  ...
                  if(strpos(',currentUser,deptManager,actor,today,now,', ",$condition->param,") !== false)
                  {
        ...
        <td class='actions'>
          <?php extCommonModel::printLink('workflowhook', 'edit',   "action=$action->id&key=$key", $lang->edit, "class='edit loadInModal'");?>
