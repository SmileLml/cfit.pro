...
class file extends control
{
    ...
    {
        $this->file->sendDownTmpHeader($fileName, $fileType, $content, $type);
    }
     ...
     * @return void
     */
    public function delete($fileID, $isAjaxDel = false, $confirm = 'no')
    {
        if($confirm == 'no')
        {
            die(js::confirm($this->lang->file->confirmDelete, inlink('delete', "fileID=$fileID&isAjaxDel=$isAjaxDel&confirm=yes")));
        }
        else
        {
            $file = $this->file->getById($fileID);
	...
	public function getFile()
    {
        ...
        $localRealFile =  dirname(__FILE__, 3).'/www/data/upload/'. $level .'/'. $filename; //实际存的附件
