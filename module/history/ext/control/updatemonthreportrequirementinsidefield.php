...
{
    /**
     * 月报统计历史数据 刷新需求池 需求任务内部反馈超期统计表 反馈单总数字段名字
     * @return void
     */
    public function updateMonthReportrequirementinsidefield(){
        $monthreportList = $this->dao->select("id")->from(TABLE_WHOLE_REPORT)->where('type')->eq('requirement_inside')->fetchAll();
        $wholereportIDList = [];
        if($monthreportList){
