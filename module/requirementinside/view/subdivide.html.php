    ...
    <div id="mainContent" class="main-content fade">
        <div class="center-block">
            <div class="main-header">
                <h2><?php echo $lang->requirementinside->subdivide;?></h2>
            </div>
            <form method='post' class='load-indicator main-form form-ajax' enctype='multipart/form-data' id="batchCreateForm">
                <table class="table table-form">
                    ...
                    <tr>
                        <!--温馨提示 -->
                        <th><?php echo $lang->requirementinside->reminder;?></th>
                        <td style='color: red;' colspan='4'><?php echo $lang->requirementinside->reminderDesc;?></td>
                    </tr>
                    <tr>
                        <!--需求条目主题 -->
                        <th><?php echo $lang->requirementinside->demandTitle;?></th>
                    ...
                    <tr>
                        <!--期望实现日期 -->
                        <th><?php echo $lang->requirementinside->endDate;?></th>
                        ...
                        <td class='required' colspan='2'>
                            <div class='input-group'>
                                <span class='input-group-addon'><?php echo $lang->requirementinside->end; ?></span>
                    ...
                    <tr>
                        <!--期望实现日期 -->
                        <th></th>
                        <td colspan='2' style="color:#F00010;" id='endDateTip0'><?php echo $this->lang->requirementinside->endDateTip;?></td>
                        <td colspan='2'></td>
                    </tr>
                    <tr>
...
<!--                        <td class='required' colspan='2'>--><?php //echo html::select('app0', $apps, $requirement->app, "class='form-control chosen approw'  onchange='selectApp(this.id)'"); ?><!--</td>-->
                        <td class='required' colspan='2'>
                            <div class='input-group'>
                                <span class='input-group-addon'><?php echo $lang->requirementinside->responsiblePerson; ?></span>
                    ...
                    <tr id="productTr">
                        <!--所属产品 -->
                        <th><?php echo $lang->requirementinside->demandProduct;?></th>
                        ...
                        <td class='required' colspan='2'>
                            <div class='table-row ' >
                                <div class='table-col ' >
                                    <div class='input-group'>
                                        <span class='input-group-addon'><?php echo $lang->requirementinside->productVersion;?></span>
                                        ...
                                        <span class="input-group-btn"><a href="javascript:;" class="btn addItem" style="width:40px"><i class="icon-help" title="<?php echo $lang->requirementinside->createPlanTips?>"></i></a></span>
<!--                                        <span class="input-group-btn fix-border"><?php /*echo html::a($this->createLink('product','create'),'<i class="icon-plus"></i>'. $lang->requirementinside->newproduct ,'_blank','class="btn btn-info newproductClass" onclick="return createpro()"')*/?></span>-->
                                        <span class="input-group-btn fix-border newproductClass"  onclick="createpro(this.id)" > <span class="btn btn-info "><i class="icon-plus" title=""></i><?php echo $lang->requirementinside->newproduct?></span></span>
<!--                                        <span class="input-group-btn"><?php /*echo html::a($this->createLink('productplan','create','productID =0'),'<i class="icon-plus" title=""></i>'. $lang->requirementinside->newversion,'_blank','class="btn btn-info newversionClass" data-app="product" id="createp" onclick="return createPlan(this.id)"')*/?></span>-->
                                        <span class="input-group-btn newversionClass"  onclick="createPlan(this.id)" > <span class="btn btn-info "><i class="icon-plus" title=""></i><?php echo $lang->requirementinside->newversion; ?></span></span>
                    ...
                    <tr>
                        <th class='w-180px'><?php echo $lang->requirementinside->implementationForm; ?></th>
                        <td class='required' colspan='2'><?php echo html::select('fixType0',$lang->requirementinside->implementationFormList, '', "class='form-control chosen fixTypeClass' onchange='selectfix(this.id, this.value)'"); ?></td>
                        <td class='required' colspan='2'>
                            <div class='input-group'>
                                <span class='input-group-addon'><?php echo $lang->requirementinside->project; ?></span>
                                <?php echo html::select('project0', $plans, '', "class='form-control chosen projectClass' onchange='selectproject(this.id,this.value)'");?>
                            </div>
                        </td>
                    </tr>
                    <!--  <tr>
                         <th><?php /*echo $lang->requirementinside->execution;*/?></th>
...
<!--                    </tr>-->
                    <tr id='progressTr'>
                        <th><?php echo $lang->requirementinside->commentProgress;?></th>
                    ...
                    <tr>
                        <!--下一节处理人 -->
                        <th class='w-140px'><?php echo $lang->requirementinside->nextUser; ?></th>
                        <td colspan='2'><?php echo html::select('dealUser', $users, '', "class='form-control chosen'"); ?></td>
                        <!--抄送人 -->
                        <td colspan='2'>
                            <div class='input-group'>
                                <span class='input-group-addon'><?php echo $lang->requirementinside->mailto; ?></span>
...
<?php js::set('end', $requirement->end);?>
...
function addItem(obj)
{
   ...
   /* var tr9 = $(obj).closest('tr').nextAll('tr').eq(9).clone();*/
    ...
    tr1.find('.endDateTip').val(endDate);
    tr1.find('.endDateTip').attr({'id':'endDateTip' + descIndex});

    tr2.find('.approw').attr({'id':'app' + descIndex, 'name':'app' + descIndex });
    tr2.find('.acceptUserrow').attr({'id':'acceptUser' + descIndex, 'name':'acceptUser' + descIndex});
    tr2.find('.chosen-container').remove();

    tr3.find('.productClass').attr({'id': 'product' + descIndex, 'name':'product' + descIndex});
    tr3.find('.versionClass').attr({'id':'productPlan' + descIndex, 'name':'productPlan' + descIndex});
    tr3.find('.proandverClass').attr({'id':'proandver-' + descIndex});
    tr3.find('.newproductClass').attr({'id':'createproduct-' + descIndex});
    tr3.find('.newversionClass').attr({'id':'createp-' + descIndex});
    tr3.find('.chosen-container').remove();

    tr4.find('.fixTypeClass').attr({'id': 'fixType' + descIndex, 'name':'fixType' + descIndex});
    tr4.find('.projectClass').attr({'id': 'project' + descIndex, 'name':'project' + descIndex});
    tr4.find('.chosen-container').remove();
   ...
   /* tr5.find('.executionClass').attr({'id': 'execution' + descIndex, 'name':'execution' + descIndex});
    ...
    tr5.find('.form-control').attr({'id': 'desc' + descIndex, 'name': 'desc' + descIndex});

    tr6.find('.ke-container').remove();
    tr6.find('.form-control').attr({'id': 'reason' + descIndex, 'name': 'reason' + descIndex});
...
function delItem(obj)
{
   ...
   /* $("select[id^=execution]").each((index, item)=>
    ...
    $("input[type^=file]").each((index, item)=>
    {
       ...
       function selectProduct(productid, productvalue){
                   ...
                   bootbox.dialog({
                       ...
                       buttons:{
                           ...
                           blue:{
                               ...
                               callback:function(){
                                   window.location.href = createLink('requirementinside', 'browse');
                               }
                           }
                       }
                   })
               }
               $.get(createLink('demandinside', 'ajaxGetProductPlan', "productID=" + productvalue), function(data){
        ...
        function proandverRefresh(proandverId){
            ...
            $.get(createLink('requirementinside', 'ajaxGetProductCode', "app=" + app +"&data_id="+i), function(productlist)
            {
        ...
        function selectApp(id){
            ...
            $.get(createLink('requirementinside', 'ajaxGetProductCode', "app=" + app +"&data_id="+idstr), function(planList)
            {
    ...
    $.get(createLink('requirementinside', 'ajaxGetProductPlan', "productID=" + 0), function(planList)
    {
        ...
        $('.productPlanParent' + idstr).parent().parent().parent().find('.text-danger').remove();
            ...
            $.get(createLink('demandinside', 'ajaxGetSecondLine', "fixType="+'project' + "&app=" + null + "&sub=" + '1'), function(data)
            {
        ...
        function selectfix(id, value){
            ...
            var fixType = $('#fixType' + idstr).val();
            var app = $('#app' + idstr).val();
            $.get(createLink('demandinside', 'ajaxGetSecondLine', "fixType=" + fixType + "&app=" + app + "&sub=" + '1'), function(data)
            {
                $('#project' + idstr + '_chosen').remove();
                $('#project' + idstr).parent().addClass('projectParent' + idstr);
                $('#project' + idstr).replaceWith(data);
