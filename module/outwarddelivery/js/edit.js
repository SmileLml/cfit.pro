...
function checkDefects() {
    ...
    $.get(createLink('outwarddelivery', 'ajaxCheckDefects', 'defects=' + defects + '&outwarddeliveryId=' + outwarddeliveryId), function(data)
    {
...
function submitData() {
    ...
    $("#requirementId").removeAttr('disabled');
    $("#CBPprojectId").removeAttr('disabled');
    var formdata = $("#dataform").serialize();
    $.post(createLink('outwarddelivery', 'edit', 'id='+id),formdata,function (response) {
        ...
        if (response.result == 'success') {
            ...
            if (response.iframeUrl != '' && response.iframeUrl != undefined && (status == 'waitsubmitted' || status == 'wait' || status == 'reject' || status == 'reviewfailed')) {
                submitobject.show({url: response.iframeUrl})
            }
        }
        // if (response.result == 'success' && status != 'waitsubmitted' && status != 'wait' && status != 'reject'){
        //     $.zui.messager.success(response.message);
        //     setTimeout(function () {
        //       parent.location.href=response.locate
        //     },2000)
        // }
