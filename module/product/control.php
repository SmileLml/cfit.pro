...
class product extends control
{
    ...
    public function delete($productID, $confirm = 'no')
    {
        ...
        else
        {
            ...
            $this->product->changeApplication($productID, 'delete');
    ...
    {
        $executions = array();
        if(!empty($projectID)) $executions = $this->loadModel('project')->getExecutionByAvailable($projectID);
        $executions = empty($executions) ? array('' => '') : array('' => '') + $executions;
    ...
    public function ajaxGetProjectSelect($productID, $branch = 0, $projectID = 0, $extra = '')
    {
        $projects  = array('0' => '');
        $projects += $this->product->getProjectPairsByProduct($productID, $branch);

        if($extra == 'testcase') $extra = "onchange='getProjectExecutions(this.value)'";
        die(html::select('project', $projects, $projectID, "class='form-control' $extra"));
    }
    //判断产品编号是否在数据库中
    public function ajaxIsNew()
    {
        $value = str_replace(' ','',str_replace("-",'',$_POST['value']));
        $id = $_POST['id'];
        $codeinfos = $this->product->getCodeInfo($id);
        foreach ($codeinfos as $codeinfo){
            $codeInfo=str_replace('-','',$codeinfo->code);
            $codeInfo1=str_replace(' ','',$codeInfo);
            $codeArray[] = $codeInfo1;
        }
        if(in_array($value,$codeArray)){
            echo 1;
        }else{
            echo 0;
        }
     ...
     * @return void
     */
    public function ajaxGetProduct($productID = 0)
    {
        if(empty($productID)) die('0');
        $product = $this->product->getByID($productID);
        echo $product->app;
    }
}
