...
class taskModel extends model
{
    ...
    public function updateParentStatus($taskID, $parentID = 0, $createAction = true)
    {
        ...
        if($oldParentTask->parent != '0') $this->dao->update(TABLE_TASK)->set('parent')->eq('0')->where('id')->eq($parentID)->exec();
    ...
    public function computeHours4Multiple($oldTask, $task = null, $team = array(), $autoStatus = true)
    {
        ...
        if(!empty($team))
        {
            ...
            else
            {
                ...
                {
                    if(isset(($team[$oldTask->assignedTo]))){
        ...
        foreach($taskIDList as $taskID)
        {
            ...
            if($data->consumeds[$taskID])
            {
                ...
                else
                {
                    ...
                    $record->deptID   = $this->app->user->dept;
                    $record->task     = $taskID;
                    $record->date     = $today;
    ...
    public function recordEstimate($taskID)
    {
        ...
        foreach(array_keys($record->id) as $id)
        {
            ...
            if(!empty($record->work[$id]) or !empty($record->consumed[$id]))
            {
                ...
                $estimates[$id]->deptID   = $this->app->user->dept;
            }
        }
    ...
    public function activate($taskID)
    {
        ...
        $oldTask = $this->getById($taskID);
        if($oldTask->parent == '0') $this->config->task->activate->requiredFields = '';
     ...
     * @return array|bool
     */
    public function getListByIds($taskIds = [], $select = '*')
    {
        $data = [];
        if(!$taskIds){
            return $data;
        }
        $ret=  $this->dao->select($select)->from(TABLE_TASK)
            ->where('deleted')->eq('0')
    ...
    public function getUserTasks($account, $type = 'assignedTo', $limit = 0, $pager = null, $orderBy = "id_desc", $projectID = 0)
    {
        ...
        $tasks = $this->dao->select('t1.*, t2.id as executionID, t2.name as executionName, t3.id as storyID, t3.title as storyTitle, t3.status AS storyStatus, t3.version AS latestStoryVersion,t4.mark as mark')
            ->from(TABLE_TASK)->alias('t1')
            ->leftjoin(TABLE_PROJECT)->alias('t2')->on('t1.execution = t2.id')
            ->leftjoin(TABLE_STORY)->alias('t3')->on('t1.story = t3.id')
            ->leftjoin(TABLE_PROJECTPLAN)->alias('t4')->on('t1.project = t4.project') //20220610 增加 项目代号
     ...
     * @return void
     */
    public function printAssignedHtml($task, $users, $projectClosed = false)
    {
        $btnTextClass   = '';
        ...
        echo (!common::hasPriv('task', 'assignTo', $task) or $projectClosed) ? "<span style='padding-left: 21px' class='{$btnTextClass}'>{$assignedToText}</span>" : $assignToHtml;
    }
    ...
    {
        $users = $this->loadModel('user')->getTeamMemberPairs($task->id, 'task', 'nodeleted');
     ...
     * @return mixed
     */
    public function getProjectTask($app,$projectName,$flag){
        ...
        if(in_array($appName,$taskType)){
            ...
            if(array_keys($task)){
                $demandOrProOrSecond =  $this->loadModel('task')->getTaskDemandProblemDescAll(array_keys($task)); // 关联的问题单 需求单 二线工单
                foreach ($task as $key => $item){
                    $desc = isset($demandOrProOrSecond[$key]->desc) ? strip_tags($demandOrProOrSecond[$key]->desc) : (isset($demandOrProOrSecond[$key]->summary) ?strip_tags( $demandOrProOrSecond[$key]->summary ): '');
                    $task[$key] = $item."($desc)";
                }
            }
        }
     ...
     * @return mixed
     */
    public function getProjectAllTask($flag = null){
     ...
     * @return mixed
     */
    public function getVersionNumber($app, $project, $product, $version,$type){
        //工单列表不考虑选择的产品和产品版本
        $task =   $this->dao->select("typeid,code")->from(TABLE_TASK_DEMAND_PROBLEM)->alias('t1')
            ->leftJoin(TABLE_TASK)->alias('t2')->on('t1.taskid = t2.id')
            ->where('t1.deleted')->eq(0)
            ->andWhere('t1.project')->eq($project)
            ->andWhere('t1.application')->eq($app)
            ->beginIF(!($type == 'secondorder' && $version == 1))
                ->andWhere('t1.product')->eq($product)
                ->andWhere('t1.version')->eq($version)
            ->fi()

            ->beginIF($type == 'demand' || $type == 'demandionside')->andWhere('t1.type')->in('demand,demandinside')->fi()
            ->beginIF($type != 'demand' && $type != 'demandionside')->andWhere('t1.type')->in($type)->fi()
            ->andWhere('t2.deleted')->eq(0)
            ->andWhere('t2.dataVersion')->eq(2)
            ->orderBy('t1.id_asc')
            ->fetchPairs("typeid","code");
        return $task;
    }
    /**
     * 查询版本下所属任务和阶段
     ...
     * @return mixed
     */
    public function getExecutionTask($app,$project,$product,$version,$codes = null){
        if($codes){
            $codes = array_values(array_filter(array_unique($codes)));
            $tasks = [];
            foreach ($codes as $key => $code){
                $task =   $this->dao->select("t1.typeid,t1.code,t1.taskid,t1.execution")->from(TABLE_TASK_DEMAND_PROBLEM)->alias('t1')
                    ->leftJoin(TABLE_TASK)->alias('t2')->on('t1.taskid = t2.id')
                    ->where('t1.deleted')->eq(0)
                    ->andWhere('t1.project')->eq($project)
                    ->andWhere('t1.application')->eq($app)
                    ->beginIF(strpos($code,'CFIT-T-') === false)
                    ->andWhere('t1.product')->eq($product)
                    ->andWhere('t1.version')->eq($version)
                    ->fi()
                    ->andWhere('t1.type')->in('problem,demand,secondorder,demandinside')
                    ->andWhere('t1.code')->in($code)
                    ->andWhere('t2.deleted')->eq(0)
                    ->andWhere('t2.dataVersion')->eq(2)
                    ->orderBy('t1.id_asc')
                    ->fetchAll();
            ...
            $tasks =   $this->dao->select("t1.typeid,t1.code,t1.taskid,t1.execution")->from(TABLE_TASK_DEMAND_PROBLEM)->alias('t1')
                ->leftJoin(TABLE_TASK)->alias('t2')->on('t1.taskid = t2.id')
                ->where('t1.deleted')->eq(0)
                ->andWhere('t1.project')->eq($project)
                ->andWhere('t1.application')->eq($app)
        ...
        $executionList = isset($executionList) ? $executionList : '';
   ...
   /**
     ...
     * @return array|string
     */
    public function getByTaskToOldTask($taskID){
        $task = $this->dao->select('comment')->from(TABLE_ACTION)->where('comment')->like("%$taskID%")->fetch();
        if($task){
            $comment = str_replace('：',':',$task->comment);
            $taskName = strstr($comment,'新任务',true);
            $taskOld =  strstr(ltrim(strstr(strstr($comment,'新任务',true),':'),':'),':',true);
            if($taskOld){
               $effort = $this->loadModel('effort')->getByObject('task', $taskOld); //历史工作量
                unset($effort['typeList']);
            }
        }
        return $task ? array('taskName' =>$taskName,'taskOld' =>$taskOld,'effort' => $effort) : '';
    }
}
