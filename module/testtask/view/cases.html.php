...
<div id='mainContent' class='main-row fade'>
  ...
  <div class='main-col'>
    ...
    <form class='main-table table-cases' data-hot='true' method='post' name='casesform' id='casesForm' <?php if(!$useDatatable) echo "data-ride='table'";?>>
      ...
      $canBatchConfirm = common::hasPriv('testtask', 'batchConfirmChange');

      $canBatchAction = ($canBeChanged and ($canBatchEdit or $canBatchUnlink or $canBatchAssign or $canBatchRun or $canBatchConfirm));
        ...
        <table class='table has-sort-head<?php if($useDatatable) echo ' datatable';?>' id='caseList' data-fixed-left-width='<?php echo $widths['leftWidth']?>' data-fixed-right-width='<?php echo $widths['rightWidth']?>' data-checkbox-name='caseIDList[]'>
          ...
          <tbody>
            ...
            <tr data-id='<?php echo $run->id?>'>
              <?php foreach($setting as $key => $value) $this->testtask->printCell($value, $run, $users, $task, $branches, $useDatatable ? 'datatable' : 'table', $products, $projects);?>
            </tr>
            <?php endforeach;?>
          </tbody>
        </table>
      ...
      <div class='table-footer'>
        ...
        <div class='table-actions btn-toolbar'>
          <div class='btn-group dropup'>
            <?php
            $actionLink = $this->createLink('testcase', 'batchEdit', "applicationID=$applicationID&productID=$productID");
            $misc       = $canBatchEdit ? "onclick=\"setFormAction('$actionLink')\"" : "disabled='disabled'";
            echo html::commonButton($lang->edit, $misc);
            ?>
            <?php if($canBatchUnlink or $canBatchConfirm):?>
            ...
            <ul class='dropdown-menu'>
              ...
              if($canBatchConfirm)
              {
                  $actionLink = $this->createLink('testtask', 'batchConfirmChange', "taskID=$task->id");
                  $misc       = "onclick=\"setFormAction('$actionLink')\"";
                  echo "<li>" . html::a('javascript:;', $lang->testtask->confirmChange, '', $misc) . "</li>";
              }
              ?>
            </ul>
            <?php endif;?>
          ...
          if($canBatchRun)
          {
              $actionLink = inLink('batchRun', "applicationID=$applicationID&productID=$productID&orderBy=id_desc&from=testtask&taskID=$taskID");
